...
void extractDataFromCustomFile(FILE* EntriesFile, bool url, RawData& data)
{
    char buf[200];
    ...
    fgets(buf,200,EntriesFile); sscanf(buf,"%s",num_str);
    fgets(buf,200,EntriesFile); sscanf(buf,"%s",repo_str);
    fgets(buf,200,EntriesFile); sscanf(buf,"%s",data.date_str);
    fgets(buf,200,EntriesFile); sscanf(buf,"%s",data.time_str);
...
bool extractDataFromCustomFile(std::string filename, bool url, RawData& data)
{
    FILE* EntriesFile = fopen(filename.c_str(), "r");
    if(!EntriesFile)
        return false;

    extractDataFromCustomFile(EntriesFile,url,data);
    fclose(EntriesFile);
    return true;
}
...
int main(int argc, char **argv)
{
    ...
    bool custom_file = true;
    std::string path;
    std::string outfile = "revision.h";
    std::string infile = "revision";
...

    {
        ...
        if (custom_file)
            res = extractDataFromCustomFile(path+"revision",use_url,data);
