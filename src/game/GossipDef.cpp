...
void PlayerMenu::SendQuestGiverQuestDetails(Quest const* pQuest, ObjectGuid guid, bool ActivateAccept)
{
    ...
    else*/
    {
        ...
        {
            float moneyMod = GetMenuSession()->IsPremiumActive() && pQuest->GetRewOrReqMoney() > 0 ? GetMenuSession()->GetPlayer()->GetPremiumMoneyModifier() : 1.0f;
            data << uint32(pQuest->GetRewOrReqMoney() * moneyMod);
        }

        float xpMod = GetMenuSession()->IsPremiumActive() ? GetMenuSession()->GetPlayer()->GetPremiumXPModifier() : 1.0f;
        data << uint32(pQuest->XPValue(GetMenuSession()->GetPlayer())*sWorld.getConfig(CONFIG_FLOAT_RATE_XP_QUEST)*xpMod);
    }
...
void PlayerMenu::SendQuestQueryResponse( Quest const *pQuest )
{
    ...
    {
        float moneyMod = GetMenuSession()->IsPremiumActive() && pQuest->GetRewOrReqMoney() > 0 ? GetMenuSession()->GetPlayer()->GetPremiumMoneyModifier() : 1.0f;
        data << uint32(pQuest->GetRewOrReqMoney() * moneyMod); // reward money (below max lvl)
    }
