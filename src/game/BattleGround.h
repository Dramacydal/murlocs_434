...
class ArenaChartHolder
{
    friend class BattleGround;

    public:
        ArenaChartHolder() { bg = NULL; }
        ~ArenaChartHolder() {}
        std::map<ObjectGuid, ArenaChartElement> chart;
        void SaveToDB();

        void FillArenaChartAtStart(GroupQueueInfo* ginfo);
        void FillArenaChartAtEnd(ArenaTeam* winner, ArenaTeam* loser, uint32 winner_rating = 0, uint32 loser_rating = 0, int32 winner_change = 0, int32 loser_change = 0);
        void FillArenaChartAtPlayerLeave(ObjectGuid guid, ArenaTeam* playerTeam, ArenaTeam* otherTeam, Player* player = NULL);
        void FillGaps();
...
#define ARENA_TIMEOUT_RATING_CHANGE -16
...
class BattleGround
{
    ...
    public:
        ...
        virtual uint32 GetPlayerScore(Player *Source, uint32 type);
        ...
        void SetArenaMatchmakerRating(Team team, uint32 MMR){ m_ArenaTeamMMR[GetTeamIndex(team)] = MMR; }
        uint32 GetArenaMatchmakerRating(Team team) const            { return m_ArenaTeamMMR[GetTeamIndex(team)]; }
        ...
        void SetEventRespawn(uint8 event1, uint8 event2, uint32 respawnTime);
        ...
        void CloseDoorEvent(uint8 event1, uint8 event2 = 0);
        ...
        virtual bool IsWithinLOSInBG(float x1, float y1, float z1, float x2, float y2, float z2)
        {
            DEBUG_LOG("Dynamic LOS: BG type %u has no overloaded function for LoS check", GetTypeID(true));
            return true;
        }
        ...
        void SetArenaGameId(uint32 gameId) { m_ArenaGameId = gameId; }
        uint32 GetArenaGameId() { return m_ArenaGameId; }

        void UpdateAchievementCriteriaOnTeam(Team team, AchievementCriteriaTypes type, uint32 miscvalue1 = 0, uint32 miscvalue2 = 0, Unit* unit = NULL, uint32 time = 0);

        virtual void HandlePlayerLogin(Player* player) { }

        ArenaChartHolder& GetArenaChartHolder() { return m_arenaChartHolder; }
    ...
    protected:
        ...
        */
        ...
        bool   m_BuffChange;
        bool   m_IsRandom;
        uint32 m_ArenaGameId;
        ArenaChartHolder m_arenaChartHolder;
    ...
    private:
        ...
        uint32 m_ArenaTeamMMR[PVP_TEAM_COUNT];
