...
void WardenWin::Init(WorldSession *pClient, BigNumber *K)
{
    ...
    */
    ...
    DEBUG_LOG("WARDEN: Server side warden (Win) for client %u initializing...", pClient->GetAccountId());
...
void WardenWin::HandleData(ByteBuffer &buff)
{
    ...
    //Ranger: found but not in world? kick him!
    else if (found && !Client->GetPlayer())
        Client->KickPlayer();
}