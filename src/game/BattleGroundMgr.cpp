    ...
    //add players from group to ginfo
    {
        ...
        if (grp)
        {
            for(GroupReference *itr = grp->GetFirstMember(); itr != NULL; itr = itr->next())
            {
                ...
                pl_info.preferedBG       = leader->GetPreferedBG();
...
void BattleGroundQueue::RemovePlayer(ObjectGuid guid, bool decreaseInvitedCount)
{
    ...
    {
        ERROR_LOG("BattleGroundQueue: couldn't find for remove: %s", guid.GetString().c_str());
        return;
    }
    ...
    if (bracket_id == -1)
    {
        ERROR_LOG("BattleGroundQueue: ERROR Cannot find groupinfo for %s", guid.GetString().c_str());
        return;
    }
...
void BattleGroundQueue::Update(BattleGroundTypeId bgTypeId, BattleGroundBracketId bracket_id, ArenaType arenaType, bool isRated, uint32 arenaRating)
{
    ...
    if (!bg_template)
    {
        ERROR_LOG("Battleground: Update: bg template not found for %u", bgTypeId);
        return;
    }
    ...
    if (!bracketEntry)
    {
        ERROR_LOG("Battleground: Update: bg bracket entry not found for map %u bracket id %u", bg_template->GetMapId(), bracket_id);
        return;
    }
    ...
    if (bg_template->isBattleGround())
    {
        ...
        if (CheckPremadeMatch(bracket_id, MinPlayersPerTeam, MaxPlayersPerTeam))
        {
            ...
            if (!bg2)
            {
                ERROR_LOG("BattleGroundQueue::Update - Cannot create battleground: %u", bgTypeId);
                return;
            }
    ...
    if (!isRated)
    {
            ...
            || (bg_template->isArena() && CheckSkirmishForSameFaction(bracket_id, MinPlayersPerTeam)) )
        {
            ...
            BattleGround * bg2 = sBattleGroundMgr.CreateNewBattleGround(bgTypeId, bracketEntry, arenaType, false, m_SelectionPools);
            if (!bg2)
            {
                ERROR_LOG("BattleGroundQueue::Update - Cannot create battleground: %u", bgTypeId);
                return;
    ...
    else if (bg_template->isArena())
    {
        ...
        if (m_SelectionPools[TEAM_INDEX_ALLIANCE].GetPlayerCount() && m_SelectionPools[TEAM_INDEX_HORDE].GetPlayerCount())
        {
            ...
            if (!arena)
            {
                ERROR_LOG("BattlegroundQueue::Update couldn't create arena instance for rated arena match!");
                return;
            }

            (*(itr_team[TEAM_INDEX_ALLIANCE]))->OpponentsTeamRating = (*(itr_team[TEAM_INDEX_HORDE]))->ArenaTeamRating;
            (*(itr_team[TEAM_INDEX_ALLIANCE]))->OpponentsMatchmakerRating = (*(itr_team[TEAM_INDEX_HORDE]))->OpponentsMatchmakerRating;
            ...
            (*(itr_team[TEAM_INDEX_HORDE]))->OpponentsMatchmakerRating = (*(itr_team[TEAM_INDEX_ALLIANCE]))->OpponentsMatchmakerRating;
            ...
            arena->SetArenaMatchmakerRating(ALLIANCE, (*(itr_team[TEAM_INDEX_ALLIANCE]))->ArenaMatchmakerRating);
            arena->SetArenaMatchmakerRating(   HORDE, (*(itr_team[TEAM_INDEX_HORDE]))->ArenaMatchmakerRating);

            InviteGroupToBG(*(itr_team[TEAM_INDEX_ALLIANCE]), arena, ALLIANCE);
            InviteGroupToBG(*(itr_team[TEAM_INDEX_HORDE]), arena, HORDE);

            arena->GetArenaChartHolder().FillArenaChartAtStart(*(itr_team[TEAM_INDEX_ALLIANCE]));
            arena->GetArenaChartHolder().FillArenaChartAtStart(*(itr_team[TEAM_INDEX_HORDE]));
...
BattleGround * BattleGroundMgr::CreateNewBattleGround(BattleGroundTypeId bgTypeId, PvPDifficultyEntry const* bracketEntry, ArenaType arenaType, bool isRated, BattleGroundQueue::SelectionPool* selectionPool)
{
    // get the template BG
    BattleGround *bg_template = GetBattleGroundTemplate(bgTypeId);
    if (!bg_template)
    {
        ERROR_LOG("BattleGround: CreateNewBattleGround - bg template not found for %u", bgTypeId);
        return NULL;
    }
    ...
    if (bgTypeId == BATTLEGROUND_RB)
    {
        if (selectionPool)
        {
            std::map<int8, uint32> bgScores;
            for (uint32 i = 0; i < PVP_TEAM_COUNT; i++)
                for (BattleGroundQueue::GroupsQueueType::const_iterator citr = selectionPool[TEAM_INDEX_ALLIANCE + i].SelectedGroups.begin(); citr != selectionPool[TEAM_INDEX_ALLIANCE + i].SelectedGroups.end(); ++citr)
                {
                    GroupQueueInfo* ginfo = *citr;
                    if (ginfo->IsInvitedToBGInstanceGUID)
                        continue;

                    for (GroupQueueInfoPlayers::iterator itr = ginfo->Players.begin(); itr != ginfo->Players.end(); ++itr)
                    {
                        uint8 preferedBG = itr->second->preferedBG;
                        if (bgScores.find(preferedBG) == bgScores.end())
                            bgScores[preferedBG] = 1;
                        else
                            ++bgScores[preferedBG];
                    }
                }
            ...
            uint32 num = 0;
            for (std::map<int8, uint32>::iterator itr = bgScores.begin(); itr != bgScores.end(); ++itr)
                if (itr->second > num)
                {
                    bgTypeId = BattleGroundTypeId(itr->first);
                    num = itr->second;
                }
        }
    ...
    if (bg->isArena() && bg->isRated())
        bg->SetArenaGameId(sObjectMgr.GenerateArenaGameId());

    if (bg->isArena())
        bg->GetBgMap()->SetVisibilityDistance(50.0f);
...
void BattleGroundMgr::CreateInitialBattleGrounds()
{
    ...
    do
    {
        ...
        if (!bl)
        {
            ERROR_LOG("Battleground ID %u not found in BattlemasterList.dbc. Battleground not created.", bgTypeID_);
            continue;
        }
...
void BattleGroundMgr::SendToBattleGround(Player *pl, uint32 instanceId, BattleGroundTypeId bgTypeId)
{
    ...
    {
        ERROR_LOG("player %u trying to port to nonexistent bg instance %u",pl->GetGUIDLow(), instanceId);
    }
}
