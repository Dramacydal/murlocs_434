...
int32 ReputationMgr::GetReputation(uint32 faction_id) const
{
    ...
    if (!factionEntry)
    {
        ERROR_LOG("ReputationMgr::GetReputation: Can't get reputation of %s for unknown faction (faction id) #%u.",m_player->GetName(), faction_id);
        return 0;
    }
    ...
    if(result)
    {
        do
        {
            ...
            if( factionEntry && (factionEntry->reputationListID >= 0))
            {
                ...
                uint16 dbFactionFlags = fields[2].GetUInt16();
