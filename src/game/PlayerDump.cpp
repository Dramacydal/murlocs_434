...
std::string PlayerDumpWriter::GetDump(uint32 guid)
{
    ...
    if (result)
    {
        ...
        if (!reqName.empty())
        {
            ...
            ERROR_LOG("Table 'character_db_version' not have revision guard field, revision guard query not added to pdump.");

        delete result;
    }
    else
        ERROR_LOG("Character DB not have 'character_db_version' table, revision guard query not added to pdump.");
...
DumpReturn PlayerDumpReader::LoadDump(const std::string& file, uint32 account, std::string name, uint32 guid)
{
    ...
    while(!feof(fin))
    {
        ...
        if (tn.empty())
        {
            ERROR_LOG("LoadPlayerDump: Can't extract table name from line: '%s'!", line.c_str());
            ROLLBACK(DUMP_FILE_BROKEN);
        }
        ...
        if (!dTable->isValid())
        {
            ERROR_LOG("LoadPlayerDump: Unknown table: '%s'!", tn.c_str());
            ROLLBACK(DUMP_FILE_BROKEN);
        }
        ...
        switch(type)
        {
            ...
            default:
                ERROR_LOG("Unknown dump table type: %u",type);
                break;
        }
