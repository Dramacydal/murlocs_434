...
void Channel::List(Player* player)
{
    ...
    else
    {
        ...
        for(PlayerList::const_iterator i = m_players.begin(); i != m_players.end(); ++i)
        {
            ...
            if (plr && (player->GetSession()->GetSecurity() > SEC_CURATOR || plr->GetSession()->GetSecurity() <= gmLevelInWhoList) &&
...
void Channel::Say(ObjectGuid p, const char *what, uint32 lang, bool ignoreOn)
{
    if (!what)
        return;
