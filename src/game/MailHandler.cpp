...
void WorldSession::HandleSendMail(WorldPacket & recv_data )
{
    ...
    if (!sWorld.getConfig(CONFIG_BOOL_ALLOW_TWO_SIDE_INTERACTION_MAIL) && pl->GetTeam() != rc_team && GetSecurity() <= SEC_CURATOR)
    {
    ...
    if (items_count > 0 || money > 0)
    {
        if (items_count > 0)
        {
            ...
            {
                Item* item = items[i];
                if (GetSecurity() > SEC_CURATOR && sWorld.getConfig(CONFIG_BOOL_GM_LOG_TRADE))
                {
                ...
                pl->DeleteRefundReference(item->GetObjectGuid());
        ...
        if (money > 0 &&  GetSecurity() > SEC_CURATOR && sWorld.getConfig(CONFIG_BOOL_GM_LOG_TRADE))
        {
