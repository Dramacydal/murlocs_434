...
Map::~Map()
{
    ...
    if (GetPersistentState())
        GetPersistentState()->SetUsedByMapState(NULL);
  ...
  i_data(NULL), i_script_id(0)
{
    ...
    summProcessTime = 0;
    higestProcessTime = 0;
    normalProcessTime = sWorld.getConfig(CONFIG_UINT32_INTERVAL_MAPUPDATE);
    lastInfoDump = WorldTimer::getMSTime();
#endif
...
#ifdef DUMP_MAP_STAT
    uint32 totalProcessTime = WorldTimer::getMSTime() - proc_startTime;
    summProcessTime += totalProcessTime;
    processionsCount++;
    if (higestProcessTime < totalProcessTime)
    higestProcessTime = totalProcessTime;
    if (totalProcessTime > normalProcessTime)
    lagProcCount++;
    if (WorldTimer::getMSTime() - lastInfoDump >= 60000)
    {
        LoginDatabase.PQuery("INSERT INTO map_stat (realmId, map, instance, avgProcessTime, hiProcessTime, lagPercent) VALUES (%u, %u, %u, %u, %u, %u)", 
            realmID, GetId(), GetInstanceId(), uint32(summProcessTime / processionsCount), higestProcessTime, uint32((lagProcCount * 100.0f) / processionsCount));
        processionsCount = 0;
        lagProcCount = 0;
        summProcessTime = 0;
        higestProcessTime = 0;
...
Map::Remove(T *obj, bool remove)
{
    ...
    DEBUG_LOG("Map::Remove: Remove %s from grid[%u,%u]", obj->GetGuidStr().c_str(), cell.data.Part.grid_x, cell.data.Part.grid_y);
    NGridType *grid = getNGrid(cell.GridX(), cell.GridY());
    MANGOS_ASSERT( grid != NULL );
...
void Map::SendInitSelf( Player * player )
{
    ...
    if (Transport* transport = player->GetTransport())
    {
        ...
        {
            Unit* passenger = GetUnit(itr->guid);
            if (!passenger)
                continue;

            if (player->GetObjectGuid() != itr->guid && player->HaveAtClient(passenger))
        ...
        if (pGroup)
        {
            ...
            else
            {
                ...
                if (groupBind->state != GetPersistentState())
                {
                    ...
                    ERROR_LOG("MapSave players: %d, group count: %d",
 ...
 */
void Map::MonsterYellToMap(ObjectGuid guid, int32 textId, uint32 language, Unit* target, uint32 zoneId)
{
    if (guid.IsAnyTypeCreature())
    {
        ...
        MonsterYellToMap(cInfo, textId, language, target, guid.GetCounter(), zoneId);
    }
    else
    {
 ...
 */
void Map::MonsterYellToMap(CreatureInfo const* cinfo, int32 textId, uint32 language, Unit* target, uint32 senderLowGuid /*= 0*/, uint32 zoneId /*=0*/)
{
