...
void GuildMgr::LoadGuilds()
{
    ...
    //   7               8    9    10         11        12    13         14              15
        ...
        "FROM guild WHERE guild.guildid >= %u AND guild.guildid < %u ORDER BY guildid ASC", startGuid, hiGuid);
    ...
    //                                                                      0       1     2   3       4
    QueryResult* guildBankTabRightsResult = CharacterDatabase.PQuery("SELECT guildid,TabId,rid,gbright,SlotPerDay FROM guild_bank_right WHERE guildid >= %u AND guildid < %u ORDER BY guildid ASC, TabId ASC", startGuid, hiGuid);
