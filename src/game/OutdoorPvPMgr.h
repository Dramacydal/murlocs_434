...
class OutdoorPvPMgr
{
    public:
        ...
        // called when a player enters an world pvp zone
        void HandlePlayerEnterZone(Player* pPlayer, uint32 uiZoneId);

        // called when player leaves an world pvp zone
        void HandlePlayerLeaveZone(Player* pPlayer, uint32 uiZoneId);

        // called when player enters an area in world pvp zone
        void HandlePlayerEnterArea(Player* pPlayer, uint32 uiZoneId, uint32 uiAreaId);

        // called when player leaves an area in world pvp zone
        void HandlePlayerLeaveArea(Player* pPlayer, uint32 uiZoneId, uint32 uiAreaId);
        ...
        OutdoorPvP* GetOutdoorPvPById(uint8 id);
        OutdoorPvP* GetOutdoorPvPByBattlefieldId(uint8 id);
