...
void WorldSession::HandleSellItemOpcode( WorldPacket & recv_data )
{
    ...
    if (pItem)
    {
        ...
        if (pItem->HasFlag(ITEM_FIELD_FLAGS, ITEM_DYNFLAG_REFUNDABLE))
        ...
        if (pProto)
        {
            if (pProto->SellPrice > 0)
            {
                if (count < pItem->GetCount())              // need split items
                {
                    ...
                    if (!pNewItem)
                    {
                        ERROR_LOG("WORLD: HandleSellItemOpcode - could not create clone of item %u; count = %u", pItem->GetEntry(), count );
                        _player->SendSellError(SELL_ERR_CANT_SELL_ITEM, pCreature, itemGuid, 0);
                        return;
                    }
...
void WorldSession::SendListInventory(ObjectGuid vendorguid)
{
    ...
    for (uint8 vendorslot = 0; vendorslot < numitems; ++vendorslot)
    {
        ...
        if (crItem)
        {
            ...
            if (pProto)
            {
                // skip checking requirements, if player is gamemaster or item has ITEM_FLAG_BOA flag
                if (!_player->isGameMaster() && (pProto->Flags & ITEM_FLAG_BOA) == 0)
                {
...
{
    DEBUG_LOG("WORLD: CMSG_ITEM_REFUND_INFO");
    ...
    if (!item)
    {
        DEBUG_LOG("Item refund info: item %s not found!", itemGuid.GetString().c_str());
        return;
    }

    if (!(item->GetProto()->Flags & ITEM_FLAG_REFUNDABLE))
    {
        DEBUG_LOG("Item refund info: item %s not refundable!", itemGuid.GetString().c_str());
        return;
    }
...
void WorldSession::HandleItemRefund(WorldPacket &recv_data)
{
    ...
    if (!item)
    {
        DEBUG_LOG("Item refund: item %s not found!", itemGuid.GetString().c_str());
        return;
    }

    if (!(item->GetProto()->Flags & ITEM_FLAG_REFUNDABLE))
    {
        DEBUG_LOG("Item refund: item %s not refundable!", itemGuid.GetString().c_str());
        return;
    }
