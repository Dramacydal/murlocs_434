...
namespace Movement
{
    ...
    void PacketBuilder::WriteMonsterMove(const MoveSpline& move_spline, WorldPacket& data)
    {
        ...
        if (splineflags & MoveSplineFlag::UncompressedPath)
        {
            if (splineflags.cyclic)
                WriteCatmullRomCyclicPath(move_spline, data);
            else
                WriteCatmullRomPath(move_spline, data);
        }
        else
            WriteLinearPath(move_spline, data);
    }
    ...
    Vector3 PacketBuilder::CalcTransportOffset(const MoveSpline& move_spline, Vector3 oldVec)
    {
        ...
        result.z = oldVec.z - move_spline.transportPos.z;

        float a1 = oldVec.x,
            a2 = oldVec.y,
            b1 = move_spline.transportPos.x,
            b2 = move_spline.transportPos.y,
            c = cosf(move_spline.transportPos.orientation),
            s = sinf(move_spline.transportPos.orientation);

        if (fabs((b1 - a1)/s - (a2 - b2)/c) < G3D::fnan())
        {
            result.x = 0.0f;
            result.y = (a2 - b2) / c;
        }
        ...
        {
            result.y = c * (a2 - b2) - s * (a1 - b1);
            result.x = (a1 - b1 + result.y * s) / c;
        }
