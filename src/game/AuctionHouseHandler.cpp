...
void WorldSession::HandleAuctionRemoveItem(WorldPacket & recv_data)
{
    ...
    if (!auction || auction->owner != pl->GetGUIDLow())
    {
        ...
        ERROR_LOG("CHEATER : %u, he tried to cancel auction (id: %u) of another player, or auction is NULL", pl->GetGUIDLow(), auctionId);
        return;
    }
    ...
    if (!pItem)
    {
        ERROR_LOG("Auction id: %u has nonexistent item (item guid : %u)!!!", auction->Id, auction->itemGuidLow);
...
void WorldSession::HandleAuctionListBidderItems(WorldPacket & recv_data)
{
    ...
    {
        ERROR_LOG("Client sent bad opcode!!! with count: %u and size : %u (must be: %u)", outbiddedCount, (uint32)recv_data.size(), (16 + outbiddedCount * 4));
        outbiddedCount = 0;
    }
