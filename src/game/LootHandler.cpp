...
void WorldSession::HandleAutostoreLootItemOpcode( WorldPacket & recv_data )
{
    ...
    switch( lguid.GetHigh())
    {
        ...
        default:
        {
            ERROR_LOG("%s is unsupported for looting.",lguid.GetString().c_str());
            return;
        }
    }
    ...
    if ( msg == EQUIP_ERR_OK )
    {
        AllowedLooterSet* looters = item->IsEligibleForSoublountTrade(player) ? loot->GetAllowedLooters() : NULL;
...
void WorldSession::DoLootRelease(ObjectGuid lguid)
{
    ...
    switch(lguid.GetHigh())
    {
        case HIGHGUID_GAMEOBJECT:
        {
            ...
            // add autodespawn time in non-instanceable maps
            else if (go->GetGoType() == GAMEOBJECT_TYPE_CHEST && !go->GetMap()->Instanceable())
...
void WorldSession::HandleLootMasterGiveOpcode( WorldPacket & recv_data )
{
    ...
    AllowedLooterSet* looters = item.IsEligibleForSoublountTrade(target) ? pLoot->GetAllowedLooters() : NULL;
