...
void WorldSession::HandleDuelAcceptedOpcode(WorldPacket& recvPacket)
{
    ...
    if (sWorld.getConfig(CONFIG_BOOL_FUN_DUEL_RESET_STATS))
    {
        if (!pl->GetMap()->Instanceable() && !pl->IsFFAPvP())
        {
            ...
            {
                pl->SetPower(power, pl->GetMaxPower(power));
                if (power == POWER_ENERGY && pl->getClass() == CLASS_DRUID)
        ...
        if (!plTarget->GetMap()->Instanceable() && !plTarget->IsFFAPvP())
        {
            ...
            if (power == POWER_MANA || power == POWER_ENERGY)
            {
                plTarget->SetPower(power, plTarget->GetMaxPower(power));
                if (power == POWER_ENERGY && plTarget->getClass() == CLASS_DRUID)
