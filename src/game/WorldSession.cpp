...
{
    m_isPremium = false;
    m_premiumType = PREMIUM_DEFAULT;
    m_premiumRateType = PREMIUM_RATE_XP;
    m_PremiumTimeEnd = 0;
...
{
    ERROR_LOG("Client (account %u) send packet %s (%u) with size " SIZEFMTD " but expected %u (attempt crash server?), skipped",
...
{
    DEBUG_LOG( "SESSION: opcode %s (0x%.4X) have unprocessed tail data (read stop at " SIZEFMTD " from " SIZEFMTD ")",
        LookupOpcodeName(packet->GetOpcode()),
        packet->GetOpcode(),
...
bool WorldSession::Update(PacketFilter& updater)
{
    ...
    while (m_Socket && !m_Socket->IsClosed() && _recvQueue.next(packet, updater))
    {
        ...
        try
        {
            ...
            switch (opHandle.status)
            {
                ...
                case STATUS_AUTHED:
                    ...
                    break;
                case STATUS_NEVER:
                    DEBUG_LOG( "SESSION: received not allowed opcode %s (0x%.4X)",
                        LookupOpcodeName(packet->GetOpcode()),
                        packet->GetOpcode());
                    break;
                case STATUS_UNHANDLED:
                    ...
                    break;
                default:
                    ERROR_LOG("SESSION: received wrong-status-req opcode %s (0x%.4X)",
                        LookupOpcodeName(packet->GetOpcode()),
                        packet->GetOpcode());
                    break;
            }
        ...
        {
            ERROR_LOG("WorldSession::Update ByteBufferException occured while parsing a packet (opcode: %u) from client %s, accountid=%i.",
...
void WorldSession::LogoutPlayer(bool Save)
{
    ...
    if (_player)
    {
        ...
        if(BattleGround *bg = _player->GetBattleGround())
            ...
            {
                _player->ScheduleDelayedOperation(DELAYED_REMOVE_SPECTATOR);
...
{
        ERROR_LOG("SESSION: received unimplemented opcode %s (0x%.4X)",
...
{
    ERROR_LOG( "SESSION: received opcode %s (0x%.4X) that must be processed in WorldSocket::OnRead",
        LookupOpcodeName(recvPacket.GetOpcode()),
        recvPacket.GetOpcode());
}
...
{
        ERROR_LOG("SESSION: received server-side opcode %s (0x%.4X)",
        LookupOpcodeName(recvPacket.GetOpcode()),
        recvPacket.GetOpcode());
}
...
{
    ERROR_LOG( "SESSION: received deprecated opcode %s (0x%.4X)",
        LookupOpcodeName(recvPacket.GetOpcode()),
        recvPacket.GetOpcode());
}
...
void WorldSession::LoadAccountData(QueryResult* result, uint32 mask)
{
    ...
    do
    {
        ...
        {
            ERROR_LOG("Table `%s` have invalid account data type (%u), ignore.",
        ...
        if ((mask & (1 << type))==0)
        {
            ERROR_LOG("Table `%s` have non appropriate for table  account data type (%u), ignore.",
                mask == GLOBAL_CACHE_MASK ? "account_data" : "character_account_data", type);
            continue;
        }
...
void WorldSession::ReadAddonsInfo(WorldPacket &data)
{
    ...
    if(size > 0xFFFFF)
    {
        ERROR_LOG("WorldSession::ReadAddonsInfo addon info too big, size %u", size);
        return;
    }
    ...
    if (uncompress(const_cast<uint8*>(addonInfo.contents()), &uSize, const_cast<uint8*>(data.contents() + pos), data.size() - pos) == Z_OK)
    {
        ...
        ERROR_LOG("Addon packet uncompress error!");
}
...
void WorldSession::HandleBfQueueInviteResponse(WorldPacket& recv_data)
{
    ...
    if (OutdoorPvPWG* opvp = (OutdoorPvPWG*)sOutdoorPvPMgr.GetOutdoorPvPByBattlefieldId(uiBattlefieldId))
