...
void WorldSession::HandleAttackSwingOpcode( WorldPacket & recv_data )
{
    ...
    if(!guid.IsUnit())
    {
        ERROR_LOG("WORLD: %s isn't unit", guid.GetString().c_str());
        return;
    }
    ...
    if(!pEnemy)
    {
        ERROR_LOG( "WORLD: Enemy %s not found", guid.GetString().c_str());
    ...
    {
        ERROR_LOG( "WORLD: Enemy %s is friendly",guid.GetString().c_str());
...
void WorldSession::HandleSetSheathedOpcode( WorldPacket & recv_data )
{
    ...
    {
        ERROR_LOG("Unknown sheath state %u ??",sheathed);
        return;
    }
