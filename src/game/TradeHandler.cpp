    ...
    for(int i = 0; i < TRADE_SLOT_TRADED_COUNT; ++i)
    {
        ...
        if (traderCanTrade && playerCanTrade )
        {
            ...
            if (myItems[i])
            {
                ...
                if (_player->GetSession()->GetSecurity() > SEC_CURATOR && sWorld.getConfig(CONFIG_BOOL_GM_LOG_TRADE))
                {
                ...
                {
                    uint32 timeDiff = trader->GetTotalPlayedTime() + myItems[i]->GetPlayedTimeField();
                    timeDiff = timeDiff >= _player->GetTotalPlayedTime() ? timeDiff - _player->GetTotalPlayedTime() : 0;
                    myItems[i]->SetPlayedTimeField(timeDiff);
                }
                if (myItems[i]->HasFlag(ITEM_FIELD_FLAGS, ITEM_DYNFLAG_REFUNDABLE))
                {
                    myItems[i]->SetRefundable(false);
                    _player->DeleteRefundReference(myItems[i]->GetObjectGuid());
                }
            ...
            if (hisItems[i])
            {
                ...
                if (trader->GetSession()->GetSecurity() > SEC_CURATOR && sWorld.getConfig(CONFIG_BOOL_GM_LOG_TRADE))
                {
                ...
                {
                    uint32 timeDiff = _player->GetTotalPlayedTime() + hisItems[i]->GetPlayedTimeField();
                    timeDiff = timeDiff >= trader->GetTotalPlayedTime() ? timeDiff - trader->GetTotalPlayedTime() : 0;
                    hisItems[i]->SetPlayedTimeField(timeDiff);
                }
                if (hisItems[i]->HasFlag(ITEM_FIELD_FLAGS, ITEM_DYNFLAG_REFUNDABLE))
                {
                    hisItems[i]->SetRefundable(false);
                    trader->DeleteRefundReference(hisItems[i]->GetObjectGuid());
                }
        ...
        else
        {
            ...
            {
                if (!traderCanTrade)
                    ERROR_LOG("trader can't store item: %s", myItems[i]->GetGuidStr().c_str());
                if (_player->CanStoreItem( NULL_BAG, NULL_SLOT, playerDst, myItems[i], false ) == EQUIP_ERR_OK)
                    _player->MoveItemToInventory(playerDst, myItems[i], true, true);
                else
                    ERROR_LOG("player can't take item back: %s", myItems[i]->GetGuidStr().c_str());
            }
            // return the already removed items to the original owner
            if (hisItems[i])
            {
                if (!playerCanTrade)
                    ERROR_LOG("player can't store item: %s", hisItems[i]->GetGuidStr().c_str());
                if (trader->CanStoreItem( NULL_BAG, NULL_SLOT, traderDst, hisItems[i], false ) == EQUIP_ERR_OK)
                    trader->MoveItemToInventory(traderDst, hisItems[i], true, true);
                else
                    ERROR_LOG("trader can't take item back: %s", hisItems[i]->GetGuidStr().c_str());
            }
        }
    }
}
...
void WorldSession::HandleAcceptTradeOpcode(WorldPacket& recvPacket)
{
    ...
    // Ranger: crashfix
    if (!_player || !_player->IsInWorld() || !_player->GetMap())
        return;
    ...
    // Ranger: crashfix
    if (!trader || !trader->IsInWorld() || !trader->GetMap())
        return;
    ...
    if (his_trade->IsAccepted())
    {
        ...
        {
            if (_player->GetSession()->GetSecurity() > SEC_CURATOR && my_trade->GetMoney() > 0)
            {
            ...
            if (trader->GetSession()->GetSecurity() > SEC_CURATOR && his_trade->GetMoney() > 0)
            {
