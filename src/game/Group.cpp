...
Group::~Group()
{
    if (m_bgGroup)
    {
        ...
        if (m_bgGroup->GetBgRaid(ALLIANCE) == this)
            ...
            ERROR_LOG("Group::~Group: battleground group is not linked to the correct battleground.");
    }
    if (m_bfGroup)
    {
        DEBUG_LOG("Group::~Group: battlefield group being deleted.");
        if (!m_bfGroup->OnGroupDeleted(this))
            ERROR_LOG("Group::~Group: battlefield group is not linked to the correct battleground.");
    }
    Rolls::iterator itr;
    while(!RollId.empty())
    {
...
bool Group::LoadGroupFromDB(Field* fields)
{
    ...
    if (m_groupType & GROUPTYPE_LFG)
        sLFGMgr._LoadFromDB(fields, GetObjectGuid());
...
bool Group::LoadMemberFromDB(uint32 guidLow, uint8 subgroup, GroupFlagMask flags, uint8 roles)
{
    ...
    sLFGMgr.SetupGroupMember(member.guid, GetObjectGuid());
...
bool Group::AddInvite(Player *player)
{
    ...
    sLFGMgr.OnInviteMember(this, player->GetObjectGuid());
...
bool Group::AddMember(ObjectGuid guid, const char* name)
{
    ...
    {
        sLFGMgr.OnAddMember(this, player->GetObjectGuid());
            ...
            if (player->getLevel() >= LEVELREQUIREMENT_HEROIC)
            {
                ...
                if(sWorld.getConfig(CONFIG_BOOL_ALLOW_TWO_SIDE_INTERACTION_GROUP)) //only for ES & Silvermoon
                {
                    ...
                    if(Player *leader = sObjectMgr.GetPlayer(group->GetLeaderGuid()))
                    {
                        ...
                        DEBUG_LOG( "WORLD: Group Interfaction Interactions - Faction changed (AddMember)" );
                    }
                }  
            }
        }
...
uint32 Group::RemoveMember(ObjectGuid guid, RemoveMethod method, ObjectGuid kicker /*= 0*/, const char* reason /*= NULL*/)
{
    sLFGMgr.OnRemoveMember(this, guid, method, kicker, reason);

    // remove member and change leader (if need) only if strong more 2 members _before_ member remove
    if (GetMembersCount() > uint32((isBattleGroup() || isLFGGroup()) ? 1 : 2))    // in BG group case allow 1 members group
    {
        ...
        if (Player *player = sObjectMgr.GetPlayer( guid ))
        {
            ...
            if(sWorld.getConfig(CONFIG_BOOL_ALLOW_TWO_SIDE_INTERACTION_GROUP))
            {
                ...
                DEBUG_LOG( "WORLD: Group Interfaction Interactions - Restore original faction (RemoveMember)" );
            }
        ...
        if (isLFGGroup() && GetMembersCount() == 1)
        {
            ...
            uint32 mapId = sLFGMgr.GetDungeonMapId(GetObjectGuid());
...
void Group::ChangeLeader(ObjectGuid guid)
{
    ...
    sLFGMgr.OnChangeLeader(this, m_leaderGuid, guid);
...
void Group::SendLootRollWon(ObjectGuid const& targetGuid, uint32 rollNumber, RollVote rollType, const Roll& r)
{
    ...
    data << r.lootedTargetGUID;                             // creature guid what we're looting
    data << uint32(0);                                      // item slot in loot ?
    data << uint32(r.itemid);                               // the itemEntryId for the item that shall be rolled for
    data << uint32(r.itemRandomSuffix);                     // randomSuffix
...
void Group::GroupLoot(WorldObject* object, Loot *loot)
{
    ...
    for(uint8 itemSlot = 0; itemSlot < loot->items.size(); ++itemSlot)
    {
        ...
        if (itemProto->Quality >= uint32(m_lootThreshold) && !lootItem.freeforall)
            StartLootRoll(object, GROUP_LOOT, loot, itemSlot, maxEnchantingSkill);
        else
...
void Group::NeedBeforeGreed(WorldObject* object, Loot *loot)
{
    ...
    for(uint8 itemSlot = 0; itemSlot < loot->items.size(); ++itemSlot)
    {
        ...
        if (itemProto->Quality >= uint32(m_lootThreshold) && !lootItem.freeforall)
            StartLootRoll(object, NEED_BEFORE_GREED, loot, itemSlot, maxEnchantingSkill);
        else
            lootItem.is_underthreshold = 1;
    }
}
    ...
    if (roll->totalNeed > 0)
    {
        if(!roll->playerVote.empty())
        {
            ...
            if(player && player->GetSession())
            {
                ...
                if ( msg == EQUIP_ERR_OK )
                {
                    AllowedLooterSet* looters = item->IsEligibleForSoublountTrade(player) ? roll->getLoot()->GetAllowedLooters() : NULL;
                    item->is_looted = true;
...
void Group::SendUpdate()
{
    for(member_citerator citr = m_memberSlots.begin(); citr != m_memberSlots.end(); ++citr)
    {
        ...
        if (isLFGGroup())
        {
            ...
            data << uint8(sLFGMgr.GetState(GetObjectGuid()) == LFG_STATE_FINISHED_DUNGEON ? 2 : 0); // FIXME - Dungeon save status? 2 = done
            data << uint32(sLFGMgr.GetDungeon(GetObjectGuid()));
...
GroupJoinBattlegroundResult Group::CanJoinBattleGroundQueue(BattleGround const* bgOrTemplate, BattleGroundQueueTypeId bgQueueTypeId, uint32 MinPlayerCount, uint32 MaxPlayerCount, bool isRated, uint32 arenaSlot)
{
    ...
    for(GroupReference *itr = GetFirstMember(); itr != NULL; itr = itr->next())
    {
        ...
        // can't join from dk starting zone
        if (!member->DkQuestsCompleted())
            return ERR_BATTLEGROUND_JOIN_FAILED;
        // can't use bg while in lfg
        if (member->isUsingLfg())
            return ERR_LFG_CANT_USE_BATTLEGROUND;
        // can't join bg if members are spectating
        if (member->IsSpectator())
            return ERR_BATTLEGROUND_NOT_IN_BATTLEGROUND;

        if (member->IsTaxiFlying())
        {
            member->GetSession()->SendNotification(LANG_YOU_IN_FLIGHT);
            return ERR_BATTLEGROUND_JOIN_FAILED;
        }
    }
...
void Group::ConvertToLFG()
{
    ...
    if (!isBattleGroup())
        CharacterDatabase.PExecute("UPDATE groups SET groupType='%u' WHERE guid='%u'", uint32(m_groupType), m_Id);
