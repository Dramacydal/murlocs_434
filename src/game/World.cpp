...
void utf8print_4Megai2(void* arg, const char* str)
{
...
void commandFinished_4Megai2(void*, bool sucess)
{
    printf("mangos>");
    fflush(stdout);
}
...
World::World()
{
    ...
    m_NextGetConsoleCommands = m_gameTime;
    m_NextTicketsUpdate = m_gameTime;
    m_NextGiftUpdate = m_gameTime;
    ...
    m_mapManagerUpdateTime = 0;
    m_mapManagerMaxUpdateTime = 0;
}
...
World::AddSession_ (WorldSession* s)
{
    ...
    if (pLimit > 0 && Sessions >= pLimit && s->GetSecurity () <= SEC_CURATOR )
    {
        AddQueuedSession(s);
...
void World::LoadConfigSettings(bool reload)
{
    if (reload)
    {
        if (!sConfig.Reload())
        {
            ERROR_LOG("World settings reload fail: can't read settings from %s.",sConfig.GetFilename().c_str());
            return;
        }
    }
    ...
    {
        ERROR_LOG("*****************************************************************************");
        ERROR_LOG(" WARNING: mangosd.conf does not include a ConfVersion variable.");
        ERROR_LOG("          Your configuration file may be out of date!");
        ERROR_LOG("*****************************************************************************");
    ...
    else
    {
        ...
        {
            ERROR_LOG("*****************************************************************************");
            ERROR_LOG(" WARNING: Your mangosd.conf version indicates your conf file is out of date!");
            ERROR_LOG("          Please check for updates, as your current default values may cause");
            ERROR_LOG("          unexpected behavior.");
            ERROR_LOG("*****************************************************************************");
            Log::WaitBeforeContinueIfNeed();
        }
    }
    ...
    setConfig(CONFIG_UINT32_CHNCNT, "Channel.Count", 1);
    setConfig(CONFIG_UINT32_CHNID, "Channel.Id", 1);
    setConfig(CONFIG_UINT32_RATE_BUY_PRICE, "Rate.Buy.Price", 1);
    // must be set after CONFIG_UINT32_RATE_BUY_PRICE
    setConfig(CONFIG_UINT32_RATE_SELL_PRICE, "Rate.Sell.Price", getConfig(CONFIG_UINT32_RATE_BUY_PRICE));
    setConfig(CONFIG_UINT32_RATE_REPAIR_PRICE, "Rate.Repair.Price", 1);
    ...
    setConfig(CONFIG_FLOAT_RATE_GUILD_REPUTATION_GAIN,           "Rate.Guild.Reputation.Gain", 1.0f);
    ...
    // Amaru: anticheat
    m_AnticheatBuggedValueSpd               = (uint32)sConfig.GetIntDefault("Anticheat.BuggedValue.SpellPower", 0);
    m_AnticheatBuggedValueAP                = (uint32)sConfig.GetIntDefault("Anticheat.BuggedValue.AttackPower", 0);
    m_AnticheatBuggedValueHealth            = (uint32)sConfig.GetIntDefault("Anticheat.BuggedValue.Health", 0);
    m_AnticheatBuggedValueMana              = (uint32)sConfig.GetIntDefault("Anticheat.BuggedValue.Mana", 0);
    ...
    setConfig(CONFIG_BOOL_ANTICHEAT_WARDEN,              "Anticheat.Warden", true);
    ...
    setConfig(CONFIG_BOOL_CHARACTER_ADDON_ENABLED, "CharacterAddonEnabled", false);
    ...
    {
        ERROR_LOG("MaxOverspeedPings (%i) must be in range 2..infinity (or 0 to disable check). Set to 2.", getConfig(CONFIG_UINT32_MAX_OVERSPEED_PINGS));
    ...
    setConfig(CONFIG_UINT32_CUSTOM_NPC_TELEPORT_GOLD_COST, "Custom.NPC.Teleport.Cost", 500);
    ...
    if(int clientCacheId = sConfig.GetIntDefault("ClientCacheVersion", 0))
    {
        ...
        if(clientCacheId > 0)
        {
            ...
            ERROR_LOG("ClientCacheVersion can't be negative %d, ignored.", clientCacheId);
    }
    ...
    {
        ERROR_LOG("Visibility.Distance.Grey.Unit can't be greater %f",MAX_VISIBILITY_DISTANCE);
    ...
    {
        ERROR_LOG("Visibility.Distance.Grey.Object can't be greater %f",MAX_VISIBILITY_DISTANCE);
    ...
    {
        ERROR_LOG("Visibility.Distance.Continents can't be less max aggro radius %f", 45*getConfig(CONFIG_FLOAT_RATE_CREATURE_AGGRO));
    ...
    {
        ERROR_LOG("Visibility.Distance.Continents can't be greater %f",MAX_VISIBILITY_DISTANCE - m_VisibleUnitGreyDistance);
    ...
    {
        ERROR_LOG("Visibility.Distance.Instances can't be less max aggro radius %f",45*getConfig(CONFIG_FLOAT_RATE_CREATURE_AGGRO));
    ...
    {
        ERROR_LOG("Visibility.Distance.Instances can't be greater %f",MAX_VISIBILITY_DISTANCE - m_VisibleUnitGreyDistance);
    ...
    {
        ERROR_LOG("Visibility.Distance.BGArenas can't be less max aggro radius %f",45*getConfig(CONFIG_FLOAT_RATE_CREATURE_AGGRO));
    ...
    {
        ERROR_LOG("Visibility.Distance.BGArenas can't be greater %f",MAX_VISIBILITY_DISTANCE - m_VisibleUnitGreyDistance);
    ...
    {
        ERROR_LOG("Visibility.Distance.InFlight can't be greater %f",MAX_VISIBILITY_DISTANCE-m_VisibleObjectGreyDistance);
    ...
    if (reload)
    {
        if (dataPath != m_dataPath)
            ERROR_LOG("DataDir option can't be changed at mangosd.conf reload, using current value (%s).", m_dataPath.c_str());
    }
    else
    {
        m_dataPath = dataPath;
    ...
    ignoreMapIds = sConfig.GetStringDefault("mmap.ignoreMapIds", "");
    MMAP::MMapFactory::preventPathfindingOnMaps(ignoreMapIds.c_str());
    sLog.outString("WORLD: mmap pathfinding %sabled", getConfig(CONFIG_BOOL_MMAP_ENABLED) ? "en" : "dis");

    ///- HappyTime1
    setConfig(CONFIG_BOOL_HAPPYTIME1_ENABLED,                       "HappyTime1.Enabled",       false);
    setConfigMinMax(CONFIG_UINT32_HAPPYTIME1_START_HOUR,            "HappyTime1.Start.Hour",    10, 0, 23);
    setConfig(CONFIG_UINT32_HAPPYTIME1_DURATION,                    "HappyTime1.Duration",      480);
    setConfigMinMax(CONFIG_UINT32_HAPPYTIME1_WEEK_DAY,              "HappyTime1.Week.Day",      5, 0, 6);

    setConfig(CONFIG_FLOAT_HAPPYTIME1_MODIFIER_DROP_ITEM,           "HappyTime1.Modifier.Drop.Item",        1.0f);
    setConfig(CONFIG_FLOAT_HAPPYTIME1_MODIFIER_DROP_MONEY,          "HappyTime1.Modifier.Drop.Money",       1.0f);
    setConfig(CONFIG_FLOAT_HAPPYTIME1_MODIFIER_XP,                  "HappyTime1.Modifier.XP",               1.0f);
    setConfig(CONFIG_FLOAT_HAPPYTIME1_MODIFIER_HONOR,               "HappyTime1.Modifier.Honor",            1.0f);
    setConfig(CONFIG_FLOAT_HAPPYTIME1_MODIFIER_REPUTATION,          "HappyTime1.Modifier.Reputation",       1.0f);

    ///- Premium Account Features
    setConfig(CONFIG_FLOAT_PREMIUM_MODIFIER_RATE_DROP_MONEY,    "Premium.Modifier.Drop.Money",  1.0f);
    setConfig(CONFIG_FLOAT_PREMIUM_MODIFIER_RATE_HONOR,         "Premium.Modifier.Rate.Honor",  1.0f);
    setConfig(CONFIG_FLOAT_PREMIUM_MODIFIER_RATE_XP,            "Premium.Modifier.Rate.XP",     1.0f);

    setConfig(CONFIG_UINT32_PREMIUM_TELEPORT_COOLDOWN_MINUTES,  "Premium.Teleport.Cooldown.Minutes",    10);
    ...
    setConfig(CONFIG_BOOL_FUN_RESTRICT_INSTANCES, "Fun.RestrictInstances", false);
    setConfig(CONFIG_BOOL_FUN_UNLIMITED_RANDOM_BG, "Fun.UnlimitedRandomBG", false);
    setConfigMinMax(CONFIG_UINT32_FUN_REROLL_COST_TYPE_CUSTOMIZE, "Fun.Type.Customize", uint8(COST_AP), 0, uint8(COST_MAX));
    setConfigMinMax(CONFIG_UINT32_FUN_REROLL_COST_TYPE_FACTION, "Fun.Type.Faction", uint8(COST_AP), 0, uint8(COST_MAX));
    setConfigMinMax(CONFIG_UINT32_FUN_REROLL_COST_TYPE_RACE, "Fun.Type.Race", uint8(COST_AP), 0, uint8(COST_MAX));
    ...
    setConfig(CONFIG_UINT32_FUN_RATE_A3, "Fun.Rate.A3", 2200);
    setConfig(CONFIG_UINT32_FUN_RATE_A4, "Fun.Rate.A4", 2300);

    setConfigMinMax(CONFIG_UINT32_FUN_BRACKET_A1, "Fun.Bracket.A1", 0, 0, 2);
    setConfigMinMax(CONFIG_UINT32_FUN_BRACKET_A2, "Fun.Bracket.A2", 0, 0, 2);
    setConfigMinMax(CONFIG_UINT32_FUN_BRACKET_A3, "Fun.Bracket.A3", 0, 0, 2);
    setConfigMinMax(CONFIG_UINT32_FUN_BRACKET_A4, "Fun.Bracket.A4", 0, 0, 2);


    ///- Bg Anti AFK system
    setConfig(CONFIG_FLOAT_MIN_ANTIAFK_DISTANCE, "Battleground.AntiAFK.Distance", 10.0f);
    ...
    setConfig(CONFIG_UINT32_ITEM_SOULBOUND_TRADE_TIME, "Item.SoulboundTradeTime", 2 * HOUR);
    setConfig(CONFIG_UINT32_ITEM_REFUND_TIME, "Item.RefundTime", 2 * HOUR);

    setConfig(CONFIG_BOOL_LOG_AUCTION, "Log.Auction", false);

    setConfig(CONFIG_UINT32_KICK_DELAY, "Kick.Delay", 5000);

    setConfig(CONFIG_BOOL_SPECTATING_ENABLED, "Spectate.Enabled", false);

    ///- GCD anticheat
    setConfig(CONFIG_BOOL_GCD_ANTICHEAT_ENABLED, "Anticheat.GCD.Enabled", true);
    ...
    if (reload)
    {
        ...
        m_timers[WUPDATE_AUTOBROADCAST].SetCurrent(0);
    }

    ///- Wintergrasp
    setConfig(CONFIG_UINT32_WINTERGRASP_COOLDOWN_DURATION, "Wintergrasp.CooldownDuraton", 150);
...
void World::SetInitialWorldSettings()
{
    ...
    sLog.outString("Loading Race/Faction change conversion data...");
    sObjectMgr.LoadRFCConversionData();

    sLog.outString("Loading Allowed Pets...");
    sObjectMgr.LoadAllowedPets();
    ...
    sLog.outString("Calculate next Event1 start time...");
    InitEvent1Start();
...
void World::DetectDBCLang()
{
    ...
    {
        ERROR_LOG("Incorrect DBC.Locale! Must be >= 0 and < %d (set to 0)",MAX_LOCALE);
    ...
    {
        ERROR_LOG("Unable to determine your DBC Locale! (corrupt DBC?)");
        Log::WaitBeforeContinueIfNeed();
        exit(1);
    }
...
{
    m_updateTime = diff;
    if (m_updateTime > m_maxUpdateTime)
        m_maxUpdateTime = m_updateTime;
    ...
    if (m_gameTime > m_NextGiftUpdate)
        GetNewGifts();

    if (m_gameTime > m_NextGetConsoleCommands)
        GetConsoleCommands();

    // Handle Event1
    if (m_gameTime > m_NextEvent1Start && getConfig(CONFIG_BOOL_HAPPYTIME1_ENABLED))
        ApplyEvent1();
    ...
    if (m_gameTime > m_NextTicketsUpdate)
        GetTicketsUpdate();
    ...
    m_mapManagerUpdateTime = WorldTimer::getMSTime();
    sMapMgr.Update(diff);
    m_mapManagerUpdateTime = WorldTimer::getMSTime() - m_mapManagerUpdateTime;

    if (m_mapManagerUpdateTime > m_mapManagerMaxUpdateTime)
        m_mapManagerMaxUpdateTime = m_mapManagerUpdateTime;
    ...
    if (m_timers[WUPDATE_UNBAN_CHAR].Passed())
    {
        m_timers[WUPDATE_UNBAN_CHAR].Reset();
        CharacterDatabase.Execute("UPDATE character_banned SET active = 0 WHERE active = 1 AND unbandate<=UNIX_TIMESTAMP() AND unbandate<>bandate");
...
void World::SendUpdateWintergraspTimerWorldState(BattleFieldWG* opvp)
{
    ...
    for (itr = m_sessions.begin(); itr != m_sessions.end(); ++itr)
    {
        if (itr->second &&
            itr->second->GetPlayer() &&
            itr->second->GetPlayer()->IsInWorld() &&
            itr->second->GetPlayer()->GetMapId() != 571)
        {
...
uint32 World::GetAvgLatency()
{
    ...
    for(SessionMap::const_iterator itr = m_sessions.begin(); itr != m_sessions.end(); ++itr)
    {
        ...
        latency += itr->second->GetLatency();
        count++;
    }

    if (count == 0) return 0;

    return uint32(latency / count);
}
...
/// Send text to all GMs
void World::SendGMGlobalText(AccountTypes sec, int32 string_id, ...)
{
    va_list ap;
    va_start(ap, string_id);
    ...
    for(SessionMap::const_iterator itr = m_sessions.begin(); itr != m_sessions.end(); ++itr)
    {
        if(!itr->second || !itr->second->GetPlayer() || !itr->second->GetPlayer()->IsInWorld() || itr->second->GetSecurity() < sec)
            continue;
...
void World::SendTeamText(Team team, int32 string_id, ...)
{
    va_list ap;
    va_start(ap, string_id);
...
void World::SendZoneMessage(uint32 zone, WorldPacket* packet, WorldSession* self, Team team)
{
    SessionMap::const_iterator itr;
    for (itr = m_sessions.begin(); itr != m_sessions.end(); ++itr)
    {
...
BanReturn World::BanAccount(BanMode mode, std::string nameOrIP, uint32 duration_secs, std::string reason, std::string author)
{
    ...
    QueryResult* result = NULL;                     //used for kicking
    ...
    switch(mode)
    {
        case BAN_IP:
            ...
            result = LoginDatabase.PQuery("SELECT id FROM account WHERE last_ip = '%s'", nameOrIP.c_str());
            LoginDatabase.PExecute("INSERT INTO ip_banned VALUES ('%s',UNIX_TIMESTAMP(),UNIX_TIMESTAMP()+%u,'%s','%s')", nameOrIP.c_str(), duration_secs, safe_author.c_str(), reason.c_str());
            break;
        case BAN_ACCOUNT:
            //No SQL injection as string is escaped
            result = LoginDatabase.PQuery("SELECT id FROM account WHERE username = '%s'", nameOrIP.c_str());
            break;
        case BAN_CHARACTER:
        case BAN_CHARACTER_ACC:
            //No SQL injection as string is escaped
            result = CharacterDatabase.PQuery("SELECT account, guid FROM characters WHERE name = '%s'", nameOrIP.c_str());
            break;
        default:
    ...
    do
    {
        ...
        if (mode == BAN_ACCOUNT || mode == BAN_CHARACTER_ACC)
        {
            // No SQL injection as strings are escaped
        ...
        {
            CharacterDatabase.PExecute("INSERT INTO character_banned VALUES ('%u', UNIX_TIMESTAMP(), UNIX_TIMESTAMP() + %u, '%s', '%s', '1')",
                fields[1].GetUInt32(), duration_secs, safe_author.c_str(), reason.c_str());
        }

        if (WorldSession* sess = FindSession(account))
            if(std::string(sess->GetPlayerName()) != author)
                if (sess->GetPlayer())
                    sess->GetPlayer()->SetKickDelay(sWorld.getConfig(CONFIG_UINT32_KICK_DELAY));
                else
                    sess->KickPlayer();
    }
...
bool World::RemoveBanAccount(BanMode mode, std::string nameOrIP)
{
    ...
    else
    {
        uint32 accOrGuid = 0;
        if (mode == BAN_ACCOUNT)
            accOrGuid = sAccountMgr.GetId(nameOrIP);
        else if (mode == BAN_CHARACTER)
            accOrGuid = sObjectMgr.GetPlayerGuidByName(nameOrIP).GetCounter();
        else if (mode == BAN_CHARACTER_ACC)
            accOrGuid = sObjectMgr.GetPlayerAccountIdByPlayerName(nameOrIP);
        ...
        if (mode == BAN_CHARACTER)
            CharacterDatabase.PExecute("UPDATE character_banned SET active = '0' WHERE guid = '%u'", accOrGuid);
        else
            LoginDatabase.PExecute("UPDATE account_banned SET active = '0' WHERE id = '%u'", accOrGuid);
    }
    return true;
}
...
void World::ShutdownMsg(bool show /*= false*/, Player* player /*= NULL*/)
{
        ...
        (m_ShutdownTimer >= 12 * HOUR && (m_ShutdownTimer % (12 * HOUR)) == 0))     // >= 12 h; every 12 h
    {
        ...
        sLog.outString("Server is %s in %s",(m_ShutdownMask & SHUTDOWN_MASK_RESTART ? "restart" : "shutting down"),str.c_str());
    }
}
...
void World::GetNewGifts()
{
    m_NextGiftUpdate = time_t(m_NextGiftUpdate + 10*MINUTE);
    //megai2: added mail text, subject and gold from db. if you want add multiple items, and not forgot to modify sentFlag
    if (QueryResult * result = CharacterDatabase.PQuery("SELECT `qIdx`, `charGuid`, `itemId`, `itemStackCnt`, `text`, `subj`, `gold` FROM `queuedGifts` WHERE `sentFlag` = 0"))
    {
        do
        {
            Field *fields = result->Fetch();
            uint32 qIdx = fields[0].GetUInt32();         //индекс очереди     KEY
            ObjectGuid charGuid = ObjectGuid(HIGHGUID_PLAYER, fields[1].GetUInt32());     //гуид чара
            uint32 itemId = fields[2].GetUInt32();       //айди вещи с вовхеда
            uint16 itemStackCnt = fields[3].GetUInt32(); //
            std::string text = fields[4].GetCppString();
            std::string subj = fields[5].GetCppString();
            ...
            if (subj.empty())
                subj = "You received a gift!";

            if (itemId)
            {
                Item* item = Item::CreateItem(itemId, itemStackCnt);
                if (!item)
                {
                ...
                // fill mail
                MailDraft(subj.c_str(), text.c_str())                              // item list preparing
                    .AddItem(item)
                    .SetMoney(money)
                    .SendMailTo(MailReceiver(pl, charGuid), MailSender(MAIL_NORMAL, charGuid.GetCounter(), MAIL_STATIONERY_GM));
                CharacterDatabase.PExecute("UPDATE `queuedGifts` SET `sentFlag` = 1, `itemGuid` = %u WHERE `qIdx` = %u", item->GetGUIDLow(), qIdx);
            }
            else
            {
                // fill mail
                MailDraft(subj.c_str(), text.c_str())
                    .SetMoney(money)
                    .SendMailTo(MailReceiver(pl, charGuid), MailSender(MAIL_NORMAL, charGuid.GetCounter(), MAIL_STATIONERY_GM));
                CharacterDatabase.PExecute("UPDATE `queuedGifts` SET `sentFlag` = 1, `itemGuid` = %u WHERE `qIdx` = %u", money, qIdx);
            }
        }
        while (result->NextRow());
...
void World::GetTicketsUpdate()
{
    m_NextTicketsUpdate = time_t(m_NextTicketsUpdate + MINUTE);
    sTicketMgr.UpdateFromDB();
}

void World::GetConsoleCommands()
{
    m_NextGetConsoleCommands = time_t(m_NextGetConsoleCommands + 10*MINUTE);
#ifdef SQL_STAT_COLLECT 
    uint32 c_online = GetActiveAndQueuedSessionCount();
    CharacterDatabase.DumpInfo(c_online, realmID, 0, &LoginDatabase);
    WorldDatabase.DumpInfo(c_online, realmID, 1, &LoginDatabase);
    LoginDatabase.DumpInfo(c_online, realmID, 2, &LoginDatabase);
#endif
    QueryResult * result = CharacterDatabase.PQuery("SELECT command, id FROM `console` WHERE done IS NULL");
    if (result)
    {
        do
        {
            ...
            sWorld.QueueCliCommand(new CliCommandHolder(0, SEC_CONSOLE, NULL, str.c_str(), &utf8print_4Megai2, &commandFinished_4Megai2));
            CharacterDatabase.PExecute("UPDATE console SET done = NOW() where id=%u", cmdId);
        } while (result->NextRow());
...
void World::setConfigPos(eConfigFloatValues index, char const* fieldname, float defvalue)
{
    ...
    {
        ERROR_LOG("%s (%f) can't be negative. Using %f instead.", fieldname, getConfig(index), defvalue);
        setConfig(index, defvalue);
    }
}
...
void World::setConfigMin(eConfigUInt32Values index, char const* fieldname, uint32 defvalue, uint32 minvalue)
{
    ...
    {
        ERROR_LOG("%s (%u) must be >= %u. Using %u instead.", fieldname, getConfig(index), minvalue, minvalue);
...
void World::setConfigMin(eConfigInt32Values index, char const* fieldname, int32 defvalue, int32 minvalue)
{
    ...
    if (getConfig(index) < minvalue)
    {
        ERROR_LOG("%s (%i) must be >= %i. Using %i instead.", fieldname, getConfig(index), minvalue, minvalue);
        setConfig(index, minvalue);
    }
}
...
void World::setConfigMin(eConfigFloatValues index, char const* fieldname, float defvalue, float minvalue)
{
    ...
    if (getConfig(index) < minvalue)
    {
        ERROR_LOG("%s (%f) must be >= %f. Using %f instead.", fieldname, getConfig(index), minvalue, minvalue);
        setConfig(index, minvalue);
    }
}
...
void World::setConfigMinMax(eConfigUInt32Values index, char const* fieldname, uint32 defvalue, uint32 minvalue, uint32 maxvalue)
{
    ...
    if (getConfig(index) < minvalue)
    {
        ERROR_LOG("%s (%u) must be in range %u...%u. Using %u instead.", fieldname, getConfig(index), minvalue, maxvalue, minvalue);
        setConfig(index, minvalue);
    }
    else if (getConfig(index) > maxvalue)
    {
        ERROR_LOG("%s (%u) must be in range %u...%u. Using %u instead.", fieldname, getConfig(index), minvalue, maxvalue, maxvalue);
...
void World::setConfigMinMax(eConfigInt32Values index, char const* fieldname, int32 defvalue, int32 minvalue, int32 maxvalue)
{
    ...
    if (getConfig(index) < minvalue)
    {
        ERROR_LOG("%s (%i) must be in range %i...%i. Using %i instead.", fieldname, getConfig(index), minvalue, maxvalue, minvalue);
        setConfig(index, minvalue);
    }
    else if (getConfig(index) > maxvalue)
    {
        ERROR_LOG("%s (%i) must be in range %i...%i. Using %i instead.", fieldname, getConfig(index), minvalue, maxvalue, maxvalue);
        setConfig(index, maxvalue);
    }
}
...
void World::setConfigMinMax(eConfigFloatValues index, char const* fieldname, float defvalue, float minvalue, float maxvalue)
{
    ...
    if (getConfig(index) < minvalue)
    {
        ERROR_LOG("%s (%f) must be in range %f...%f. Using %f instead.", fieldname, getConfig(index), minvalue, maxvalue, minvalue);
        setConfig(index, minvalue);
    }
    else if (getConfig(index) > maxvalue)
    {
        ERROR_LOG("%s (%f) must be in range %f...%f. Using %f instead.", fieldname, getConfig(index), minvalue, maxvalue, maxvalue);
        setConfig(index, maxvalue);
    }
}
...
bool World::configNoReload(bool reload, eConfigUInt32Values index, char const* fieldname, uint32 defvalue)
{
    ...
    if (val != getConfig(index))
        ERROR_LOG("%s option can't be changed at mangosd.conf reload, using current value (%u).", fieldname, getConfig(index));
...
bool World::configNoReload(bool reload, eConfigInt32Values index, char const* fieldname, int32 defvalue)
{
    ...
    if (val != getConfig(index))
        ERROR_LOG("%s option can't be changed at mangosd.conf reload, using current value (%i).", fieldname, getConfig(index));
...
bool World::configNoReload(bool reload, eConfigFloatValues index, char const* fieldname, float defvalue)
{
    ...
    if (val != getConfig(index))
        ERROR_LOG("%s option can't be changed at mangosd.conf reload, using current value (%f).", fieldname, getConfig(index));
...
bool World::configNoReload(bool reload, eConfigBoolValues index, char const* fieldname, bool defvalue)
{
    ...
    if (val != getConfig(index))
        ERROR_LOG("%s option can't be changed at mangosd.conf reload, using current value (%s).", fieldname, getConfig(index) ? "'true'" : "'false'");

    return false;
}

void World::ApplyEvent1()
{
    DETAIL_LOG("Starting Event1.");

    m_NextEvent1Start = time_t(m_NextEvent1Start + WEEK); 
    CharacterDatabase.PExecute("UPDATE saved_variables SET NextEvent1Start = '"UI64FMTD"'", uint64(m_NextRandomBGReset));

    setConfig(CONFIG_FLOAT_RATE_DROP_ITEM_POOR,             getConfig(CONFIG_FLOAT_RATE_DROP_ITEM_POOR)             * getConfig(CONFIG_FLOAT_HAPPYTIME1_MODIFIER_DROP_ITEM));
    setConfig(CONFIG_FLOAT_RATE_DROP_ITEM_NORMAL,           getConfig(CONFIG_FLOAT_RATE_DROP_ITEM_NORMAL)           * getConfig(CONFIG_FLOAT_HAPPYTIME1_MODIFIER_DROP_ITEM));
    setConfig(CONFIG_FLOAT_RATE_DROP_ITEM_UNCOMMON,         getConfig(CONFIG_FLOAT_RATE_DROP_ITEM_UNCOMMON)         * getConfig(CONFIG_FLOAT_HAPPYTIME1_MODIFIER_DROP_ITEM));
    setConfig(CONFIG_FLOAT_RATE_DROP_ITEM_RARE,             getConfig(CONFIG_FLOAT_RATE_DROP_ITEM_RARE)             * getConfig(CONFIG_FLOAT_HAPPYTIME1_MODIFIER_DROP_ITEM));
    setConfig(CONFIG_FLOAT_RATE_DROP_ITEM_EPIC,             getConfig(CONFIG_FLOAT_RATE_DROP_ITEM_EPIC)             * getConfig(CONFIG_FLOAT_HAPPYTIME1_MODIFIER_DROP_ITEM));
    setConfig(CONFIG_FLOAT_RATE_DROP_ITEM_LEGENDARY,        getConfig(CONFIG_FLOAT_RATE_DROP_ITEM_LEGENDARY)        * getConfig(CONFIG_FLOAT_HAPPYTIME1_MODIFIER_DROP_ITEM));
    setConfig(CONFIG_FLOAT_RATE_DROP_ITEM_ARTIFACT,         getConfig(CONFIG_FLOAT_RATE_DROP_ITEM_ARTIFACT)         * getConfig(CONFIG_FLOAT_HAPPYTIME1_MODIFIER_DROP_ITEM));
    setConfig(CONFIG_FLOAT_RATE_DROP_ITEM_REFERENCED,       getConfig(CONFIG_FLOAT_RATE_DROP_ITEM_REFERENCED)       * getConfig(CONFIG_FLOAT_HAPPYTIME1_MODIFIER_DROP_ITEM));
    setConfig(CONFIG_FLOAT_RATE_DROP_MONEY,                 getConfig(CONFIG_FLOAT_RATE_DROP_MONEY)                 * getConfig(CONFIG_FLOAT_HAPPYTIME1_MODIFIER_DROP_MONEY));

    setConfig(CONFIG_FLOAT_RATE_XP_KILL,                    getConfig(CONFIG_FLOAT_RATE_XP_KILL)                    * getConfig(CONFIG_FLOAT_HAPPYTIME1_MODIFIER_XP));
    setConfig(CONFIG_FLOAT_RATE_XP_QUEST,                   getConfig(CONFIG_FLOAT_RATE_XP_QUEST)                   * getConfig(CONFIG_FLOAT_HAPPYTIME1_MODIFIER_XP));
    setConfig(CONFIG_FLOAT_RATE_XP_EXPLORE,                 getConfig(CONFIG_FLOAT_RATE_XP_EXPLORE)                 * getConfig(CONFIG_FLOAT_HAPPYTIME1_MODIFIER_XP));
    setConfig(CONFIG_FLOAT_RATE_HONOR,                      getConfig(CONFIG_FLOAT_RATE_HONOR)                      * getConfig(CONFIG_FLOAT_HAPPYTIME1_MODIFIER_HONOR));

    setConfig(CONFIG_FLOAT_RATE_REPUTATION_GAIN,            getConfig(CONFIG_FLOAT_RATE_REPUTATION_GAIN)            * getConfig(CONFIG_FLOAT_HAPPYTIME1_MODIFIER_REPUTATION));
    setConfig(CONFIG_FLOAT_RATE_REPUTATION_LOWLEVEL_KILL,   getConfig(CONFIG_FLOAT_RATE_REPUTATION_LOWLEVEL_KILL)   * getConfig(CONFIG_FLOAT_HAPPYTIME1_MODIFIER_REPUTATION));
    setConfig(CONFIG_FLOAT_RATE_REPUTATION_LOWLEVEL_QUEST,  getConfig(CONFIG_FLOAT_RATE_REPUTATION_LOWLEVEL_QUEST)  * getConfig(CONFIG_FLOAT_HAPPYTIME1_MODIFIER_REPUTATION));
}

void World::RemoveEvent1()
{
    DETAIL_LOG("Removing Event1.");

    m_NextEvent1End = time_t(m_NextEvent1End + WEEK);
...
void World::InitEvent1Start()
{
    QueryResult* result = CharacterDatabase.Query("SELECT NextEvent1Start FROM saved_variables");
    if (!result)
        m_NextEvent1Start = time_t(time(NULL));        // game time not yet init
    else
        m_NextEvent1Start = time_t((*result)[0].GetUInt64());

    // generate time by config
    time_t curTime = time(NULL);
    tm localTm = *localtime(&curTime);

    int week_day_offset = localTm.tm_wday - int(getConfig(CONFIG_UINT32_HAPPYTIME1_WEEK_DAY));

    // current week reset time
    localTm.tm_hour = getConfig(CONFIG_UINT32_HAPPYTIME1_START_HOUR);
    localTm.tm_min  = 0;
    localTm.tm_sec  = 0;
    time_t nextStartTime = mktime(&localTm);
    nextStartTime -= week_day_offset * DAY;             // move time to proper day
    ...
    // normalize reset time
    m_NextEvent1Start = m_NextEvent1Start < curTime ? nextStartTime - WEEK : nextStartTime;
    m_NextEvent1End = time_t(m_NextEvent1Start + getConfig(CONFIG_UINT32_HAPPYTIME1_DURATION) * MINUTE);
    if (!result)
        CharacterDatabase.PExecute("INSERT INTO saved_variables (NextEvent1Start) VALUES ('"UI64FMTD"')", uint64(m_NextEvent1Start));
    else
        delete result;
}

void World::SendGMGlobalText(const char* str, AccountTypes sec)
{
    // Chat output
    WorldPacket data;
    ...
    {
        ChatHandler::FillMessageData(&data, NULL, CHAT_MSG_SYSTEM, LANG_UNIVERSAL, line);
        SendGMGlobalMessage(&data, sec);
    }

    delete[] buf;
}

void World::ApplyEvent102Morphs(bool apply)
{
    SessionMap::const_iterator itr;
    for (itr = m_sessions.begin(); itr != m_sessions.end(); ++itr)
    {
            ...
            itr->second->GetPlayer()->IsInWorld())
        {
            ...
            if (apply)
            {
                uint8 gender = plr->getGender();
                if (plr->GetTeam() == ALLIANCE)
                {
                    plr->SetNativeDisplayId(gender == GENDER_MALE ? 1098 : 1947);
                    plr->SetDisplayId(gender == GENDER_MALE ? 1098 : 1947);
                }
                else
                {
                    plr->SetNativeDisplayId(gender == GENDER_MALE ? 7061 : 16171);
                    plr->SetDisplayId(gender == GENDER_MALE ? 7061 : 16171);
                }
            }
            else
                plr->InitDisplayIds();
...
void World::ApplyEvent103Morphs(bool apply)
{
    SessionMap::const_iterator itr;
    for (itr = m_sessions.begin(); itr != m_sessions.end(); ++itr)
    {
...
void World::LoadAutoBroadcasts()
{
    ...
    QueryResult* result = WorldDatabase.Query("SELECT `text` FROM `autobroadcast` ORDER BY `id`");
    if (!result)
    {
        BarGoLink bar(1);
        ...
        sLog.outString();
        sLog.outString(">> Loaded %u autobroadcasts.", count);
        return;
    }
    ...
    do
    {
        ...
        m_autoBroadcasts[count] = fields[0].GetCppString();
...
void World::SendAutoBroadcast()
{
    ...
    if (m_broadcastNext >= m_autoBroadcasts.size())
        m_broadcastNext = 0;

    std::string msg = m_autoBroadcasts[m_broadcastNext];
...
bool World::GetWorldState(uint32 index, uint32& val) const
{
    WorldStatesMap::const_iterator itr = m_worldstates.find(index);
...
FunRerollCostType FunSettings::GetRerollCostType(RerollType type) const
{
    if (sWorld.getConfig(CONFIG_BOOL_FUN_REROLL_ENABLED))
    {
        switch (type)
        {
            case REROLL_CUSTOMIZE:
                return FunRerollCostType(sWorld.getConfig(CONFIG_UINT32_FUN_REROLL_COST_TYPE_CUSTOMIZE));
            case REROLL_RACE:
                return FunRerollCostType(sWorld.getConfig(CONFIG_UINT32_FUN_REROLL_COST_TYPE_RACE));
            case REROLL_FACTION:
                return FunRerollCostType(sWorld.getConfig(CONFIG_UINT32_FUN_REROLL_COST_TYPE_FACTION));
            default:
                return COST_NONE;
        }
    }
...
uint32 FunSettings::GetRerollCost(RerollType type) const
{
    switch (type)
    {
        case REROLL_CUSTOMIZE:
            return sWorld.getConfig(CONFIG_UINT32_FUN_REROLL_COST_CUSTOMIZE);
        case REROLL_RACE:
            return sWorld.getConfig(CONFIG_UINT32_FUN_REROLL_COST_RACE);
        case REROLL_FACTION:
            return sWorld.getConfig(CONFIG_UINT32_FUN_REROLL_COST_FACTION);
        default:
            return 0;
    }
}
