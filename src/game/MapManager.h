...
typedef std::list<TransportCreatureInfo> TransportCreatureList;
typedef UNORDERED_MAP<uint32, TransportCreatureList> TransportCreatureMap;
...
class MANGOS_DLL_DECL MapManager : public MaNGOS::Singleton<MapManager, MaNGOS::ClassLevelLockable<MapManager, ACE_Recursive_Thread_Mutex> >
{
    ...
    public:
        ...
        std::map<uint32, uint32> m_LastMapUpdateTimes;
        std::map<uint32, uint32> m_MaxMapUpdateTimes;
        ...
        TransportCreatureMap m_TransportCreatures;
        ...
        uint32 GetTotalGridsSize()
        {
            ...
            for(MapMapType::const_iterator itr = i_maps.begin(); itr != i_maps.end(); ++itr)
                ret += const_cast<TerrainInfo*>(itr->second->GetTerrain())->GetTotalGridsSize();
