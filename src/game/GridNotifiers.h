...
namespace MaNGOS
{
    ...
    */
    ...
    // Success at unit in range, range update for next check (this can be use with CreatureLastSearcher to find nearest creature)
    class NearestCreatureEntryInObjectRangeCheck
    {
        public:
            NearestCreatureEntryInObjectRangeCheck(WorldObject const& obj,uint32 entry, float range, bool alive)
                : i_obj(obj), i_entry(entry), i_range(range), i_alive(alive) {}
            WorldObject const& GetFocusObject() const { return i_obj; }
            bool operator()(Creature* u)
            {
                if (u->GetEntry() == i_entry && i_alive && u->isAlive() && i_obj.IsWithinDistInMap(u, i_range))
                {
                    i_range = i_obj.GetDistance(u);         // use found unit range as new range limit for next check
                    return true;
                }
                else if (u->GetEntry() == i_entry && !i_alive && (!u->isAlive() || u->IsCorpse()) && i_obj.IsWithinDistInMap(u, i_range))
                {
                    i_range = i_obj.GetDistance(u);         // use found unit range as new range limit for next check
                    return true;
                }
        ...
        private:
            ...
            // prevent clone this object
            NearestCreatureEntryInObjectRangeCheck(NearestCreatureEntryInObjectRangeCheck const&);
    };
