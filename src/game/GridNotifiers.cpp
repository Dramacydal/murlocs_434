...
void VisibleNotifier::Notify()
{
    ...
    {
        for (Transport::PassengerList::const_iterator itr = transport->GetPassengers().begin(); itr != transport->GetPassengers().end(); ++itr)
        {
            if (!itr->guid.IsPlayer())
                continue;

           if (i_clientGUIDs.find(itr->guid) != i_clientGUIDs.end())
            {
                Player* passenger = transport->GetMap()->GetPlayer(itr->guid);
                if (!passenger)
                    continue;
