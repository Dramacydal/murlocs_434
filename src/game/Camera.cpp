...
void Camera::SetView(WorldObject *obj, bool update_far_sight_field /*= true*/)
{
    ...
    {
        ERROR_LOG("Camera::SetView, viewpoint is not in map with camera's owner");
        return;
    }

    if (!obj->isType(TypeMask(TYPEMASK_DYNAMICOBJECT | TYPEMASK_UNIT)))
    {
        ERROR_LOG("Camera::SetView, viewpoint type is not available for client");
        return;
    }
...
ViewPoint::~ViewPoint()
{
    if (!m_cameras.empty())
    {
        ERROR_LOG("ViewPoint destructor called, but some cameras referenced to it");
    }
}
