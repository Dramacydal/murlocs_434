...
void LoadSkillExtraItemTable()
{
    ...
    if (result)
    {
        ...
        do
        {
            ...
            {
                ERROR_LOG("Skill specialization %u has nonexistent spell id in `skill_extra_item_template`!", spellId);
                continue;
            }
            ...
            {
                ERROR_LOG("Skill specialization %u have nonexistent required specialization spell id %u in `skill_extra_item_template`!", spellId,requiredSpecialization);
                continue;
            }
            ...
            {
                ERROR_LOG("Skill specialization %u has too low additional create chance in `skill_extra_item_template`!", spellId);
                continue;
            }
            ...
            if (!additionalMaxNum)
            {
                ERROR_LOG("Skill specialization %u has 0 max number of extra items in `skill_extra_item_template`!", spellId);
                continue;
            }
