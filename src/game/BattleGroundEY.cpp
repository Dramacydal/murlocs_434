...
void BattleGroundEY::CheckSomeoneJoinedPoint()
{
    for (uint8 i = 0; i < BG_EY_NODES_MAX; ++i)
    {
        ...
        while (j < m_PlayersNearPoint[BG_EY_PLAYERS_OUT_OF_POINTS].size())
        {
            ...
            if (!plr)
            {
                DEBUG_LOG("BattleGroundEY:CheckSomeoneJoinedPoint: %s not found!", m_PlayersNearPoint[BG_EY_PLAYERS_OUT_OF_POINTS][j].GetString().c_str());
                ++j;
                continue;
            }
...
void BattleGroundEY::CheckSomeoneLeftPoint()
{
    ...
    for(uint8 i = 0; i < BG_EY_NODES_MAX; ++i)
    {
        ...
        while (j < m_PlayersNearPoint[i].size())
        {
            ...
            if (!plr)
            {
                ERROR_LOG("BattleGroundEY:CheckSomeoneLeftPoint %s not found!", m_PlayersNearPoint[i][j].GetString().c_str());
...
bool BattleGroundEY::SetupBattleGround()
{
    ...
    for (int i = 0; i < BG_EY_NODES_MAX; ++i)
    {
        ...
        if (!at)
        {
            ERROR_LOG("BattleGroundEY: Unknown trigger: %u", m_Points_Trigger[i]);
            continue;
        }
        if (!AddObject(BG_EY_OBJECT_SPEEDBUFF_FEL_REAVER + i * 3, Buff_Entries[0], at->x, at->y, at->z, 0.907571f, 0, 0, 0.438371f, 0.898794f, RESPAWN_ONE_DAY)
            ...
            ERROR_LOG("BattleGroundEY: Cannot spawn buff");
    }
...
WorldSafeLocsEntry const *BattleGroundEY::GetClosestGraveYard(Player* player)
{
    ...
    for(uint8 i = 0; i < BG_EY_NODES_MAX; ++i)
    {
        if (m_PointOwnedByTeam[i]==player->GetTeam() && m_PointState[i]==EY_POINT_UNDER_CONTROL)
        {
            ...
            if (!entry)
                ERROR_LOG("BattleGroundEY: Not found graveyard: %u",CapturingPointTypes[i].GraveYardId);
            else
            {
                distance = (entry->x - plr_x)*(entry->x - plr_x) + (entry->y - plr_y)*(entry->y - plr_y) + (entry->z - plr_z)*(entry->z - plr_z);
