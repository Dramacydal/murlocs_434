...
bool WorldSession::processChatmessageFurtherAfterSecurityChecks(std::string& msg, uint32 lang)
{
    if (lang != LANG_ADDON)
    {
        ...
        {
            ERROR_LOG("Player %s (GUID: %u) sent a chatmessage with an invalid link: %s", GetPlayer()->GetName(),
...
void WorldSession::HandleMessagechatOpcode( WorldPacket & recv_data )
{
    ...
    {
        ERROR_LOG("CHAT: Wrong message type received: %u", type);
        return;
    }
    ...
    else
    {
        ...
        else
        {
            ...
            if (sWorld.getConfig(CONFIG_BOOL_ALLOW_TWO_SIDE_INTERACTION_CHAT) || IsPremiumActive() && (lang == LANG_COMMON || lang == LANG_ORCISH))
                lang = LANG_UNIVERSAL;
            else
            {
                switch(type)
    ...
    switch(type)
    {
        ...
        case CHAT_MSG_YELL:
        {
            ...
            if(type == CHAT_MSG_SAY) {
                ...
                sChatLogger.logSayMessage(GetPlayer(), msg);
            }
            else if(type == CHAT_MSG_EMOTE) {
                GetPlayer()->TextEmote(msg);
                sChatLogger.logEmoteMessage(GetPlayer(), msg);
            }
            else if(type == CHAT_MSG_YELL) {
                GetPlayer()->Yell(msg, lang);
                sChatLogger.logYellMessage(GetPlayer(), msg);
            }
            break;
        }
        ...
        case CHAT_MSG_WHISPER:
        {
            ...
            if (!player || (tSecurity <= SEC_CURATOR && pSecurity > SEC_CURATOR && !player->isAcceptWhispers()))
            {
                SendPlayerNotFoundNotice(to);
                return;
            }

            if (!sWorld.getConfig(CONFIG_BOOL_ALLOW_TWO_SIDE_INTERACTION_CHAT) && !GetPlayer()->IsPremiumActive() && !player->IsPremiumActive() && tSecurity < SEC_MODERATOR && pSecurity < SEC_MODERATOR )
            {
            ...
            sChatLogger.logWhisperMessage(GetPlayer(), player, msg);
            break;
        }
        case CHAT_MSG_PARTY:
        ...
        case CHAT_MSG_GUILD:
        {
            ...
            if (GetPlayer()->GetGuildId())
                if (Guild *guild = sGuildMgr.GetGuildById(GetPlayer()->GetGuildId()))
                {
                    ...
                    sChatLogger.logGuildMessage(GetPlayer(), guild, msg);
                }
            break;
        }
        case CHAT_MSG_OFFICER:
        {
            ...
            if (GetPlayer()->GetGuildId())
                if (Guild *guild = sGuildMgr.GetGuildById(GetPlayer()->GetGuildId()))
                {
                    ...
                    sChatLogger.logOfficerMessage(GetPlayer(), guild, msg);
                }
            break;
        }
        case CHAT_MSG_RAID:
        ...
        default:
            ERROR_LOG("CHAT: unknown message type %u, lang: %u", type, lang);
            break;
    }
}
