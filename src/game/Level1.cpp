...
bool ChatHandler::HandleGMAnnounceCommand(char* args)
{
    ...
    // Amaru: какая-то херня с кодировкой, у кого получится - замените на русский (надо ли?)
    argstr << "|c0000FF00[Team message]|r<" << "|cffffffff|Hplayer:" << Player << "|h[" << Player << "]|h|r" << ">: " << args;

    sWorld.SendGMGlobalText(argstr.str().c_str(), SEC_CURATOR);
    return true;
}
...
bool ChatHandler::HandleGPSCommand(char* args)
{
    ...
    if (obj->GetObjectGuid().IsUnit())
    {
        Unit* unit = (Unit*)obj;
        PSendSysMessage("UnitStateMask:    %X", unit->getUnitState());
        PSendSysMessage("UnitFlagsMask:    %X", unit->GetUInt32Value(UNIT_FIELD_FLAGS));
        PSendSysMessage("UnitFlags2Mask:   %X", unit->GetUInt32Value(UNIT_FIELD_FLAGS_2));
        PSendSysMessage("UnitDynFlagsMask: %X", unit->GetUInt32Value(UNIT_DYNAMIC_FLAGS));
...
bool ChatHandler::HandleGonameCommand(char* args)
{
    ...
    else
    {
        ...
        return HandleGoHelper(_player, map, x + 0.5f, y, &z);
    }
...
bool ChatHandler::HandleGroupgoCommand(char* args)
{
    ...
    // before GM
    float x, y, z;
    m_session->GetPlayer()->GetClosePoint(x, y, z, m_session->GetPlayer()->GetObjectBoundingRadius());

    for (GroupReference* itr = grp->GetFirstMember(); itr != NULL; itr = itr->next())
    {
        Player* pl = itr->getSource();
        if (!pl || !pl->IsInWorld() || pl == m_session->GetPlayer())
            continue;
        ...
        {
            Map* plMap = pl->GetMap();
            if (plMap && plMap->Instanceable() && plMap->GetInstanceId() != gmMap->GetInstanceId())
            {
                // cannot summon from instance to instance
...
bool ChatHandler::HandleSetViewCommand(char* args)
{
    ...
    m_session->GetPlayer()->GetCamera().SetView(target);
    PSendSysMessage(LANG_SETVIEW_DONE, name.c_str());
    
    return true;
}

bool ChatHandler::HandleUnViewCommand(char* /*args*/)
{
    Player* plr = m_session->GetPlayer();
    plr->GetCamera().SetView(plr);
    PSendSysMessage(LANG_SETVIEW_RESET);
