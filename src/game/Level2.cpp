...
bool ChatHandler::HandleMuteCommand(char* args)
{
    ...
    time_t oldMuteTime = time(NULL);
    if (target)
        oldMuteTime = target->GetSession()->m_muteTime;
    else
    {
        QueryResult* result = LoginDatabase.PQuery("SELECT mutetime FROM account WHERE id = '%u'", account_id);
        if (result)
        {
            oldMuteTime = (*result)[0].GetUInt64();
            delete result;
        }
    }

    if (oldMuteTime >= time(NULL))
    {
        std::string timeStr = secsToTimeString(oldMuteTime - time(NULL));
        PSendSysMessage("%s is already muted! Current mute expires in %s", target_name.c_str(), timeStr.c_str());
    ...
    argstr << who << " has just muted " << target_name << " for " << secsToTimeString(notspeaktime * 60) << " due to " << reason << "! ";
...
bool ChatHandler::HandleNpcAddCommand(char* args)
{
    ...
    if (Transport* transport = chr->GetTransport())
    {
        posX = chr->GetTransOffsetX() * cos(transport->GetOrientation()) - chr->GetTransOffsetY() * sin(transport->GetOrientation());
        posY = chr->GetTransOffsetY() * cos(transport->GetOrientation()) + chr->GetTransOffsetX() * sin(transport->GetOrientation());
        posZ = transport->GetPositionZ() + chr->GetTransOffsetZ();
        posO = transport->GetOrientation() + chr->GetTransOffsetO();
    }
    else
    {
        posX = chr->GetPositionX();
        posY = chr->GetPositionY();
        posZ = chr->GetPositionZ();
...
bool ChatHandler::HandleNpcDeleteCommand(char* args)
{
    ...
    switch (unit->GetSubtype())
    {
        case CREATURE_SUBTYPE_GENERIC:
        {
            ...
            {
                if (Transport* trans = unit->GetTransport())
                {
                    if (Transport::Passenger* pas = trans->FindPassenger(unit->GetObjectGuid()))
                        if (pas->localGuid)
                            unit->DeleteFromDB();
                }
...
bool ChatHandler::HandleNpcMoveCommand(char* args)
{
    ...
    if (pCreature)
    {
        if (Transport* trans = pCreature->GetTransport())
        {
            if (trans == chr->GetTransport())
            {
                if (Transport::Passenger* pass = trans->FindPassenger(pCreature->GetObjectGuid()))
                    if (pass->localGuid)
                    {
                        pCreature->m_movementInfo.SetTransportData(trans->GetObjectGuid(), chr->GetTransOffsetX(), chr->GetTransOffsetY(), chr->GetTransOffsetZ(), chr->GetTransOffsetO(), 0, -1);
                        trans->UpdateCreaturePosition(pCreature);
                        pCreature->SaveToDB();
                    }
            }
            return true;
...
{
    Player* target = NULL;
    char* args1 = ExtractArg(&args);
    if (!args1)
        return false;

    char* args2 = ExtractArg(&args);
    ...
    if (!args2)
    {
        ...
        if (ObjectGuid guid  = m_session->GetPlayer()->GetSelectionGuid())
    ...
    if (m_session && m_session->GetSecurity() < SEC_ADMINISTRATOR && target->GetMap()->IsBattleGroundOrArena())
    {
        PSendSysMessage("Can't kick, %s is in battleground or arena!", target->GetName());
        SetSentErrorMessage(true);
        return false;
    }
    ...
    std::stringstream argstr;
    std::stringstream sysmsgdota;
    argstr << who << " has just kicked " << target->GetName() << " due to " << reason << "! ";
    ...
    target->SetKickDelay(sWorld.getConfig(CONFIG_UINT32_KICK_DELAY));
...
bool ChatHandler::HandlePInfoCommand(char* args)
{
    ...
    else
    {
        ...
        //                                                     0          1      2      3        4     5      6
        QueryResult *result = CharacterDatabase.PQuery("SELECT totaltime, level, money, account, race, class, gender FROM characters WHERE guid = '%u'", target_guid.GetCounter());
        if (!result)
            return false;
    ...
    if (result)
    {
        ...
        email = GetAccessLevel() >= SEC_ADMINISTRATOR ? fields[4].GetCppString() : "-";
    ...
    bool isActive = false;
    AccountPremiumType premiumType = PREMIUM_DEFAULT;
    AccountPremiumRateType premiumRateType = PREMIUM_RATE_XP;
    if (QueryResult* pQuery = LoginDatabase.PQuery("SELECT type, rateType, UNIX_TIMESTAMP(active) FROM account_premium WHERE id = '%u'", accId))
    {
        Field* fields = pQuery->Fetch();
        isPremium = true;
        premiumType = AccountPremiumType(fields[0].GetUInt8());
        premiumRateType = AccountPremiumRateType(fields[1].GetUInt8());
        isActive = time_t(fields[2].GetUInt64()) > time(NULL);
        delete pQuery;
    }

    uint32 raceStrId = LANG_ERROR;
    uint32 classStrId = LANG_ERROR;
    uint32 genderStrId = LANG_ERROR;
    switch(playerRace)
    {
        case RACE_HUMAN: raceStrId = LANG_RACE_HUMAN; break;
        case RACE_ORC: raceStrId = LANG_RACE_ORC; break;
        case RACE_DWARF: raceStrId = LANG_RACE_DWARF; break;
        case RACE_NIGHTELF: raceStrId = LANG_RACE_NIGHTELF; break;
        case RACE_UNDEAD: raceStrId = LANG_RACE_UNDEAD; break;
        case RACE_TAUREN: raceStrId = LANG_RACE_TAUREN; break;
        case RACE_GNOME: raceStrId = LANG_RACE_GNOME; break;
        case RACE_TROLL: raceStrId = LANG_RACE_TROLL; break;
        case RACE_BLOODELF: raceStrId = LANG_RACE_BLOODELF; break;
        case RACE_DRAENEI: raceStrId = LANG_RACE_DRAENEI; break;
    }
    switch(playerClass)
    {
        case CLASS_WARRIOR: classStrId = LANG_CLASS_WARRIOR; break;
        case CLASS_PALADIN: classStrId = LANG_CLASS_PALADIN; break;
        case CLASS_HUNTER: classStrId = LANG_CLASS_HUNTER; break;
        case CLASS_ROGUE: classStrId = LANG_CLASS_ROGUE; break;
        case CLASS_PRIEST: classStrId = LANG_CLASS_PRIEST; break;
        case CLASS_DEATH_KNIGHT: classStrId = LANG_CLASS_DEATH_KNIGHT; break;
        case CLASS_SHAMAN: classStrId = LANG_CLASS_SHAMAN; break;
        case CLASS_MAGE: classStrId = LANG_CLASS_MAGE; break;
        case CLASS_WARLOCK: classStrId = LANG_CLASS_WARLOCK; break;
        case CLASS_DRUID: classStrId = LANG_CLASS_DRUID; break;
    }
    switch(gender)
    {
        case GENDER_MALE: genderStrId = LANG_GENDER_MALE; break;
        case GENDER_FEMALE: genderStrId = LANG_GENDER_FEMALE; break;
    }

    std::string nameLink = playerLink(target_name);

    PSendSysMessage(LANG_PINFO_ACCOUNT, (target?"":GetMangosString(LANG_OFFLINE)), nameLink.c_str(), target_guid.GetCounter(), username.c_str(), accId, security, GetMangosString(raceStrId), GetMangosString(classStrId), GetMangosString(genderStrId), last_ip.c_str(), last_login.c_str(), latency);
    PSendSysMessage("E-mail: %s", email.c_str());

    if (isPremium)
    {
        PSendSysMessage("Account is >Premium<, Type: %s, Rate Type: %s, Active: %s",
            premiumType ? "EXTENDED" : "DEFAULT",
            premiumRateType == PREMIUM_RATE_XP ? "Experience" : (premiumRateType == PREMIUM_RATE_MONEY ? "Money" : "Honor"),
            isActive ? "TRUE" : "FALSE");
    }
...
bool ChatHandler::HandleCharacterChangeFactionCommand(char* args)
{
    ...
    if (target)
    {
        PSendSysMessage(LANG_CHANGEFACTION_PLAYER, GetNameLink(target).c_str());
        target->SetAtLoginFlag(AT_LOGIN_CHANGE_FACTION);
        target->SetAtLoginFlag(AT_LOGIN_CONVERT_TITLES);
    ...
    else
    {
        ...
        PSendSysMessage(LANG_CHANGEFACTION_PLAYER_GUID, oldNameLink.c_str(), target_guid.GetCounter());
...
bool ChatHandler::HandleCharacterChangeRaceCommand(char* args)
{
    ...
    if (target)
    {
        ...
        PSendSysMessage(LANG_CHANGERACE_PLAYER, GetNameLink(target).c_str());
    ...
    else
    {
        ...
        PSendSysMessage(LANG_CHANGERACE_PLAYER_GUID, oldNameLink.c_str(), target_guid.GetCounter());
...
// convert player titles
bool ChatHandler::HandleCharacterConvertTitlesCommand(char* args)
{
    Player* target;
    ObjectGuid target_guid;
    std::string target_name;
    ...
    if (target)
    {
        PSendSysMessage(LANG_CONVERTTITLES_PLAYER, GetNameLink(target).c_str());
        target->SetAtLoginFlag(AT_LOGIN_CONVERT_TITLES);
        CharacterDatabase.PExecute("UPDATE characters SET at_login = at_login | '256' WHERE guid = '%u'", target->GetGUIDLow());
    }
    else
    {
        std::string oldNameLink = playerLink(target_name);

        PSendSysMessage(LANG_CONVERTTITLES_PLAYER_GUID, oldNameLink.c_str(), target_guid.GetCounter());
        CharacterDatabase.PExecute("UPDATE characters SET at_login = at_login | '256' WHERE guid = '%u'", target_guid.GetCounter());
    }
...
{
    PSendSysMessage("Maze Clean: start working");

    Map* map = sMapMgr.FindMap(13);
    if (!map)
        return false;
    //Player* pl = m_session->GetPlayer();
    ...
    if (result)
    {
        do
        {
            ...
            if (GameObjectData const* go_data = sObjectMgr.GetGOData(guid))
                ...
                obj = map->GetGameObject(ObjectGuid(HIGHGUID_GAMEOBJECT, go_data->id, guid));
            if (!obj)
                continue;
    ...
    WorldDatabase.PQuery("DELETE FROM gameobject WHERE map=13");

    PSendSysMessage("Maze Clean: %u objects deleted from map 'Testing (13)'",count);
    return true;
}
...
{
    if (m_session->GetPlayer()->GetMap()->GetId() != 13)
    {
        PSendSysMessage("You must be in Test map (id 13) - type .tele test");
        return false;
    }

    PSendSysMessage("Maze Generate: start working");
    int level;
    if (*args)
        level = (int)atoi(args);
    else
    ...
    if (level == 0 || level == -1)
    {
        PSendSysMessage("Maze Generate: maze[xSteps+1][ySteps+1][10]; (%u iterations)", iterations);
        int wallLength = 0;
        // cleaning maze
        for (x = 0; x <= xSteps; ++x)
            for (y = 0; y <= ySteps; ++y)
        ...
        for (z = 1; z <= zSteps; z++)
        {
            ...
            if (z == 3)
            {
                // if floor 3 then need more perimetrs
                for (x = 0; x <= xSteps; ++x)
                {
            ...
            for (i=0; i<=iterations; ++i)
            {
                ...
                if (maze[x][y][z])
                    continue; // if at this point already is a wall, then continue to randomly throw points
                ...
                count = 0;
                // until we meet another wall
                while (!maze[x][y][z] && count <= wallLength)
                {
    ...
    if (level == 0 || level == 1)
    {
        PSendSysMessage("Maze Generate: placing level 1: walls of 1st floor");
    ...
    if (level == 0 || level == 2)
    {
        PSendSysMessage("Maze Generate: placing level 2: walls of 1st floor, ground of microlevel");
        for (int x = 0; x <= xSteps; ++x)
           for (int y = 0; y <= ySteps; ++y)
    ...
    //PSendSysMessage("Maze Generate: skipping level 3: empty walls of 2st floor");
    //PSendSysMessage("Maze Generate: skipping level 4: empty roof of 2st floor");

    if (level == 0 || level == 5)
    {
        PSendSysMessage("Maze Generate: placing level 5: ground of 2th floor");
        for (int x = 0; x <= xSteps; ++x)
            for (int y = 0; y <= ySteps; ++y)
            {
    ...
    if (level == 0 || level == 6)
    {
        PSendSysMessage("Maze Generate: placing level 6: walls of 2th floor");
        for (int x = 0; x <= xSteps; ++x)
            for (int y = 0; y <= ySteps; ++y)
    ...
    if (level == 0 || level == 7)
    {
        PSendSysMessage("Maze Generate: placing level 7: walls of 2th floor (only in center 1\4)");
    ...
    if (level == 0 || level == 8)
    {
        PSendSysMessage("Maze Generate: placing level 8: hole-roof of 2th floor - hole-ground of 3th floor ");
        for (int x = 1 * xSteps / 4; x <= 3 * xSteps / 4; ++x)
    ...
    if (level == 0 || level == 9)
    {
        PSendSysMessage("Maze Generate: placing level 9: walls of 3th floor");
        for (int x = 1 * xSteps / 4; x <= 3 * xSteps / 4; ++x)
            for (int y = 1 * ySteps / 4; y <= 3 * ySteps/4; ++y) // (only in center 1\4)
    ...
    if (level==0||level==10){
        PSendSysMessage("Maze Generate: placing level 10: walls(2) of 3th floor");
        for (int x = 1 * xSteps / 4; x <= 3 * xSteps / 4; ++x)
            for (int y = 1 * ySteps / 4; y <= 3 * ySteps / 4; ++y) // (only in center 1\4)
                if (maze[x][y][3]) // get info from maze[x][y][3]
    ...
    if (level == 0 || level == 11)
    {
        PSendSysMessage("Maze Generate: placing level 11: roof of 3th floor (only in center 1\4)");
        for (int x = 1 * xSteps / 4; x <= 3 * xSteps / 4; ++x)
            for (int y = 1 * ySteps / 4; y <= 3 * ySteps / 4; ++y)
    ...
    {
        int lvl = 12;
        PSendSysMessage("Maze Generate: placing level 12: walls and chests of 4th WIN-floor");
    ...
    if (level == 0 || level == -2)
    {
        PSendSysMessage("Maze Generate: placing staircases");
        // center 1->3
    ...
    PSendSysMessage("Maze Generate: done!");
    return true;
}
...
bool ChatHandler::MazeAddGameObject(int id, float x, float y, float z, float o)
{
    ...
    if (!pGameObj->Create(db_lowGUID, gInfo->id, map, chr->GetPhaseMgr()->GetPhaseMaskForSpawn(), x, y, z, o))
    {
        delete pGameObj;
        return false;
    }
    ...
    pGameObj->SaveToDB(map->GetId(), (1 << map->GetSpawnMode()), chr->GetPhaseMgr()->GetPhaseMaskForSpawn());
...
bool ChatHandler::HandleGroupInfoCommand(char* args)
{
    ...
    std::string leaderLink = playerLink(grp->GetLeaderName());
    PSendSysMessage(LANG_GINFO, nameLink.c_str(), grp->GetMembersCount(), leaderLink.c_str(), grp->GetObjectGuid().GetCounter());
    uint32 counter = 0;
    for (GroupReference* itr = grp->GetFirstMember(); itr != NULL; itr = itr->next())
    {
        ...
        PSendSysMessage(LANG_GINFO_MEMBER, ++counter, plNameLink.c_str(), mapEntry ? mapEntry->name[GetSessionDbcLocale()] : "<unknown>", mapId, pl->GetObjectGuid() == grp->GetLeaderGuid() ? GetMangosString(LANG_GINFO_LEADER) : "", pl->IsTaxiFlying() ? GetMangosString(LANG_GINFO_ON_TAXI) : "");
    }
