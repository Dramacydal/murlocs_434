...
CreatureEventAI::CreatureEventAI(Creature *c ) : CreatureAI(c)
{
    ...
    if (creatureEventsItr != sEventAIMgr.GetCreatureEventAIMap().end())
    {
        ...
        if (events_count == 0)
            ERROR_LOG("CreatureEventAI: Creature %u has events but no events added to list because of instance flags.", m_creature->GetEntry());
        else
        {
        ...
        ERROR_LOG("CreatureEventAI: EventMap for Creature %u is empty but creature is using CreatureEventAI.", m_creature->GetEntry());
...
void CreatureEventAI::ProcessAction(CreatureEventAI_Action const& action, uint32 rnd, uint32 EventId, Unit* pActionInvoker)
{
    switch (action.type)
    {
        ...
        case ACTION_T_CAST:
        {
            ...
            if (!target)
            {
                DEBUG_LOG("CreatureEventAI: NULL target for ACTION_T_CAST creature entry %u casting spell id %u", m_creature->GetEntry(), action.cast.spellId);
                return;
            }
        ...
        case ACTION_T_SET_PHASE:
            DEBUG_FILTER_LOG(LOG_FILTER_AI_AND_MOVEGENSS, "CreatureEventAI: Event %d set Phase on %i. CreatureEntry = %u", EventId, action.set_phase.phase, m_creature->GetEntry());
        ...
        case ACTION_T_INC_PHASE:
        {
            ...
            if (action.set_inc_phase.step < 0)
                DEBUG_FILTER_LOG(LOG_FILTER_AI_AND_MOVEGENSS, "CreatureEventAI: Event %d decrease Phase on %i. Old Phase is %u. New Phase is %i. Inverse mask is %u. CreatureEntry = %u", EventId, action.set_inc_phase.step, m_Phase, new_phase, (1 << m_Phase), m_creature->GetEntry());
            else
                DEBUG_FILTER_LOG(LOG_FILTER_AI_AND_MOVEGENSS, "CreatureEventAI: Event %d increase Phase on %i. Old Phase is %u. New Phase is %i. Inverse mask is %u. CreatureEntry = %u", EventId, action.set_inc_phase.step, m_Phase, new_phase, (1 << m_Phase), m_creature->GetEntry());
...
{
    if (!pUnit || !pSpell || m_bEmptyList)
        return;

    DEBUG_FILTER_LOG(LOG_FILTER_AI_AND_MOVEGENSS, "CreatureEventAI::SpellHit: %s cast spell %u", pUnit->GetGuidStr().c_str(), pSpell->Id);
