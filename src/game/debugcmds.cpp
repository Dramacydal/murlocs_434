...
bool ChatHandler::HandleDebugBgEventCommand(char* args)
{
    uint32 event1;
    if (!ExtractUInt32(&args, event1))
        return false;

    uint32 event2;
    if (!ExtractUInt32(&args, event2))
        return false;
        
    uint32 spawn;
    if (!ExtractUInt32(&args, spawn))
        return false;

    if (m_session && m_session->GetPlayer() && m_session->GetPlayer()->GetBattleGround())
    {
        BattleGround* bg = m_session->GetPlayer()->GetBattleGround();
        bg->SpawnEvent(event1, event2, spawn);
    }

    return true;
}

bool ChatHandler::HandleDebugBgObjectsCommand(char* args)
{
    uint32 event1;
    if (!ExtractUInt32(&args, event1))
        return false;
    ...
    {
        bool active = bg->IsActiveEvent(event1, event2);
        PSendSysMessage("Listing battleground objects for %s event %u %u for battleground %s",
            active ? "ACTIVE" : "NON-ACTIVE", event1, event2, bg->GetName());
        int counter = 0;
        GuidVector::const_iterator itr = bg->m_EventObjects[MAKE_PAIR32(event1, event2)].creatures.begin();
        if (itr == bg->m_EventObjects[MAKE_PAIR32(event1, event2)].creatures.end())
            PSendSysMessage("Event creatures are empty!");
        else
        {
            PSendSysMessage("Event creatures:");
            for (; itr != bg->m_EventObjects[MAKE_PAIR32(event1, event2)].creatures.end(); ++itr)
                PSendSysMessage("%02u: %s Present: %s", counter++, itr->GetString().c_str(), player->GetMap()->GetCreature(*itr) ? "YES" : "NO");
        }

        counter = 0;
        GuidVector::const_iterator itr2 = bg->m_EventObjects[MAKE_PAIR32(event1, event2)].gameobjects.begin();
        if (itr2 == bg->m_EventObjects[MAKE_PAIR32(event1, event2)].gameobjects.end())
            PSendSysMessage("Event gameobjects are empty!");
        else
        {
            PSendSysMessage("Event gameobjects:");
            for (; itr2 != bg->m_EventObjects[MAKE_PAIR32(event1, event2)].gameobjects.end(); ++itr2)
                PSendSysMessage("%02u: %s Present: %s", counter++, itr2->GetString().c_str(), player->GetMap()->GetGameObject(*itr2) ? "YES" : "NO");
        }
    }
    else
        PSendSysMessage("You are not in battleground!");

    return true;
}

bool ChatHandler::HandleDebugBgOpenDoorEventCommand(char* args)
{
    uint32 event1;
    if (!ExtractUInt32(&args, event1))
        return false;
    ...
    Player* player = m_session->GetPlayer();
    if (BattleGround* bg = player->GetBattleGround())
        bg->OpenDoorEvent(event1, event2);
    else
        PSendSysMessage("You are not in battleground!");
...
bool ChatHandler::HandleDebugDistanceCommand(char* args)
{
    Unit* unit = getSelectedUnit();
    Unit* player = m_session->GetPlayer();
    if (!unit || unit == player)
    {
        PSendSysMessage("Need select target unit.");
        SetSentErrorMessage(true);
        return false;
    }

    PSendSysMessage("Distance to %s:", unit->GetObjectGuid().GetString().c_str());
    PSendSysMessage("Exact distance: %f", GetExactDistance2d(unit->GetPositionX(), unit->GetPositionY(), player->GetPositionX(), player->GetPositionY()));
    PSendSysMessage("Distance:       %f", unit->GetDistance(player));
    PSendSysMessage("Distance 2d:    %f", unit->GetDistance2d(player));
...
bool ChatHandler::HandleDebugAnimCommand(char* args)
{
    ...
    m_session->GetPlayer()->HandleEmote(emote_id);
    return true;
}

bool ChatHandler::HandleDebugTextEmoteCommand(char* args)
{
    uint32 text_emote;
    ObjectGuid guid;

    if (!ExtractUInt32(&args, text_emote))
        return false;

    guid = m_session->GetPlayer()->GetSelectionGuid();
    ...
    m_session->HandleTextEmoteOpcode(data);
    return true;
}
...
bool ChatHandler::HandleDebugAuraCommand(char* args)
{
    if (!*args)
        return false;
    ...
    {
        PSendSysMessage("Spell %u does not apply any aura!", spellId);
        SetSentErrorMessage(true);
        return false;
    }

    uint32 counter = 0;
    Unit::SpellAuraHolderBounds spair = player->GetSpellAuraHolderBounds(spellId);
    for (Unit::SpellAuraHolderMap::const_iterator iter = spair.first; iter != spair.second; ++iter)
    {
        SpellAuraHolder* holder = iter->second;

        PSendSysMessage("%u: Spell: %u Caster: %s", ++counter, spellId, holder->GetCasterGuid().GetString().c_str());
        ...
        for (uint32 i = 0; i < MAX_EFFECT_INDEX; ++i)
        {
            ...
            if (aura)
            {
                ...
                PSendSysMessage("EffIndex: %u, m_auraname %u, m_amount: %i m_miscvalue %i", i, mod->m_auraname, mod->m_amount, mod->m_miscvalue);
            }
            else
                PSendSysMessage("EffIndex: %u: -//-", i);
        }
    }
...
bool ChatHandler::HandleDebugResistanceCommand(char* args)
{
    Unit* target = getSelectedUnit();
    if(!target)
    {
    ...
    std::string name = target->GetTypeId() == TYPEID_PLAYER ? playerLink(target->GetName()) : target->GetName();

    PSendSysMessage("Resistances of %s:", name.c_str());
    PSendSysMessage("Normal: %u", target->GetResistance(SPELL_SCHOOL_NORMAL));
    PSendSysMessage("Holy:   %u", target->GetResistance(SPELL_SCHOOL_HOLY));
    PSendSysMessage("Fire:   %u", target->GetResistance(SPELL_SCHOOL_FIRE));
    PSendSysMessage("Nature: %u", target->GetResistance(SPELL_SCHOOL_NATURE));
    PSendSysMessage("Frost:  %u", target->GetResistance(SPELL_SCHOOL_FROST));
    PSendSysMessage("Shadow: %u", target->GetResistance(SPELL_SCHOOL_SHADOW));
    PSendSysMessage("Arcane: %u", target->GetResistance(SPELL_SCHOOL_ARCANE));
    Player* plr = target->GetCharmerOrOwnerPlayerOrPlayerItself();
    PSendSysMessage("Spell Penetration: %i", plr ? plr->GetSpellPenetrationItemMod() : 0);
...
bool ChatHandler::HandleDebugTransportCommand(char* args)
{
    ...
    Transport* trans = plr->GetTransport();

    PSendSysMessage("MO Transport: %s ", trans ? trans->GetGuidStr().c_str() : "NONE");
    PSendSysMessage("Movement info: Trans: %s Tx: %f Ty: %f Tz: %f To: %f", plr->m_movementInfo.GetTransportGuid().GetString().c_str(),
        plr->m_movementInfo.GetTransportPos()->x, plr->m_movementInfo.GetTransportPos()->y, plr->m_movementInfo.GetTransportPos()->z, plr->m_movementInfo.GetTransportPos()->o);
