...
MapManager::Update(uint32 diff)
{
    ...
    for (MapMapType::iterator iter=i_maps.begin(); iter != i_maps.end(); ++iter)
    {
        ...
        {
            m_LastMapUpdateTimes[iter->first.nMapId] = WorldTimer::getMSTime();
            iter->second->Update((uint32)i_timer.GetCurrent());
            m_LastMapUpdateTimes[iter->first.nMapId] = WorldTimer::getMSTime() - m_LastMapUpdateTimes[iter->first.nMapId];
            if (m_MaxMapUpdateTimes.find(iter->first.nMapId) != m_MaxMapUpdateTimes.end())
                if (m_MaxMapUpdateTimes[iter->first.nMapId] < m_LastMapUpdateTimes[iter->first.nMapId])
                    m_MaxMapUpdateTimes[iter->first.nMapId] = m_LastMapUpdateTimes[iter->first.nMapId];
        }
    }
