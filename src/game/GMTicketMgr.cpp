...
void GMTicketMgr::UpdateFromDB()
{
    QueryResult * result = CharacterDatabase.PQuery("SELECT guid, UNIX_TIMESTAMP(ticket_lastchange) FROM `character_ticket`");
    if (result)
    {
        do
        {
            Field *fields = result->Fetch();
            uint32 guidlow = fields[0].GetUInt32();
            uint64 time = fields[1].GetUInt32();
            ObjectGuid guid = ObjectGuid(HIGHGUID_PLAYER, guidlow);
            GMTicket* ticket = GetGMTicket(guid);
            if (!ticket)
                continue;

            if (!ticket->GetPlayerGuid().IsEmpty())                 // already exist
            {
                if (ticket->GetLastUpdate() < time)
                {
                    QueryResult * result2 = CharacterDatabase.PQuery("SELECT response_text FROM `character_ticket` WHERE guid = %u", guid.GetCounter());
                    if (result2)
                    {
                        Field *fields2 = result2->Fetch();
                        ticket->SetResponseText(fields2[0].GetCppString().c_str());
