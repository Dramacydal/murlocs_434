...
pAuraHandler AuraHandler[TOTAL_AURAS]=
{
    ...
    &Aura::HandleNoImmediateEffect,                         //303 SPELL_AURA_MOD_DAMAGE_DONE_VERSUS_AURASTATE implemented in Unit::SpellDamageBonusDone, Unit::MeleeDamageBonusDone
    &Aura::HandleAuraFakeInebriation,                       //304 SPELL_AURA_MOD_FAKE_INEBRIATE
    ...
    &Aura::HandleNoImmediateEffect                          //316 SPELL_AURA_MOD_PERIODIC_HASTE
};
...
m_isPersistent(false), m_in_use(0), m_spellAuraHolder(holder)
{
    ...
    if (caster && caster->GetTypeId() == TYPEID_PLAYER && spellproto->SpellFamilyName == SPELLFAMILY_POTION)
    {
        ...
        switch (spellspec)
        {
            ...
            case SPELL_FLASK_ELIXIR:
            {
                ...
                uint32 trigger = spellproto->EffectTriggerSpell[0];
        ...
        case SPELLFAMILY_DRUID:
        {
            ...
            if (GetSpellProto()->SpellIconID == 2855)
            {
                ...
                switch(GetId())
                {
                    ...
                    default:
                        ERROR_LOG("HandleAuraDummy: Not handled rank of IMF (Spell: %u)",GetId());
                        return;
                }
    ...
    if (apply)
    {
        switch(GetSpellProto()->SpellFamilyName)
        {
            case SPELLFAMILY_HUNTER:
            {
                ...
                if (GetEffIndex() == EFFECT_INDEX_1 && (GetSpellProto()->SpellFamilyFlags & UI64LIT(0x400)))
...
void Aura::HandleAuraModStun(bool apply, bool Real)
{
    ...
    if (apply)
    {
        ...
        else if (GetId() == 20066)  // Repentance
            target->RemoveAurasDueToSpell(61840);   // Righteous Vengeance
    }
    else
    {
        // Frost stun aura -> freeze/unfreeze target
        ...
        if (GetSpellProto()->SpellFamilyName == SPELLFAMILY_HUNTER && GetSpellProto()->SpellFamilyFlags & UI64LIT(0x0000100000000000))
        {
            ...
            switch (GetId())
            {
                ...
                default:
                    ERROR_LOG("Spell selection called for unexpected original spell %u, new spell for this spell family?",GetId());
                    return;
            }
...
void Aura::HandlePeriodicHeal(bool apply, bool /*Real*/)
{
    ...
    if (apply)
    {
        ...
        if (GetSpellProto()->SpellFamilyName == SPELLFAMILY_DRUID && GetSpellProto()->SpellFamilyFlags & UI64LIT(0x1000000000))
...
void Aura::HandleModDamagePercentDone(bool apply, bool Real)
{
    ...
    {
        Unit::AuraList const& auras = GetTarget()->GetAurasByType(SPELL_AURA_MOD_SPELL_CRIT_CHANCE);
        for (Unit::AuraList::const_iterator i = auras.begin(); i != auras.end(); ++i)
                ...
                for(Unit::AuraList::const_iterator i = dummyAuras.begin(); i != dummyAuras.end(); ++i)
                {
                        ...
                        (*i)->GetSpellProto()->SpellIconID == 2855)
                    {
                        ...
                        switch((*i)->GetId())
                        {
                            ...
                            case 48396:spell_id=50172;break;//Rank 3
                            default:
                                ERROR_LOG("Aura::HandleShapeshiftBoosts: Not handled rank of IMF (Spell: %u)",(*i)->GetId());
                                break;
                        }
                ...
                case SPELLFAMILY_PRIEST:
                    ...
                    if (spellProto->SpellFamilyFlags & UI64LIT(0x0000000000000001))
                    {
                        ...
                        for (Unit::AuraList::const_iterator itr = borrowedTime.begin(); itr != borrowedTime.end(); ++itr)
                        {
                            ...
                            if (spellProto->SpellFamilyName == SPELLFAMILY_PRIEST && spellProto->SpellIconID == 2899 && spellProto->EffectMiscValue[(*itr)->GetEffIndex()] == 24)
                            {
                        ...
                        for (Unit::AuraList::const_iterator itr = ipwstd.begin(); itr != ipwstd.end(); ++itr)
                        {
                            ...
                            SpellEntry const * spellProto = (*itr)->GetSpellProto();
                            if (spellProto->SpellFamilyName == SPELLFAMILY_PRIEST && spellProto->SpellIconID == 566)
                            {
                                IMP = (*itr)->GetModifier()->m_amount / 100.0f;
                                ++counter;
                            }
                            // Twin Disciplines
                            if (spellProto->SpellFamilyName == SPELLFAMILY_PRIEST && spellProto->SpellIconID == 2292)
                            {
                        ...
                        {
                            SpellEntry const * spellProto = (*itr)->GetSpellProto();
                            if (spellProto->SpellFamilyName == SPELLFAMILY_PRIEST && spellProto->SpellIconID == 2210)
                            {
    ...
    switch(m_modifier.m_auraname)
    {
        ...
        case SPELL_AURA_OBS_MOD_HEALTH:
        {
            ...
            if(target != pCaster && spellProto->SpellVisual[0] == 163)
            {
                ...
                if (dmg > gain)
                    dmg = gain;
                if(pCaster->GetHealth() <= uint32(dmg) && pCaster->GetTypeId()==TYPEID_PLAYER)
                {
...
//              case 42919: break;
                ...
                case 42993:
                {
                    ...
                    if (SpellAuraHolder* holder = target->GetSpellAuraHolder(43052))
                    {
                        if (holder->GetStackAmount() != holder->GetSpellProto()->StackAmount)
        ...
        case SPELLFAMILY_DEATHKNIGHT:
        {
            ...
            if (spell->SpellFamilyFlags & UI64LIT(0x0000000000000020))
            {
                if (Unit *caster = GetCaster())
                {
                    ...
                    if (Aura* glyph = caster->GetDummyAura(70650))
                        bp *= (glyph->GetModifier()->m_amount + 100.0f)/100.0f;

                    caster->CastCustomSpell(target, 52212, &bp, NULL, NULL, true, NULL, this);
                }
                return;
            }
            // Raise Dead
...
void SpellAuraHolder::_AddSpellAuraHolder()
{
    ...
    flags |= ((GetCasterGuid() == GetTarget()->GetObjectGuid()) ? AFLAG_NOT_CASTER : AFLAG_NONE) | ((GetSpellMaxDuration(m_spellProto) > 0 && (m_spellProto->AttributesEx5 & SPELL_ATTR_EX5_HIDE_DURATION) == 0) ? AFLAG_DURATION : AFLAG_NONE) | (IsPositive() ? AFLAG_POSITIVE : AFLAG_NEGATIVE);
    SetAuraFlags(flags);
...
void SpellAuraHolder::CleanupTriggeredSpells()
{
    for (int32 i = 0; i < MAX_EFFECT_INDEX; ++i)
    {
        ...
        uint32 tSpellId = GetSpellProto()->EffectTriggerSpell[i];
        if(!tSpellId)
            continue;
            ...
            else if (m_spellProto->IsFitToFamilyMask(0x1000000))
            {
                if (apply)
                {
                    if (Unit* caster = GetCaster())
                    {
                        ...
                        {
                            if ((*itr)->GetSpellProto()->SpellFamilyName == SPELLFAMILY_WARLOCK && (*itr)->GetSpellProto()->SpellIconID == 153 && (*itr)->GetEffIndex() == EFFECT_INDEX_0)
                            {
                                switch((*itr)->GetId())
                                {
            ...
            switch (GetId())
            {
                ...
                case 48266:                                 // Blood Presence
                {
                    ...
                    if (GetId()==48263 || GetId()==48265)   // Frost Presence or Unholy Presence
                    {
                        ...
                        m_target->CastSpell(m_target, 63611, true, NULL, NULL, GetCasterGuid());
                    else 
                        m_target->RemoveAurasDueToSpell(63611);
                    ...
                    if (GetId()==48263 || GetId()==48266)   // Frost Presence or Blood Presence
                    {
                        ...
                        if (power_pct)
                            m_target->CastCustomSpell(m_target, 49772, &power_pct, NULL, NULL, true, NULL, NULL, GetCasterGuid());        // Unholy Presence, speed part, spell1 used for Improvement presence fit to own presence
                        else
                            m_target->RemoveAurasDueToSpell(49772);
                    }
                    else if(apply)
                        m_target->CastSpell(m_target, 49772, true, NULL, NULL, GetCasterGuid());
                    else
                        m_target->RemoveAurasDueToSpell(49772);
    ...
    switch(GetSpellProto()->SpellFamilyName)
    {
        case SPELLFAMILY_GENERIC:
        {
            switch (GetId())
            {
                ...
                {
                    if (GetStackAmount() == GetSpellProto()->StackAmount)
                    {
...
void Aura::HandleAuraLinked(bool apply, bool Real)
{
    ...
    SpellEntry const * spellInfo = GetSpellProto();
    uint32 linkedSpell = spellInfo->EffectTriggerSpell[m_effIndex];
    ...
    {
        if (spellInfo->EffectImplicitTargetA[i] == TARGET_RANDOM_ENEMY_CHAIN_IN_AREA
            || spellInfo->EffectImplicitTargetB[i] == TARGET_RANDOM_ENEMY_CHAIN_IN_AREA
            || spellInfo->EffectImplicitTargetA[i] == TARGET_CHANNEL
            || spellInfo->EffectImplicitTargetB[i] == TARGET_CHANNEL
            || spellInfo->EffectImplicitTargetA[i] == TARGET_AREAEFFECT_CUSTOM
            || spellInfo->EffectImplicitTargetB[i] == TARGET_AREAEFFECT_CUSTOM)
            caster = GetCaster();
    }
