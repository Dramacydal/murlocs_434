...
void WorldSession::HandleVoidStorageTransfer(WorldPacket& recvData)
{
    ...
    {
        ERROR_LOG("WORLD: HandleVoidStorageTransfer - %s wants to deposit more than %u items (%u).", player->GetGuidStr().c_str(), VOID_STORAGE_MAX_DEPOSIT, countDeposit);
        recvData.rfinish();
        return;
    }
    ...
    {
        ERROR_LOG("WORLD: HandleVoidStorageTransfer - %s wants to withdraw more than %u items (%u).", player->GetGuidStr().c_str(), VOID_STORAGE_MAX_WITHDRAW, countWithdraw);
        recvData.rfinish();
        return;
    }
    ...
    if (!player->IsVoidStorageUnlocked())
    {
        ERROR_LOG("WORLD: HandleVoidStorageTransfer - %s queried void storage without unlocking it.", player->GetGuidStr().c_str());
        return;
    }
    ...
    for (std::vector<ObjectGuid>::iterator itr = itemIds.begin(); itr != itemIds.end(); ++itr)
    {
        ...
        if (!itemVS)
        {
            ERROR_LOG("WORLD: HandleVoidStorageTransfer - %s tried to withdraw an invalid item (id: " UI64FMTD ")", player->GetGuidStr().c_str(), itr->GetRawValue());
            continue;
        }
...
void WorldSession::HandleVoidSwapItem(WorldPacket& recvData)
{
    ...
    if (!player->IsVoidStorageUnlocked())
    {
        ERROR_LOG("WORLD: HandleVoidSwapItem - %s queried void storage without unlocking it.", player->GetGuidStr().c_str());
        return;
    }
    ...
    {
        ERROR_LOG("WORLD: HandleVoidSwapItem - %s requested swapping an invalid item (slot: %u, itemid: " UI64FMTD ").", player->GetGuidStr().c_str(), newSlot, itemId.GetRawValue());
        return;
    }
