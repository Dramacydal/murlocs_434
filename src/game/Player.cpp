...
bool Player::LoadFromDB(ObjectGuid guid, SqlQueryHolder *holder )
{
    ...
    {
        ERROR_LOG("Player can have not more %u stable slots, but have in DB %u",MAX_PET_STABLES,uint32(m_stableSlots));
    ...
    m_GrantableLevelsCount = fields[59].GetUInt32();
    ...
    if (sWorld.getConfig(CONFIG_BOOL_CHARACTER_ADDON_ENABLED))
    {
        ...
        if (result_addon)
        {
            ...
            if (phasemask > 0)
                GetPhaseMgr()->SetCustomPhase(fields_addon[2].GetUInt32());
...
void Player::RemoveItemDependentAurasAndCasts(Item * pItem)
{
    ...
    for (uint32 i = 0; i < CURRENT_MAX_SPELL; ++i)
        if (Spell* spell = GetCurrentSpell(CurrentSpellTypes(i)))
            if (spell->getState()!=SPELL_STATE_DELAYED && !spell->m_customSpellData.HasFlag(CUSTOM_SPELL_FLAG_IGNORE_EQUIPPED_ITEM_REQ) && !HasItemFitToSpellReqirements(spell->m_spellInfo,pItem) )
