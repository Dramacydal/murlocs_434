...
void Player::RemoveItemDependentAurasAndCasts(Item * pItem)
{
    ...
    for (uint32 i = 0; i < CURRENT_MAX_SPELL; ++i)
        if (Spell* spell = GetCurrentSpell(CurrentSpellTypes(i)))
            if (spell->getState()!=SPELL_STATE_DELAYED && !spell->m_customSpellData.HasFlag(CUSTOM_SPELL_FLAG_IGNORE_WEAPON_REQ) && !HasItemFitToSpellReqirements(spell->m_spellInfo,pItem) )
