...
TradeSlots const TradeData::GetTradeSlotForItem(ObjectGuid itemGuid)
{
...
Player::Player (WorldSession *session): Unit(), m_mover(this), m_camera(this), m_achievementMgr(this), m_reputationMgr(this)
{
    ...
    m_GMkillscore = 0;
    m_GMtimekillscore = WorldTimer::getMSTime();
    ...
    if(GetSession()->GetSecurity() <= SEC_CURATOR)
    ...
    m_lastOkBgAfkTime = 0;
    m_bgAfkDistance = 0.f;
    GetPosition(m_bgLastAfkPosition.x, m_bgLastAfkPosition.y, m_bgLastAfkPosition.z);

    m_nextVehicleId = 0;

    m_vehicleDespawnTimer = 0;
    ...
    m_uiSoulboundUpdateTimer = 1 * IN_MILLISECONDS;
...
bool Player::Create( uint32 guidlow, const std::string& name, uint8 race, uint8 class_, uint8 gender, uint8 skin, uint8 face, uint8 hairStyle, uint8 hairColor, uint8 facialHair, uint8 /*outfitId */)
{
    ...
    if(!info)
    {
        ERROR_LOG("Player have incorrect race/class pair. Can't be loaded.");
        return false;
    }
    ...
    if(!cEntry)
    {
        ERROR_LOG("Class %u not found in DBC (Wrong DBC files?)",class_);
        return false;
    }
    ...
    if (GetAccountLinkedState() != STATE_NOT_LINKED)
        SetByteValue(PLAYER_BYTES_2, 3, REST_STATE_NORMAL | REST_STATE_RAF_LINKED); // rest state = refer-a-friend
    else
...
bool Player::StoreNewItemInBestSlots(uint32 titem_id, uint32 titem_amount)
{
    ...
    // item can't be added
    ERROR_LOG("STORAGE: Can't equip or store initial item %u for race %u class %u , error msg = %u",titem_id,getRace(),getClass(),msg);
    return false;
}
...
void Player::SetDrunkValue(uint16 newDrunkenValue, uint32 itemId)
{
    ...
    if(newDrunkenState >= DRUNKEN_DRUNK)
        m_detectInvisibilityMask |= (1<<INVISIBILITY_DRUNK);
    else
        m_detectInvisibilityMask &= ~(1<<INVISIBILITY_DRUNK);
...
void Player::Update(uint32 update_diff, uint32 p_time)
{
    ...
    if (m_uiSoulboundUpdateTimer < update_diff)
    {
        m_uiSoulboundUpdateTimer = 1 * IN_MILLISECONDS;
        ...
        m_uiSoulboundUpdateTimer -= update_diff;
    ...
    if (now > m_Last_tick)
    {
        ...
        if (GetSession()->GetSecurity() > SEC_PLAYER && !isAFK())
            m_Played_time[PLAYED_TIME_GM] += elapsed;
        m_Last_tick = now;
    }
    ...
    if (Pet* pet = GetPet())
    {
        ...
        {
            if (m_petScalingUpdateTimer <= update_diff)
            {
                pet->UpdateScalingAuras();
                m_petScalingUpdateTimer = 0;
            }
            else
                m_petScalingUpdateTimer -= update_diff;
        }
    }

    // Bg Anti-AFK check
    BattleGround* bg = GetBattleGround();
    if (!isGameMaster() && bg && !bg->isArena())
    {
        if (!m_lastOkBgAfkTime)
        {
            m_lastOkBgAfkTime = time(NULL);
            m_bgAfkDistance = 0.f;
            GetPosition(m_bgLastAfkPosition.x, m_bgLastAfkPosition.y, m_bgLastAfkPosition.z);
        }
        else if (time(NULL) - m_lastOkBgAfkTime > 5 * MINUTE + 10)
        {
            m_lastOkBgAfkTime = 0;
            m_bgAfkDistance = 0.f;
    ...
    if (m_vehicleCheckTimer <= update_diff)
    {
        m_vehicleCheckTimer = 1000;

        if (VehicleKit* vehicle = GetVehicle())
        {
            bool ok = true;
            Unit* base = vehicle->GetBase();
            if (base && base->GetTypeId() == TYPEID_UNIT && base->GetCharmerOrOwnerGuid() == GetObjectGuid())
            {
                if (uint32 spell_id = base->GetUInt32Value(UNIT_CREATED_BY_SPELL))
                {
                    if (SpellEntry const* spellEntry = sSpellStore.LookupEntry(spell_id))
                    {
                        ...
                        if (sSpellMgr.GetSpellAllowedInLocationError(spellEntry, GetMapId(), zoneid, areaid, this) != SPELL_CAST_OK)
                        {
                            // maybe despawn timer already active?
                            if (!m_vehicleDespawnTimer)
                            {
                                m_vehicleDespawnTimer = 15 * IN_MILLISECONDS;
                                
                                if (m_lastOkVehicleAreaId && GetSession())
                                    SendReturnToAreaMessage(m_lastOkVehicleAreaId, m_vehicleDespawnTimer);
                            }
                            ok = false;
                        }
                        else
                            m_lastOkVehicleAreaId = areaid;
                    }
                }
            }
            if (ok)
        ...
        m_vehicleCheckTimer -= update_diff;

    if (m_vehicleDespawnTimer)
    {
        if (m_vehicleDespawnTimer <= update_diff)
        {
            m_vehicleDespawnTimer = 0;
            ...
            if (VehicleKit* vehicle = GetVehicle())
            {
                Unit* base = vehicle->GetBase();
                if (base && base->GetTypeId() == TYPEID_UNIT && base->GetCharmerOrOwnerGuid() == GetObjectGuid())
                    ((Creature*)base)->ForcedDespawn();
            }
        }
        else
            m_vehicleDespawnTimer -= update_diff;
    }

    if (m_kickDelay)
    {
        if (m_kickDelay <= update_diff)
        {
            m_kickDelay = 0;
            if (GetSession())
        ...
        {
            uint32 val1 = m_kickDelay / 1000;
            m_kickDelay -= update_diff;
            uint32 val2 = m_kickDelay / 1000;
            if (val1 != val2)
            {
                ChatHandler(this).PSendSysMessage(LANG_YOU_WILL_BE_KICKED_IN, val1);
                GetSession()->SendNotification(LANG_YOU_WILL_BE_KICKED_IN, val1);
            }
        }
    }
    ...
    {
        if (sWorld.GetAnticheatBuggedValueSpd() > 0)
        {
            uint32 spd[7];
            spd[0] = GetUInt32Value(PLAYER_FIELD_MOD_HEALING_DONE_POS);
            for (int i = SPELL_SCHOOL_HOLY; i < MAX_SPELL_SCHOOL; i++)
                spd[i] = GetUInt32Value(PLAYER_FIELD_MOD_DAMAGE_DONE_POS + i);

            for (int i = 0; i < 7; i++)
            {
                if (spd[i] > sWorld.GetAnticheatBuggedValueSpd())
                {
                    std::stringstream banString;
                    banString << "Exploit 1";
                    for (int i = 0; i < 7; i++)
                        banString << " " << spd[i];
                    sWorld.BanAccount(BAN_CHARACTER_ACC, GetName(), -1, banString.str().c_str(), "Anticheat (WAF)");
                    break;
                }
            }
        }
        if (sWorld.GetAnticheatBuggedValueAP() > 0)
        {
            uint32 baseAp = GetTotalAttackPowerValue(BASE_ATTACK);
            uint32 offAp = GetTotalAttackPowerValue(OFF_ATTACK);
            uint32 rangedAp = GetTotalAttackPowerValue(RANGED_ATTACK);
            if (baseAp > sWorld.GetAnticheatBuggedValueAP() ||
                offAp > sWorld.GetAnticheatBuggedValueAP() ||
                rangedAp > sWorld.GetAnticheatBuggedValueAP())
            {
                std::stringstream banString;
                banString << "Exploit 2 " << baseAp << " " << offAp << " " << rangedAp;
                sWorld.BanAccount(BAN_CHARACTER_ACC, GetName(), -1, banString.str().c_str(), "Anticheat (WAF)");
            }
        }
        if (sWorld.GetAnticheatBuggedValueHealth() > 0)
        {
            uint32 value = GetMaxHealth();
            if (value > sWorld.GetAnticheatBuggedValueHealth())
            {
                std::stringstream banString;
                banString << "Exploit 3 " << value;
                sWorld.BanAccount(BAN_CHARACTER_ACC, GetName(), -1, banString.str().c_str(), "Anticheat (WAF)");
            }
        }
...
uint8 Player::GetChatTag() const
{
    ...
    if (HasFlag(PLAYER_FLAGS, PLAYER_FLAGS_DEVELOPER))
        tag |= CHAT_TAG_DEV;
    if (HasFlag(PLAYER_FLAGS, PLAYER_FLAGS_COMMENTATOR | PLAYER_FLAGS_COMMENTATOR_UBER))
        tag |= CHAT_TAG_COM;
...
bool Player::TeleportTo(uint32 mapid, float x, float y, float z, float orientation, uint32 options)
{
    ...
    {
        ERROR_LOG("TeleportTo: invalid map %d or absent instance template.", mapid);
        return false;
    }
...
void Player::ProcessDelayedOperations()
{
    ...
    if (m_DelayedOperations & DELAYED_REMOVE_SPECTATOR)
        SetSpectator(false);
    ...
    switch (power)
    {
        case POWER_MANA:
        {
            ...
            if (HasFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_PREPARATION))
                addvalue = GetMaxPower(POWER_MANA) / 3;
...
GameObject* Player::GetGameObjectIfCanInteractWith(ObjectGuid guid, uint32 gameobject_type) const
{
    ...
    if (GameObject *go = GetMap()->GetGameObject(guid))
    {
        if (uint32(go->GetGoType()) == gameobject_type || gameobject_type == MAX_GAMEOBJECT_TYPE)
        {
            ...
            ERROR_LOG("GetGameObjectIfCanInteractWith: GameObject '%s' [GUID: %u] is too far away from player %s [GUID: %u] to be used by him (distance=%f, maximal %f is allowed)",
...
void Player::InitStatsForLevel(bool reapplyMods)
{
    ...
    RemoveFlag(PLAYER_FLAGS, PLAYER_FLAGS_AFK | PLAYER_FLAGS_DND | PLAYER_FLAGS_GM | PLAYER_FLAGS_GHOST | PLAYER_FLAGS_ALLOW_ONLY_ABILITY);
...
bool Player::addSpell(uint32 spell_id, bool active, bool learning, bool dependent, bool disabled)
{
    ...
    if (!spellInfo)
    {
        ...
        {
            ERROR_LOG("Player::addSpell: nonexistent in SpellStore spell #%u request, deleting for all characters in `character_spell`.",spell_id);
            CharacterDatabase.PExecute("DELETE FROM character_spell WHERE spell = '%u'",spell_id);
        }
        else
            ERROR_LOG("Player::addSpell: nonexistent in SpellStore spell #%u request.",spell_id);
    ...
    if(!SpellMgr::IsSpellValid(spellInfo,this,false))
    {
        ...
        if(!IsInWorld() && !learning)                       // spell load case
        {
            ERROR_LOG("Player::addSpell: Broken spell #%u learning not allowed, deleting for all characters in `character_spell`.",spell_id);
            CharacterDatabase.PExecute("DELETE FROM character_spell WHERE spell = '%u'",spell_id);
        }
        else
            ERROR_LOG("Player::addSpell: Broken spell #%u learning not allowed.",spell_id);
...
void Player::removeSpell(uint32 spell_id, bool disabled, bool learn_low_rank, bool sendUpdate)
{
    ...
    if (talentPos)
    {
        ...
        if (iter != m_talents[m_activeSpec].end())
        {
            ...
            ERROR_LOG("removeSpell: Player (GUID: %u) has talent spell (id: %u) but doesn't have talent",GetGUIDLow(), spell_id );
...
void Player::_LoadSpellCooldowns(QueryResult *result)
{
    ...
    if(result)
    {
        ...
        do
        {
            ...
            {
                ERROR_LOG("Player %u has unknown spell %u in `character_spell_cooldown`, skipping.",GetGUIDLow(),spell_id);
                continue;
            }
...
void Player::DeleteFromDB(ObjectGuid playerguid, uint32 accountId, bool updateRealmChars, bool deleteFinally)
{
    ...
    switch(charDelete_method)
    {
        ...
        case 0:
        {
            ...
            if (resultMail)
            {
                do
                {
                    ...
                    if (has_items)
                    {
                        ...
                        //                                                          0    1    2         3
                        QueryResult *resultItems = CharacterDatabase.PQuery("SELECT data,text,item_guid,item_template FROM mail_items JOIN item_instance ii ON item_guid = ii.guid LEFT JOIN item_instance_text iit ON item_guid = iit.guid WHERE mail_id='%u'", mail_id);
                        if (resultItems)
                        {
                            do
                            {
                                ...
                                if (!itemProto)
                                {
                                    ...
                                    CharacterDatabase.PExecute("DELETE FROM item_instance_text WHERE guid = '%u'", item_guidlow);
                                    continue;
                                }
            ...
            CharacterDatabase.PExecute("DELETE FROM item_instance_text WHERE guid IN (SELECT guid FROM item_instance WHERE owner_guid = '%u')", lowguid);
        ...
        case 1:
            ...
            break;
        default:
            ERROR_LOG("Player::DeleteFromDB: Unsupported delete method: %u.", charDelete_method);
    }
...
void Player::BuildPlayerRepop()
{
    ...
    if(GetCorpse())
    {
        ERROR_LOG("BuildPlayerRepop: player %s(%d) already has a corpse", GetName(), GetGUIDLow());
        //ASSERT(false);
        return; // Aaaioeenu ?oi ee? A anee ii niaieo eiiieo Resurrect?
    }
    ...
    if(!corpse)
    {
        ERROR_LOG("Error creating corpse for Player %s [%u]", GetName(), GetGUIDLow());
        return;
    }
    GetMap()->Add(corpse);
...
uint32 Player::DurabilityRepair(uint16 pos, bool cost, float discountMod, bool guildBank)
{
    ...
    if(cost)
    {
        ...
        if(LostDurability>0)
        {
            ...
            if(!dcost)
            {
                ERROR_LOG("RepairDurability: Wrong item lvl %u", ditemProto->ItemLevel);
                return TotalCost;
            }
            ...
            if(!dQualitymodEntry)
            {
                ERROR_LOG("RepairDurability: Wrong dQualityModEntry %u", dQualitymodEntryId);
                return TotalCost;
            }
            ...
            costs = costs * sWorld.getConfig(CONFIG_UINT32_RATE_REPAIR_PRICE);
...
void Player::HandleBaseModValue(BaseModGroup modGroup, BaseModType modType, float amount, bool apply)
{
    ...
    {
        ERROR_LOG("ERROR in HandleBaseModValue(): nonexistent BaseModGroup of wrong BaseModType!");
        return;
    }
...
float Player::GetBaseModValue(BaseModGroup modGroup, BaseModType modType) const
{
    if(modGroup >= BASEMOD_END || modType > MOD_END)
    {
        ERROR_LOG("trial to access nonexistent BaseModGroup or wrong BaseModType!");
        return 0.0f;
    }
...
float Player::GetTotalBaseModValue(BaseModGroup modGroup) const
{
    ...
    {
        ERROR_LOG("wrong BaseModGroup in GetTotalBaseModValue()!");
        return 0.0f;
    }
...
uint32 Player::GetShieldBlockDamageValue(bool capped) const
{
...
bool Player::UpdateSkillPro(uint16 SkillId, int32 Chance, uint32 step)
{
    ...
    if ( Roll <= Chance )
    {
        ...
        UpdateAllItemEnchantsAtSkill(SkillId, SkillValue, new_value);
    ...
    else if(currVal)                                        // add
    {
        for (int i = 0; i < PLAYER_MAX_SKILLS; ++i)
        {
            ...
            if (!GetUInt16Value(PLAYER_SKILL_LINEID_0 + field, offset))
            {
                ...
                if(!pSkill)
                {
                    ERROR_LOG("Skill not found in SkillLineStore: skill #%u", id);
                    return;
                }
...
void Player::SendActionButtons(uint8 state) const
{
...
bool Player::IsActionButtonDataValid(uint8 button, uint32 action, uint8 type, Player* player, bool msg)
{
    ...
    if(action >= MAX_ACTION_BUTTON_ACTION_VALUE)
    {
        ...
        {
            if (player)
                ERROR_LOG( "Action %u not added into button %u for player %s: action must be < %u", action, button, player->GetName(), MAX_ACTION_BUTTON_ACTION_VALUE );
            else
                ERROR_LOG( "Table `playercreateinfo_action` have action %u into button %u : action must be < %u", action, button, MAX_ACTION_BUTTON_ACTION_VALUE );
        }
        return false;
    }
    ...
    switch(type)
    {
        case ACTION_BUTTON_SPELL:
        {
            ...
            if(!spellProto)
            {
                ...
                {
                    if (player)
                        ERROR_LOG( "Spell action %u not added into button %u for player %s: spell not exist", action, button, player->GetName() );
                    else
                        ERROR_LOG( "Table `playercreateinfo_action` have spell action %u into button %u: spell not exist", action, button );
                }
                return false;
            }
            ...
            if(player)
            {
                ...
                {
                    if (msg)
                        ERROR_LOG( "Spell action %u not added into button %u for player %s: player don't known this spell", action, button, player->GetName() );
                    return false;
                }
                else if(IsPassiveSpell(spellProto))
                {
                    if (msg)
                        ERROR_LOG( "Spell action %u not added into button %u for player %s: spell is passive", action, button, player->GetName() );
                    return false;
                }
                ...
                {
                    if (msg)
                        ERROR_LOG( "Spell action %u not added into button %u for player %s: attempt to add non totem spell to totem bar", action, button, player->GetName() );
                    return false;
                }
            }
            break;
        ...
        case ACTION_BUTTON_ITEM:
        {
            if(!ObjectMgr::GetItemPrototype(action))
            {
                ...
                {
                    if (player)
                        ERROR_LOG( "Item action %u not added into button %u for player %s: item not exist", action, button, player->GetName() );
                    else
                        ERROR_LOG( "Table `playercreateinfo_action` have item action %u into button %u: item not exist", action, button );
                }
                return false;
            }
            break;
...
void Player::CheckAreaExploreAndOutdoor()
{
    ...
    {
        ERROR_LOG("Wrong area flag %u in map data for (X: %f Y: %f) point to field PLAYER_EXPLORED_ZONES_1 + %u ( %u must be < %u ).",areaFlag,GetPositionX(),GetPositionY(),offset,offset, PLAYER_EXPLORED_ZONES_SIZE);
        return;
    }
    ...
    if (!(currFields & val))
    {
        ...
        if(!p)
        {
            ERROR_LOG("PLAYER: Player %u discovered unknown area (x: %f y: %f map: %u", GetGUIDLow(), GetPositionX(),GetPositionY(),GetMapId());
        }
        else if(p->area_level > 0)
        {
            ...
            else
            {
                ...
                uint32 XP = 0;
                float xpMod = IsPremiumActive() ? GetPremiumXPModifier() : 1.0f;
                if (diff < -5)
                {
                    XP = uint32(sObjectMgr.GetBaseXP(getLevel()+5)*sWorld.getConfig(CONFIG_FLOAT_RATE_XP_EXPLORE)*xpMod);
                }
                else if (diff > 5)
                {
                    ...
                    XP = uint32(sObjectMgr.GetBaseXP(p->area_level)*exploration_percent/100*sWorld.getConfig(CONFIG_FLOAT_RATE_XP_EXPLORE)*xpMod);
                }
                else
                {
                    XP = uint32(sObjectMgr.GetBaseXP(p->area_level)*sWorld.getConfig(CONFIG_FLOAT_RATE_XP_EXPLORE)*xpMod);
                }
...
Team Player::TeamForRace(uint8 race)
{
    ...
    if (!rEntry)
    {
        ERROR_LOG("Race %u not found in DBC: wrong DBC files?",uint32(race));
        return ALLIANCE;
    }
    ...
    ERROR_LOG("Race %u have wrong teamid %u in DBC: wrong DBC files?",uint32(race),rEntry->TeamID);
    return TEAM_NONE;
}
...
void Player::RewardReputation(Unit *pVictim, float rate)
{
    ...
    if (Rep->championingAura && GetMap()->IsNonRaidDungeon() && HasAura(Rep->championingAura))
    {
...
bool Player::RewardHonor(Unit* uVictim, uint32 groupsize, float honor)
{
    ...
    if (uVictim != NULL)
    {
        ...
        honor *= IsPremiumActive() ? GetPremiumHonorModifier() : 1.0f;
...
void Player::_ApplyItemBonuses(ItemPrototype const *proto, uint8 slot, bool apply, bool only_level_scale /*= false*/)
{
    ...
    if (minDamage > 0 )
    {
        ...
        //ERROR_LOG("applying mindam: assigning %f to weapon mindamage, now is: %f", damage, GetWeaponDamageRange(attType, MINDAMAGE));
    }
    ...
    // Amaru: fix melee damage and attack speed for Druids
...
void Player::CastItemCombatSpell(Unit* Target, WeaponAttackType attType)
{
    ...
    for (int i = 0; i < MAX_ITEM_PROTO_SPELLS; ++i)
    {
        ...
        if(!spellInfo)
        {
            ERROR_LOG("WORLD: unknown Item spellid %i", spellData.SpellId);
            continue;
        }
    ...
    for(int e_slot = 0; e_slot < MAX_ENCHANTMENT_SLOT; ++e_slot)
    {
        ...
        for (int s = 0; s < 3; ++s)
        {
            ...
            if (!spellInfo)
            {
                ERROR_LOG("Player::CastItemCombatSpell Enchant %i, cast unknown spell %i", pEnchant->ID, proc_spell_id);
                continue;
            }
...
void Player::CastItemUseSpell(Item *item,SpellCastTargets const& targets,uint8 cast_count, uint32 glyphIndex)
{
    ...
    if (proto->Spells[0].SpellId==SPELL_ID_GENERIC_LEARN || proto->Spells[0].SpellId==SPELL_ID_GENERIC_LEARN_PET)
    {
        ...
        if (!spellInfo)
        {
            ERROR_LOG("Player::CastItemUseSpell: Item (Entry: %u) in have wrong spell id %u, ignoring ",proto->ItemId, learn_spell_id);
    ...
    for(int i = 0; i < MAX_ITEM_PROTO_SPELLS; ++i)
    {
        ...
        if (!spellInfo)
        {
            ERROR_LOG("Player::CastItemUseSpell: Item (Entry: %u) in have wrong spell id %u, ignoring",proto->ItemId, spellData.SpellId);
            continue;
        }
    ...
    //megai2: remove this if some items have Cast & enchant
    if (count > 0)
        return;
    ...
    for (int e_slot = 0; e_slot < MAX_ENCHANTMENT_SLOT; ++e_slot)
    {
        ...
        for (int s = 0; s < 3; ++s)
        {
            ...
            if (!spellInfo)
            {
                ERROR_LOG("Player::CastItemUseSpell Enchant %i, cast unknown spell %i", pEnchant->ID, pEnchant->spellid[s]);
                continue;
            }
...
void Player::SendLoot(ObjectGuid guid, LootType loot_type)
{
    ...
    switch(guid.GetHigh())
    {
        case HIGHGUID_GAMEOBJECT:
        {
            ...
            // the player whose group may loot the GO
            ...
            if (go->getLootState() == GO_READY && go->isSpawned())
            {
                ...
                if (!go->GetMap()->Instanceable())
                    go->SetCooldownTime(time(NULL) + go->GetGOInfo()->GetAutoCloseTime());
            }

            if (go->getLootState() == GO_ACTIVATED && go->GetGoType() == GAMEOBJECT_TYPE_CHEST && go->GetMap()->Instanceable())
            {
                if (go->GetGOInfo()->chest.groupLootRules == 1 || sWorld.getConfig(CONFIG_BOOL_LOOT_CHESTS_IGNORE_DB))
                {
                    if(Group* group = go->GetGroupLootRecipient())
            ...
            if (!item->HasGeneratedLoot())
            {
                ...
                switch(loot_type)
                {
                    ...
                    default:
                        ...
                        if (IsPremiumActive())
                            moneyMod = GetPremiumMoneyModifier();
                        loot->generateMoneyLoot(item->GetProto()->MinMoneyLoot * moneyMod, item->GetProto()->MaxMoneyLoot * moneyMod);                        
            ...
            if (!bones->lootForBody)
            {
                ...
                switch(GetZoneId())
                {
                    case 2597:  // Alterac Valley
                        loot->FillLoot(GetTeam() == ALLIANCE ? ALTERAC_VALLEY_INSIGNIA_LOOT_ENTRY_H : ALTERAC_VALLEY_INSIGNIA_LOOT_ENTRY_A, LootTemplates_Skinning, this, false);
                        break;
                    case 4197:  // Wintergrasp
                        loot->FillLoot(WINTERGRASP_INSIGNIA_LOOT_ENTRY, LootTemplates_Skinning, this, false);
                        break;
                }

                // It may need a better formula
                // Now it works like this: lvl10: ~6copper, lvl70: ~9silver
                float moneyMod = IsPremiumActive() ? GetPremiumMoneyModifier() : 1.0f;
                bones->loot.gold = (uint32)( urand(50, 150) * 0.016f * pow( ((float)pLevel)/5.76f, 2.5f) * sWorld.getConfig(CONFIG_FLOAT_RATE_DROP_MONEY) * moneyMod );
            }
        ...
        case HIGHGUID_VEHICLE:
        {
            ...
            if (loot_type == LOOT_PICKPOCKETING)
            {
                if (!creature->lootForPickPocketed)
                {
                    ...
                    float moneyMod = IsPremiumActive() ? GetPremiumMoneyModifier() : 1.0f;
                    loot->gold = uint32(10 * (a + b) * sWorld.getConfig(CONFIG_FLOAT_RATE_DROP_MONEY) * moneyMod);
                    permission = OWNER_PERMISSION;
                }
            }
            else
            {
                ...
                if (!creature->lootForBody)
                {
                    ...
                    loot->generateMoneyLoot(creature->GetCreatureInfo()->mingold * moneyMod,creature->GetCreatureInfo()->maxgold * moneyMod);
        ...
        default:
        {
            ERROR_LOG("%s is unsupported for looting.", guid.GetString().c_str());
            return;
        }
    }
        ...
        if (pProto)
        {
            ...
            /*switch (eslot)
            {
                case EQUIPMENT_SLOT_MAINHAND:
                    if (!CanUseEquippedWeapon(BASE_ATTACK))
                ...
                case EQUIPMENT_SLOT_RANGED:
                    if (!CanUseEquippedWeapon(RANGED_ATTACK))
                        return EQUIP_ERR_NOT_WHILE_DISARMED;
                    break;
                default:
                    break;
...
InventoryResult Player::CanUseItem( ItemPrototype const *pProto ) const
{
    ...
    if( pProto )
    {
        ...
        if (pProto->ItemId == 33060 && !((Player*)this)->IsPremiumActive())
            return EQUIP_ERR_CANT_DO_RIGHT_NOW;
...
Item* Player::StoreNewItem(ItemPosCountVec const& dest, uint32 item, bool update,int32 randomPropertyId , AllowedLooterSet* allowedLooters)
{
    ...
    if (pItem)
    {
        ...
        if (allowedLooters && allowedLooters->size() > 1 && pItem->GetProto()->GetMaxStackSize() == 1 && pItem->IsSoulBound())
        {
            pItem->SetSoulboundTradeable(allowedLooters, this, true);
            pItem->SetPlayedTimeField(GetTotalPlayedTime());
            AddSoulboundTradeableItem(pItem->GetObjectGuid());

            // save data
            std::ostringstream ss;
            ss << "REPLACE INTO `item_soulbound_trade_data` VALUES (" << pItem->GetGUIDLow() << ", '";
...
Item* Player::_StoreItem( uint16 pos, Item *pItem, uint32 count, bool clone, bool update )
{
    ...
    else
    {
        ...
        if (!clone)
        {
            ...
            pItem->SetRefundable(false);
            DeleteRefundReference(pItem->GetObjectGuid());
            RemoveSoulboundTradeableItem(pItem->GetObjectGuid());
...
Item* Player::EquipItem( uint16 pos, Item *pItem, bool update )
{
    ...
    UpdateAchievementCriteria(ACHIEVEMENT_CRITERIA_TYPE_EQUIP_EPIC_ITEM, slot+1);
...
void Player::SetVisibleItemSlot(uint8 slot, Item *pItem)
{
    ...
    // don not refresh when altVis is enabled for that slot
    if (m_vis)
    {
        switch (slot)
        {
            case EQUIPMENT_SLOT_HEAD:
                change = m_vis->m_visHead == 1;
                break;
            case EQUIPMENT_SLOT_SHOULDERS:
                change = m_vis->m_visShoulders == 1;
                break;
            case EQUIPMENT_SLOT_CHEST:
                change = m_vis->m_visChest == 1;
                break;
            case EQUIPMENT_SLOT_WAIST:
                change = m_vis->m_visWaist == 1;
                break;
            case EQUIPMENT_SLOT_LEGS:
                change = m_vis->m_visLegs == 1;
                break;
            case EQUIPMENT_SLOT_FEET:
                change = m_vis->m_visFeet == 1;
                break;
            case EQUIPMENT_SLOT_WRISTS:
                change = m_vis->m_visWrists == 1;
                break;
            case EQUIPMENT_SLOT_HANDS:
                change = m_vis->m_visHands == 1;
                break;
            case EQUIPMENT_SLOT_MAINHAND:
                change = m_vis->m_visMainhand == 1;
                break;
            case EQUIPMENT_SLOT_OFFHAND:
                change = m_vis->m_visOffhand == 1;
                break;
            case EQUIPMENT_SLOT_RANGED:
                change = m_vis->m_visRanged == 1;
                break;
        }
    }
...
void Player::MoveItemFromInventory(uint8 bag, uint8 slot, bool update)
{
    if(Item* it = GetItemByPos(bag,slot))
    {
        ...
        it->SetRefundable(false, false);
        DeleteRefundReference(it->GetObjectGuid());
...
void Player::MoveItemToInventory(ItemPosCountVec const& dest, Item* pItem, bool update, bool in_characterInventoryDB)
{
    ...
    if (pLastItem->HasFlag(ITEM_FIELD_FLAGS, ITEM_DYNFLAG_BOP_TRADEABLE))
        AddSoulboundTradeableItem(pLastItem->GetObjectGuid());
}
...
void Player::RemoveAllEnchantments(EnchantmentSlot slot, bool isArenaRemove)
{
    ...
    for(EnchantDurationList::iterator itr = m_enchantDuration.begin(), next; itr != m_enchantDuration.end(); itr = next)
    {
        ...
        if (itr->slot == slot)
        {
            if(isArenaRemove && itr->item) // Do not remove poisons at arenas
                if(uint32 enchant_id = itr->item->GetEnchantmentId(slot))
                {
                    SpellItemEnchantmentEntry const *pEnchant = sSpellItemEnchantmentStore.LookupEntry(enchant_id);
    ...
    for(int i = INVENTORY_SLOT_ITEM_START; i < INVENTORY_SLOT_ITEM_END; ++i)
        if (Item* pItem = GetItemByPos(INVENTORY_SLOT_BAG_0, i))
            if (uint32 enchant_id = pItem->GetEnchantmentId(slot))
            {
                if (!isArenaRemove)
            ...
            switch(enchant_display_type)
            {
                ...
                case ITEM_ENCHANTMENT_TYPE_EQUIP_SPELL:
                {
                    if (enchant_spell_id)
                    {
                        ...
                        if (enchant_spell_id == 43588 && !CanUseEquippedWeapon(Player::GetAttackBySlot(item->GetSlot())))
                            break;
                ...
                case ITEM_ENCHANTMENT_TYPE_PRISMATIC_SOCKET:
                    ...
                    break;
                default:
                    ERROR_LOG("Unknown item enchantment (id = %d) display type: %d", enchant_id, enchant_display_type);
                    break;
            }                                               /*switch(enchant_display_type)*/
        }                                                   /*for*/
    }
...
void Player::OnGossipSelect(WorldObject* pSource, uint32 gossipListId, uint32 menuId, uint32 entry)
{
    ...
    if (pSource->GetTypeId() == TYPEID_GAMEOBJECT)
    {
        ...
        {
            ERROR_LOG("Player guid %u request invalid gossip option for GameObject entry %u", GetGUIDLow(), pSource->GetEntry());
            return;
        }
    }
    ...
    switch (gossipOptionId)
    {
        ...
        case GOSSIP_OPTION_BATTLEFIELD:
        {
            ...
            {
                ERROR_LOG("a user (guid %u) requested battlegroundlist from a npc who is no battlemaster", GetGUIDLow());
                return;
            }
...
void Player::RewardQuest(Quest const *pQuest, uint32 reward, Object* questGiver, bool announce)
{
    ...
    float xpMod = IsPremiumActive() ? GetPremiumXPModifier() : 1.0f;
    uint32 xp = uint32(pQuest->XPValue(this) * sWorld.getConfig(CONFIG_FLOAT_RATE_XP_QUEST)*xpMod);
    if (getLevel() < sWorld.getConfig(CONFIG_UINT32_MAX_PLAYER_LEVEL))
    {
        GiveXP(xp , NULL);
        ...
        if (pQuest->GetRewOrReqMoney() > 0)
        {
            ...
            UpdateAchievementCriteria(ACHIEVEMENT_CRITERIA_TYPE_MONEY_FROM_QUEST_REWARD, pQuest->GetRewOrReqMoney());
        }
    }
    else
    {
        ...
        ModifyMoney(money);
        UpdateAchievementCriteria(ACHIEVEMENT_CRITERIA_TYPE_MONEY_FROM_QUEST_REWARD, money);
    }
    ...
    if (pQuest->IsDaily())
    {
        ...
        UpdateAchievementCriteria(ACHIEVEMENT_CRITERIA_TYPE_COMPLETE_DAILY_QUEST, 1);
    }
...
void Player::KilledMonsterCredit( uint32 entry, ObjectGuid guid )
{
    ...
    UpdateAchievementCriteria(ACHIEVEMENT_CRITERIA_TYPE_KILL_CREATURE, entry, addkillcount);
...
void Player::SendQuestReward(Quest const* pQuest, uint32 XP, Object* /*questGiver*/)
{
    ...
    float moneyMod = IsPremiumActive() && pQuest->GetRewOrReqMoney() > 0 ? GetPremiumMoneyModifier() : 1.0f;
    if ( getLevel() < sWorld.getConfig(CONFIG_UINT32_MAX_PLAYER_LEVEL) )
    {
        data << uint32(pQuest->GetRewOrReqMoney() * moneyMod);
        data << uint32(XP);
    }
    else
    {
...
void Player::_LoadArenaTeamInfo(QueryResult *result)
{
    ...
    do
    {
        ...
        if(!aTeam)
        {
            ERROR_LOG("Player::_LoadArenaTeamInfo: couldn't load arenateam %u", arenateamid);
            continue;
        }
...
bool Player::LoadFromDB(ObjectGuid guid, SqlQueryHolder *holder )
{
    ...
    //"resettalents_time, primary_trees, trans_x, trans_y, trans_z, trans_o, transguid, extra_flags, at_login, zone, online, death_expire_time, taxi_path, dungeon_difficulty,"
    // 39          40          41              42           43              44
    ...
    if(!result)
    {
        ERROR_LOG("%s not found in table `characters`, can't load. ", guid.GetString().c_str());
        return false;
    }
    ...
    {
        ERROR_LOG("%s loading from wrong account (is: %u, should be: %u)",
    ...
    if (ObjectMgr::CheckPlayerName(m_name) != CHAR_NAME_SUCCESS ||
        (GetSession()->GetSecurity() <= SEC_CURATOR && sObjectMgr.IsReservedName(m_name)))
    {
        delete result;
    ...
    SetUInt16Value(PLAYER_FIELD_KILLS, 0, fields[40].GetUInt16()); // today
    SetUInt16Value(PLAYER_FIELD_KILLS, 1, fields[41].GetUInt16()); // yesterday

    _LoadBoundInstances(holder->GetResult(PLAYER_LOGIN_QUERY_LOADBOUNDINSTANCES));

    if (!IsPositionValid())
    {
        ERROR_LOG("%s have invalid coordinates (X: %f Y: %f Z: %f O: %f). Teleport to default race/class locations.",
    ...
    if (transGUID != 0)
    {
        ...
        {
            ERROR_LOG("%s have invalid transport coordinates (X: %f Y: %f Z: %f O: %f). Teleport to default race/class locations.",
    ...
    if (transGUID != 0)
    {
        for (MapManager::TransportSet::const_iterator iter = sMapMgr.m_Transports.begin(); iter != sMapMgr.m_Transports.end(); ++iter)
        {
            ...
            if (transport->GetGUIDLow() == transGUID)
            {
                ...
                transport->AddPassenger(GetObjectGuid());
        ...
        if(!m_transport)
        {
            ERROR_LOG("%s have problems with transport guid (%u). Teleport to default race/class locations.",
    ...
    if (!player_at_bg && GetInstanceId() && !state)
    {
        ...
        if (at)
            ...
            ERROR_LOG("Player %s(GUID: %u) logged in to a reset instance (map: %u) and there is no area-trigger leading to this map. Thus he can't be ported back to the entrance. This _might_ be an exploit attempt.", GetName(), GetGUIDLow(), GetMapId());
    }
    ...
    else if (!m_taxi.LoadTaxiDestinationsFromString(taxi_nodes, GetTeam()))
    {
        ...
        {
            ERROR_LOG("Character %u have wrong data in taxi destination list, teleport to homebind.",GetGUIDLow());
            RelocateToHomebind();
        }
        else                                                // have start node, to it
        {
            ERROR_LOG("Character %u have too short taxi destination list, teleport to original node.",GetGUIDLow());
    ...
    // convert 335 fields
    if (uint32 oldArenaPoints = fields[59].GetUInt32())
    {
    ...
    if (uint32 oldHonorPoints = fields[60].GetUInt32())
    {
    ...
    if (extraflags & PLAYER_EXTRA_FORBID_SPECTATE)
        m_ExtraFlags |= PLAYER_EXTRA_FORBID_SPECTATE;

    if (extraflags & PLAYER_EXTRA_SPECTATOR)
    {
        extraflags &= ~PLAYER_EXTRA_SPECTATOR;
        SetSpectator(false);
    }
    ...
    if (sWorld.getConfig(CONFIG_BOOL_CHARACTER_ADDON_ENABLED))
    {
        QueryResult* result_addon = CharacterDatabase.PQuery("SELECT model, phase, scale, faction FROM characters_addon WHERE guid = '%u'", guid.GetCounter());
        if (result_addon)
        {
            Field* fields_addon = result_addon->Fetch();
            uint32 modelid = fields_addon[0].GetUInt32();
            uint32 phasemask = fields_addon[1].GetUInt32();
            float scale = fields_addon[2].GetFloat();
            uint32 faction = fields_addon[3].GetUInt32();
            ...
            if (phasemask > 0)
                SetPhaseMask(fields_addon[2].GetUInt32(), true);
            ...
            if (faction > 0 && sFactionStore.LookupEntry(faction))
                setFaction(faction);
    ...
    if (sGameEventMgr.IsActiveEvent(102))
    {
        ...
        if (GetTeam() == ALLIANCE)
        {
            SetNativeDisplayId(gender == GENDER_MALE ? 1098 : 1947);
            SetDisplayId(gender == GENDER_MALE ? 1098 : 1947);
        }
        else
        {
            SetNativeDisplayId(gender == GENDER_MALE ? 7061 : 16171);
            SetDisplayId(gender == GENDER_MALE ? 7061 : 16171);
        }
    }
    else if (sGameEventMgr.IsActiveEvent(103))
        InitDisplayIds(true);

    // DuKJIoHuyC for visuality items
    if (QueryResult* resultvis = CharacterDatabase.PQuery("SELECT head, shoulders, chest, waist, legs, feet, wrists, hands, back, main, off, ranged, type FROM characters_visuals WHERE guid = '%u'", guid.GetCounter()))
    {
        Field* fieldsvis = resultvis->Fetch();
        VisualsType type = VisualsType(fieldsvis[11].GetUInt32());
        if (type == VISUALS_TYPE_PREMIUM_ACC && IsPremiumActive() ||
            type == VISUALS_TYPE_MORPH ||
            sWorld.getConfig(CONFIG_BOOL_CHARACTER_VISUALS_OVERRIDE) ||
            GetSession()->GetSecurity() > SEC_CURATOR)
        {
            if (!m_vis)
                m_vis = new Visuals;
    ...
    if (GetSession()->GetSecurity() > SEC_PLAYER)
    {
        if (QueryResult* result = CharacterDatabase.PQuery("SELECT `playedTime` FROM `gm_played_time` WHERE `account` = %u", GetSession()->GetAccountId()))
        {
            m_Played_time[PLAYED_TIME_GM] = (*result)[0].GetUInt32();
            delete result;
        }
        else
            m_Played_time[PLAYED_TIME_GM] = 0;
...
void Player::_LoadCUFProfiles(QueryResult* result)
{
    ...
    do
    {
        // SELECT id, name, frameHeight, frameWidth, sortBy, healthText, boolOptions, unk146, unk147, unk148, unk150, unk152, unk154 FROM character_cuf_profiles WHERE guid = '%u'
        Field* fields = result->Fetch();
        ...
        {
            sLog.outError("Player::_LoadCUFProfiles - Player %s has an CUF profile with invalid id (id: %u), max is %i.", GetGuidStr().c_str(), id, MAX_CUF_PROFILES);
            continue;
        }
...
void Player::_LoadActions(QueryResult *result)
{
    ...
    if(result)
    {
        do
        {
            ...
            {
                ERROR_LOG( "  ...at loading, and will deleted in DB also");
...
void Player::_LoadGlyphs(QueryResult *result)
{
    ...
    do
    {
        ...
        if(!gp)
        {
            ERROR_LOG("Player %s has not existing glyph entry %u on index %u, spec %u", m_name.c_str(), glyph, slot, spec);
            continue;
        }
        ...
        if (!gs)
        {
            ERROR_LOG("Player %s has not existing glyph slot entry %u on index %u, spec %u", m_name.c_str(), GetGlyphSlot(slot), slot, spec);
            continue;
        }

        if(gp->TypeFlags != gs->TypeFlags)
        {
            ERROR_LOG("Player %s has glyph with typeflags %u in slot with typeflags %u, removing.", m_name.c_str(), gp->TypeFlags, gs->TypeFlags);
            continue;
        }
...
void Player::_LoadInventory(QueryResult *result, uint32 timediff)
{
    ...
    if (result)
    {
        ...
        do
        {
            ...
            {
                ERROR_LOG( "Player::_LoadInventory: Player %s has broken item (id: #%u) in inventory, deleted.", GetName(), item_id );
                CharacterDatabase.PExecute("DELETE FROM character_inventory WHERE item = '%u'", item_lowguid);
            ...
            {
                if (QueryResult* result = CharacterDatabase.PQuery("SELECT `paidMoney`, `paidExtendedCost` FROM `item_refund` WHERE `item_guid` = '%u' AND `player_guid` = '%u'",
                    item->GetObjectGuid().GetCounter(), item->GetOwnerGuid().GetCounter()))
                {
                    item->SetRefundable(true, false, (*result)[0].GetUInt32(), (*result)[1].GetUInt16());
                    AddRefundReference(item->GetObjectGuid());
                    delete result;
                }
                else
                {
                    DEBUG_LOG("Player::_LoadInventory: player (%s) has item (%s) with refundable flags, but without data in item_refund. Removing flag.",
                        GetGuidStr().c_str(), item->GetGuidStr().c_str(), item->GetEntry());
                    item->RemoveFlag(ITEM_FIELD_FLAGS, ITEM_DYNFLAG_REFUNDABLE);
                }
            }

            if (item->HasFlag(ITEM_FIELD_FLAGS, ITEM_DYNFLAG_BOP_TRADEABLE))
            {
                if (QueryResult* result = CharacterDatabase.PQuery("SELECT `allowedPlayers` FROM `item_soulbound_trade_data` WHERE itemGuid = '%u'", item->GetGUIDLow()))
                {
                    ...
                    AddSoulboundTradeableItem(item->GetObjectGuid());

                    delete result;
                }
                else
                {
                    ERROR_LOG("Item::LoadFromDB, Item GUID: %u has flag ITEM_FLAG_BOP_TRADEABLE but has no data in item_soulbound_trade_data, removing flag.", item->GetGUIDLow());
            ...
            {
                ERROR_LOG("Player::_LoadInventory: Player %s has item (GUID: %u Entry: %u) can't be loaded to inventory (Bag GUID: %u Slot: %u) by some reason, will send by mail.", GetName(),item_lowguid, item_id, bag_guid, slot);
                CharacterDatabase.PExecute("DELETE FROM character_inventory WHERE item = '%u'", item_lowguid);
    ...
    if (result)
    {
        do
        {
            ...
            if (!item)
            {
                ...
                ERROR_LOG("Player::_LoadItemLoot: Player %s has loot for nonexistent item (GUID: %u) in `item_loot`, deleted.", GetName(), item_guid );
                continue;
            }
...
void Player::_LoadMailedItems(QueryResult *result)
{
    // data needs to be at first place for Item::LoadFromDB
    //         0     1     2        3          4
    // "SELECT data, text, mail_id, item_guid, item_template FROM mail_items JOIN item_instance ii ON item_guid = ii.guid LEFT JOIN item_instance_text iit ON item_guid = iit.guid WHERE receiver = '%u'", GUID_LOPART(m_guid)
    if(!result)
        return;
    ...
    do
    {
        ...
        if(!proto)
        {
            ERROR_LOG( "Player %u has unknown item_template (ProtoType) in mailed items(GUID: %u template: %u) in mail (%u), deleted.", GetGUIDLow(), item_guid_low, item_template,mail->messageID);
            CharacterDatabase.PExecute("DELETE FROM mail_items WHERE item_guid = '%u'", item_guid_low);
            CharacterDatabase.PExecute("DELETE FROM item_instance WHERE guid = '%u'", item_guid_low);
            CharacterDatabase.PExecute("DELETE FROM item_instance_text WHERE guid = '%u'", item_guid_low);
            continue;
        }
        ...
        {
            ERROR_LOG( "Player::_LoadMailedItems - Item in mail (%u) doesn't exist !!!! - item guid: %u, deleted from mail", mail->messageID, item_guid_low);
            CharacterDatabase.PExecute("DELETE FROM mail_items WHERE item_guid = '%u'", item_guid_low);
            item->FSetState(ITEM_REMOVED);
            item->SaveToDB();                               // it also deletes item object !
            continue;
...
void Player::_LoadMails(QueryResult *result)
{
    ...
    do
    {
        ...
        {
            ERROR_LOG( "Player::_LoadMail - Mail (%u) have nonexistent MailTemplateId (%u), remove at load", m->messageID, m->mailTemplateId);
            m->mailTemplateId = 0;
        }
...
void Player::LoadPet()
{
    ...
    {
        Pet *pet = new Pet;
        if(!pet->LoadPetFromDB(this, 0, 0, PET_SAVE_FIRST_AVAILABLE_SLOT))
            delete pet;
    }
}
        ...
        do
        {
            ...
            if( pQuest )
            {
                ...
                else
                {
                    ...
                    ERROR_LOG("Player %s have invalid quest %d status (%d), replaced by QUEST_STATUS_NONE(0).",GetName(),quest_id,qstatus);
                }
...
void Player::_LoadTalents(QueryResult *result)
{
    ...
    if (result)
    {
        do
        {
            ...
            if (!talentInfo)
            {
                ERROR_LOG("Player::_LoadTalents:Player (GUID: %u) has invalid talent_id: %u , this talent will be deleted from character_talent",GetGUIDLow(), talent_id );
            ...
            if (!talentTabInfo)
            {
                ERROR_LOG("Player::_LoadTalents:Player (GUID: %u) has invalid talentTabInfo: %u for talentID: %u , this talent will be deleted from character_talent",GetGUIDLow(), talentInfo->TalentTab, talentInfo->TalentID );
                CharacterDatabase.PExecute("DELETE FROM character_talent WHERE talent_id = '%u'", talent_id);
                continue;
            }
            ...
            if ((getClassMask() & talentTabInfo->ClassMask) == 0)
            {
                ERROR_LOG("Player::_LoadTalents:Player (GUID: %u) has talent with ClassMask: %u , but Player's ClassMask is: %u , talentID: %u , this talent will be deleted from character_talent",GetGUIDLow(), talentTabInfo->ClassMask, getClassMask() ,talentInfo->TalentID );
            ...
            {
                ERROR_LOG("Player::_LoadTalents:Player (GUID: %u) has invalid talent rank: %u , talentID: %u , this talent will be deleted from character_talent",GetGUIDLow(), currentRank, talentInfo->TalentID );
                CharacterDatabase.PExecute("DELETE FROM character_talent WHERE guid = '%u' AND talent_id = '%u'", GetGUIDLow(), talent_id);
                continue;
            }
            ...
            {
                ERROR_LOG("Player::_LoadTalents:Player (GUID: %u) has invalid talent spec: %u, spec will be deleted from character_talent", GetGUIDLow(), spec);
            ...
            {
                ERROR_LOG("Player::_LoadTalents:Player (GUID: %u) has invalid talent spec: %u , this spec will be deleted from character_talent.", GetGUIDLow(), spec);
...
void Player::_LoadBoundInstances(QueryResult *result)
{
    ...
    if(result)
    {
        do
        {
            ...
            {
                ERROR_LOG("_LoadBoundInstances: player %s(%d) has bind to nonexistent or not dungeon map %d", GetName(), GetGUIDLow(), mapId);
            ...
            {
                ERROR_LOG("_LoadBoundInstances: player %s(%d) has bind to nonexistent difficulty %d instance for map %u", GetName(), GetGUIDLow(), difficulty, mapId);
                CharacterDatabase.PExecute("DELETE FROM character_instance WHERE guid = '%u' AND instance = '%u'", GetGUIDLow(), instanceId);
                continue;
            }
            ...
            if(!perm && group)
            {
                ERROR_LOG("_LoadBoundInstances: %s is in group (Id: %d) but has a non-permanent character bind to map %d,%d,%d",
...
void Player::SaveToDB()
{
    ...
    static SqlStatementID insStats ;

    SqlStatement stmt = CharacterDatabase.CreateStatement(delStats, "DELETE FROM armory_character_stats WHERE guid = ?");
    stmt.PExecute(GetGUIDLow());
    ...
    stmt.PExecute(GetGUIDLow(), ps.str().c_str());
    /** World of Warcraft Armory **/

    // DuKJIoHuyC: visuality items
    static SqlStatementID delVisuals ;
    static SqlStatementID insVisuals ;

    stmt = CharacterDatabase.CreateStatement(delVisuals, "DELETE FROM characters_visuals WHERE guid = ?");
    stmt.PExecute(GetGUIDLow());
    if (m_vis)
    {
        stmt = CharacterDatabase.CreateStatement(insVisuals, "INSERT INTO characters_visuals (guid, head, shoulders, chest, waist, legs, feet, wrists, hands, back, main, off, ranged, type) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");

        stmt.addUInt32(GetGUIDLow());
        stmt.addUInt32(m_vis->m_visHead);
        stmt.addUInt32(m_vis->m_visShoulders);
        stmt.addUInt32(m_vis->m_visChest);
        stmt.addUInt32(m_vis->m_visWaist);
        stmt.addUInt32(m_vis->m_visLegs);
        stmt.addUInt32(m_vis->m_visFeet);
        stmt.addUInt32(m_vis->m_visWrists);
        stmt.addUInt32(m_vis->m_visHands);
        stmt.addUInt32(m_vis->m_visBack);
        stmt.addUInt32(m_vis->m_visMainhand);
        stmt.addUInt32(m_vis->m_visOffhand);
        stmt.addUInt32(m_vis->m_visRanged);
        stmt.addUInt32(m_vis->m_type);

        stmt.Execute();
    }

    static SqlStatementID delGmPlayed;
    static SqlStatementID insGmPlayed;
    if (GetSession()->GetSecurity() > SEC_PLAYER)
    {
        stmt = CharacterDatabase.CreateStatement(delGmPlayed, "DELETE FROM `gm_played_time` WHERE `account` = ?");
        stmt.PExecute(GetSession()->GetAccountId());

        stmt = CharacterDatabase.CreateStatement(insGmPlayed, "INSERT INTO `gm_played_time` (`account`, `playedTime`) VALUE (?, ?)");
        stmt.PExecute(GetSession()->GetAccountId(), m_Played_time[PLAYED_TIME_GM]);
    }
    ...
    SqlStatement uberInsert = CharacterDatabase.CreateStatement(insChar, "INSERT INTO characters (guid,account,name,race,class,gender,level,xp,money,playerBytes,playerBytes2,playerFlags,"
        ...
        "power4, power5, specCount, activeSpec, exploredZones, equipmentCache, knownTitles, actionBars, slot, grantableLevels) "
        "VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, "
        "?, ?, ?, ?, ?, ?, "
        "?, ?, ?, "
        "?, ?, ?, ?, ?, ?, ?, ?, "
...
void Player::_SaveInventory()
{
    ...
    for (uint8 i = BUYBACK_SLOT_START; i < BUYBACK_SLOT_END; ++i)
    {
        ...
        CharacterDatabase.PExecute("DELETE FROM item_instance_text WHERE guid = '%u'", item->GetGUIDLow());
    ...
    for(size_t i = 0; i < m_itemUpdateQueue.size(); ++i)
    {
        ...
        if (test == NULL)
        {
            ERROR_LOG("Player(GUID: %u Name: %s)::_SaveInventory - the bag(%d) and slot(%d) values for the item with guid %d are incorrect, the player doesn't have an item at that position!", GetGUIDLow(), GetName(), item->GetBagSlot(), item->GetSlot(), item->GetGUIDLow());
            error = true;
            ...
            sWorld.BanAccount(BAN_CHARACTER_ACC, GetName(), TimeStringToSecs("-1"), "unknown exploit", "Anticheat (WAF)");
        }
        else if (test != item)
        {
            ERROR_LOG("Player(GUID: %u Name: %s)::_SaveInventory - the bag(%d) and slot(%d) values for the item with guid %d are incorrect, the item with guid %d is there instead!", GetGUIDLow(), GetName(), item->GetBagSlot(), item->GetSlot(), item->GetGUIDLow(), test->GetGUIDLow());
            error = true;
            ...
            if( alarm_count > 2)
                sWorld.BanAccount(BAN_CHARACTER_ACC, GetName(), TimeStringToSecs("-1"), "unknown exploit #2", "Anticheat (WAF)");
        }
    }

    if (error)
    {
        ERROR_LOG("Player::_SaveInventory - one or more errors occurred save aborted!");
...
void Player::_SaveCUFProfiles()
{
    ...
    for (uint8 i = 0; i < MAX_CUF_PROFILES; ++i)
    {
        ...
        if (_CUFProfiles[i])
        {
            ...
            stmt.addUInt16(_CUFProfiles[i]->FrameHeight);
            stmt.addUInt16(_CUFProfiles[i]->FrameWidth);
            stmt.addUInt8(_CUFProfiles[i]->SortBy);
            stmt.addUInt8(_CUFProfiles[i]->HealthText);
            stmt.addUInt32(_CUFProfiles[i]->BoolOptions.to_ulong());    // 27 of 32 fields used, fits in an int
            stmt.addUInt8(_CUFProfiles[i]->Unk146);
            stmt.addUInt8(_CUFProfiles[i]->Unk147);
            stmt.addUInt8(_CUFProfiles[i]->Unk148);
    ...
    for (PlayerMails::iterator itr = m_mail.begin(); itr != m_mail.end(); ++itr)
    {
        ...
        else if (m->state == MAIL_STATE_DELETED)
        {
            if (m->HasItems())
            {
                ...
                SqlStatement stmt2 = CharacterDatabase.CreateStatement(deleteItemText, "DELETE FROM item_instance_text WHERE guid = ?");
                for(MailItemInfoVec::const_iterator itr2 = m->items.begin(); itr2 != m->items.end(); ++itr2)
                {
                    stmt.PExecute(itr2->item_guid);
                    stmt2.PExecute(itr2->item_guid);
                }
            }
...
void Player::UpdateSpeakTime()
{
    ...
    if (GetSession()->GetAccountId() == sStatMgr.to_mute_acct)
        return;
...
void Player::TextEmote(const std::string& text)
{
    ...
    SendMessageToSetInRange(&data,sWorld.getConfig(CONFIG_FLOAT_LISTEN_RANGE_TEXTEMOTE),true, !sWorld.getConfig(CONFIG_BOOL_ALLOW_TWO_SIDE_INTERACTION_CHAT) && !IsPremiumActive() );
}
...
void Player::PossessSpellInitialize()
{
    ...
    if(!charmInfo)
    {
        ERROR_LOG("Player::PossessSpellInitialize(): charm (GUID: %u TypeId: %u) has no charminfo!", charm->GetGUIDLow(),charm->GetTypeId());
        return;
    }
...
void Player::CharmSpellInitialize()
{
    ...
    if(!charmInfo)
    {
        ERROR_LOG("Player::CharmSpellInitialize(): the player's charm (GUID: %u TypeId: %u) has no charminfo!", charm->GetGUIDLow(),charm->GetTypeId());
        return;
    }
...
void Player::SetRestBonus (float rest_bonus_new)
{
    ...
    if (GetAccountLinkedState() != STATE_NOT_LINKED)
        SetByteValue(PLAYER_BYTES_2, 3, REST_STATE_NORMAL | REST_STATE_RAF_LINKED); // Set Reststate = Refer-A-Friend
    else
    {
        if(m_rest_bonus>10)
...
void Player::HandleStealthedUnitsDetection()
{
    ...
    for (Spell::UnitList::const_iterator i = stealthedUnits.begin(); i != stealthedUnits.end(); ++i)
    {
        if((*i)==this)
            continue;
...
bool Player::ActivateTaxiPathTo(std::vector<uint32> const& nodes, Creature* npc /*= NULL*/, uint32 spellid /*= 0*/)
{
    ...
    if (npc)
    {
        ...
        // not let cheating with start flight in time of logout process || if casting not finished || while in combat || if not use Spell's with EffectSendTaxi
        if(IsNonMeleeSpellCasted(false) || HasFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_SILENCED) || HasFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_PACIFIED))
        {
            GetSession()->SendActivateTaxiReply(ERR_TAXIPLAYERBUSY);
            return false;
        }
...
void Player::SendModifyCooldown(uint32 spell_id, int32 delta)
{
    ...
    data << uint32(spell_id);
    data << GetObjectGuid();
    data << int32(result > 0 ? delta : -int32(cooldown) * IN_MILLISECONDS);
    GetSession()->SendPacket(&data);
}
...
void Player::InitDisplayIds(bool switchFaction)
{
    uint32 race = getRace();
    if (switchFaction)
    {
        switch(race)
        {
            case RACE_HUMAN: race = RACE_ORC; break;
            case RACE_ORC: race = RACE_HUMAN; break;
            case RACE_DWARF: race = RACE_TAUREN; break;
            case RACE_NIGHTELF: race = RACE_UNDEAD; break;
            case RACE_UNDEAD: race = RACE_NIGHTELF; break;
            case RACE_TAUREN: race = RACE_DWARF; break;
    ...
    if(!info)
    {
        ERROR_LOG("Player %u has incorrect race/class pair. Can't init display ids.", GetGUIDLow());
        return;
    }
    ...
    switch(gender)
    {
        ...
        case GENDER_MALE:
            ...
            break;
        default:
            ERROR_LOG("Invalid gender %u for player",gender);
            return;
    }
}
...
bool Player::BuyItemFromVendorSlot(ObjectGuid vendorGuid, uint32 vendorslot, uint32 item, uint8 count, uint8 bag, uint8 slot)
{
    ...
    if (pProto->IsEligibleForRefund() && crItem->ExtendedCost)
    {
...
void Player::UpdateHomebindTime(uint32 time)
{
    ...
    else
    {
        ...
        data << uint32(ERR_RAID_GROUP_NONE);                // error used only when timer = 0
        GetSession()->SendPacket(&data);
        DEBUG_LOG("PLAYER: Player '%s' (GUID: %u) will be teleported to homebind in %u seconds", GetName(),GetGUIDLow(), m_HomebindTimer/1000);
    }
}
    ...
    else
    {
        ...
        if(GetMap()->IsDungeon())
        {
            if (const WorldSafeLocsEntry* entry = sObjectMgr.GetClosestGraveYard(GetPositionX(), GetPositionY(), GetPositionZ(), GetMapId(), GetTeam()))
            {
                ...
                ERROR_LOG("SetBattleGroundEntryPoint: Dungeon map %u has no linked graveyard, setting home location as entry point.", GetMapId());
        }
...
{
    BattleGround *bg = GetBattleGround();
    if(!bg || bg != reporter->GetBattleGround() || bg->isArena() || GetTeam() != reporter->GetTeam())
        return;
    ...
    if(m_bgData.bgAfkReporter.find(reporter->GetGUIDLow()) == m_bgData.bgAfkReporter.end() && !HasAura(43680, EFFECT_INDEX_0) && !HasAura(43681, EFFECT_INDEX_0) && reporter->CanReportAfkDueToLimit())
    {
        ...
        if(m_bgData.bgAfkReporter.size() >= 2)
        {
            // cast 'Idle' spell
...
bool Player::IsVisibleGloballyFor( Player* u ) const
{
    ...
    if (u->GetSession()->GetSecurity() > SEC_CURATOR)
...
void Player::SetDailyQuestStatus( uint32 quest_id )
{
    ...
    if (!quest_daily_idx)
        UpdateAchievementCriteria(ACHIEVEMENT_CRITERIA_TYPE_COMPLETE_DAILY_QUEST_DAILY, 1);
}
...
void Player::RemoveItemDependentAurasAndCasts(Item * pItem)
{
    ...
    for(SpellAuraHolderMap::const_iterator itr = auras.begin(); itr != auras.end(); )
    {
        ...
        // Shadowmourne visual
        if ((holder->GetId() == 72521 || holder->GetId() == 72523) && pItem->GetEntry() == 49623)
        {
            RemoveAurasDueToSpell(holder->GetId());
            itr = auras.begin();
            continue;
    ...
    for(AuraList::const_iterator itr = dummyAuras.begin(); itr != dummyAuras.end(); ++itr)
    {
        ...
        if( prio < 2 && (*itr)->GetSpellProto()->SpellVisual[0] == 99 && (*itr)->GetSpellProto()->SpellIconID == 92 )
        {
            switch((*itr)->GetId())
            {
                ...
                default:
                    ERROR_LOG("Unhandled spell %u: S.Resurrection",(*itr)->GetId());
                    continue;
            }
...
void Player::RewardSinglePlayerAtKill(Unit* pVictim)
{
    ...
    UpdateAchievementCriteria(ACHIEVEMENT_CRITERIA_TYPE_GET_KILLING_BLOWS, 1, 0, pVictim);
    ...
    if(!PvP)
    {
        ...
        if(pVictim->GetTypeId()==TYPEID_UNIT)
            if(CreatureInfo const* normalInfo = sCreatureStorage.LookupEntry<CreatureInfo>(pVictim->GetEntry()))
            {
...
void Player::RemoveFromBattleRaid()
{
...
void Player::_LoadSkills(QueryResult *result)
{
    ...
    if (result)
    {
        do
        {
            ...
            if(!pSkill)
            {
                ERROR_LOG("Character %u has skill %u that does not exist.", GetGUIDLow(), skill);
                continue;
            }
            ...
            if(value == 0)
            {
                ERROR_LOG("Character %u has skill %u with value 0. Will be deleted.", GetGUIDLow(), skill);
            ...
            {
                ERROR_LOG("Character %u has more than %u skills.", GetGUIDLow(), PLAYER_MAX_SKILLS);
                break;
            }
        } while (result->NextRow());
        delete result;
...
void Player::HandleFall(MovementInfo const& movementInfo)
{
    ...
    if (z_diff >= 14.57f && !isDead() && !isGameMaster() &&
        ...
        !HasAuraType(SPELL_AURA_FLY) && !IsImmunedToDamage(SPELL_SCHOOL_MASK_NORMAL) && !hasFlyMountAura)
    {
...
{
    GetAchievementMgr().UpdateAchievementCriteria(type, miscvalue1, miscvalue2, unit, time, this);
    ...
    if (Guild* guild = sGuildMgr.GetGuildById(GetGuildId()))
        guild->GetAchievementMgr().UpdateAchievementCriteria(type, miscvalue1, miscvalue2, unit, time, this);
}
...
{
    GetAchievementMgr().CompletedAchievement(sAchievementStore.LookupEntry(uiAchievementID), this);
}
...
void Player::LearnPetTalent(ObjectGuid petGuid, uint32 talentId, uint32 talentRank)
{
    ...
    if( spellid == 0 )
    {
        ERROR_LOG("Talent.dbc have for talent: %u Rank: %u spell id = 0", talentId, talentRank);
        return;
    }
...
void Player::ResummonPetTemporaryUnSummonedIfAny()
{
    ...
    Pet* NewPet = new Pet;
    if(!NewPet->LoadPetFromDB(this, 0, m_temporaryUnsummonedPetNumber, PET_SAVE_FIRST_AVAILABLE_SLOT))
        delete NewPet;
...
void Player::ActivateSpec(uint8 specNum)
{
    ...
    for (PlayerTalentMap::const_iterator tempIter = tempSpec.begin(); tempIter != tempSpec.end(); ++tempIter)
    {
        ...
        {
            ERROR_LOG("ActivateSpec: Talent spell %u expected to learned at spec switch but not listed in talents at final check!", talentSpellId);
...
void Player::WriteWowArmoryDatabaseLog(uint32 type, uint32 data)
{
    ...
    DETAIL_LOG("WoWArmory: write feed log (guid: %u, type: %u, data: %u", pGuid, type, data);
    if (type <= 0)  // Unknown type
    {
        ERROR_LOG("WoWArmory: unknown type id: %d, ignore.", type);
        return;
    }
...
void Player::RemoveBuggedPrimarySkills()
{
    ...
    const uint32 proffarray[11] = {SKILL_ALCHEMY, SKILL_BLACKSMITHING, SKILL_ENCHANTING, SKILL_ENGINEERING, SKILL_HERBALISM, SKILL_INSCRIPTION, SKILL_JEWELCRAFTING, SKILL_LEATHERWORKING, SKILL_MINING, SKILL_SKINNING, SKILL_TAILORING};
    ...
    if( numproff < (sWorld.getConfig(CONFIG_UINT32_MAX_PRIMARY_TRADE_SKILL) + 1 ) )
        return;
    ...
    SetSkill(skilltoremove, 0, 0, 0);
}

bool Player::HandleChangeSlotModel(uint16 visSlot, uint32 newItem, uint16 pos)
{
    if (newItem == 0 && visSlot != PLAYER_VISIBLE_ITEM_18_ENTRYID)  // remove model display from slot
                                                                    // do not allow remove display from ranged, client does not have animation in this case.
    {
        SetUInt32Value(visSlot, 0);
        return true;
    }
    else if (newItem == 1)  // restore original item display even with 'display on'.
                            // actually, it will not refresh, when real item is changed, must be redone.
    {
    ...
    else                    // neither 0 nor 1, hence item id. Search, check...
    {
        ItemPrototype const *itemProto = sItemStorage.LookupEntry<ItemPrototype>(newItem);
        if (!itemProto)
            return false;
        if (itemProto->Quality != ITEM_QUALITY_LEGENDARY)   // common check for items.
                                                            // exceptions like 'corrupted ashbringer' better be implemented in commands
        {
            bool condition = false;
            switch (visSlot)
            {
                case PLAYER_VISIBLE_ITEM_1_ENTRYID:        // head - thanks, Cap!
                {
                ...
                {
                    if (itemProto->InventoryType == INVTYPE_RANGED || itemProto->InventoryType == INVTYPE_RANGEDRIGHT || itemProto->InventoryType == INVTYPE_THROWN)
                        condition = true;
                    break;
                }
                default:
            ...
            if (condition)                              // all is ok, change model
            {
...
{
    if (!m_vis)     // display off, restore display of all items on character
    {
        for (int i = EQUIPMENT_SLOT_START; i < EQUIPMENT_SLOT_END; ++i)
        {
    ...
    {
        // do not straight put saved values, check them just in case
        // however this shit looks awful. need to rewrite...
        // DuKJIoHuyC: Rewrote.
...
{
    const char* format = sObjectMgr.GetMangosString(LANG_VEHICLE_ZONE_EXPIRE, GetSession()->GetSessionDbLocaleIndex());
    char str[1024];
    sprintf(str, format, GetAreaNameById(areaid, GetSession()->GetSessionDbcLocale()).c_str(), uint32(timer / IN_MILLISECONDS));
...
bool Player::DkQuestsCompleted()
{
    ...
    //uint32 questId = 0;
    //if (GetTeam() == ALLIANCE)
    //    questId = 13188;            // Where Kings Walk
    //else
    //    questId = 13189;            // Return to the Warchief

    //return GetQuestStatus(questId) == QUEST_STATUS_COMPLETE && GetQuestRewardStatus(questId);
    return true;
}
...
void Player::InstallPremiumFeatures(bool apply)
{
    CreateOrDeletePremiumItem(apply);
    ApplyPremiumSpells(apply);
}

void Player::CreateOrDeletePremiumItem(bool create)
{
    uint32 entry = 33060;
    ItemPrototype const* pProto = ObjectMgr::GetItemPrototype(entry);
    if (!pProto)
    ...
    if (create)
    {
        if (HasItemCount(entry, 1, true))
            return;

        // can the player store the new item?
        ItemPosCountVec dest;
        uint8 msg = CanStoreNewItem(NULL_BAG, NULL_SLOT, dest, entry, 1);
        if (msg != EQUIP_ERR_OK)
            return;

        // create the new item and store it
        Item* pItem = StoreNewItem(dest, entry, true);
        ...
        if(pItem)
            SendNewItem(pItem, 1, true, true);
    }
    else
    {
        uint32 count = GetItemCount(entry, true);
        if (!count)
            return;

        DestroyItemCount(entry, count, true);
    }
}

void Player::ApplyPremiumSpells(bool apply)
{
    if (apply)
    {
        if (!HasSpell(75973))
            learnSpell(75973, false);
    }
    else
    {
        if (HasSpell(75973))
...
float Player::GetPremiumXPModifier()
{
    return GetPremiumRateType() == PREMIUM_RATE_XP ? sWorld.getConfig(CONFIG_FLOAT_PREMIUM_MODIFIER_RATE_XP) : 1.0f;
}

float Player::GetPremiumHonorModifier()
{
    return GetPremiumRateType() == PREMIUM_RATE_HONOR ? sWorld.getConfig(CONFIG_FLOAT_PREMIUM_MODIFIER_RATE_HONOR) : 1.0f;
}

float Player::GetPremiumMoneyModifier()
{
    return GetPremiumRateType() == PREMIUM_RATE_MONEY ? sWorld.getConfig(CONFIG_FLOAT_PREMIUM_MODIFIER_RATE_DROP_MONEY) : 1.0f;
}
...
{
    WriteGuard guard(i_tradeableItemslock);
    ...
    for (GuidSet::iterator itr = m_itemSoulboundTradeable.begin(); itr != m_itemSoulboundTradeable.end();)
    {
        ...
        if (!item)
        {
            ...
            itr = m_itemSoulboundTradeable.begin();
            continue;
        }

        if (item->UpdateSoulboundPlayedTime())
        {
            m_itemSoulboundTradeable.erase(itr);
            itr = m_itemSoulboundTradeable.begin();
            continue;
...
void Player::RemoveSoulboundTradeableItem(ObjectGuid itemGuid)
{
    WriteGuard guard(i_tradeableItemslock);
    GuidSet::iterator itr = m_itemSoulboundTradeable.find(itemGuid);
    if (itr != m_itemSoulboundTradeable.end())
        m_itemSoulboundTradeable.erase(itr);
}

void Player::AddSoulboundTradeableItem(ObjectGuid itemGuid)
{
    WriteGuard guard(i_tradeableItemslock);
    GuidSet::iterator itr = m_itemSoulboundTradeable.find(itemGuid);
    if (itr == m_itemSoulboundTradeable.end())
        m_itemSoulboundTradeable.insert(itemGuid);
}

void Player::UpdateRefundableItems()
{
    if (m_refundableItems.empty())
        return;

    DEBUG_LOG("Called UpdateRefundableItems for %s", GetGuidStr().c_str());

    for (GuidSet::iterator itr = m_refundableItems.begin(); itr != m_refundableItems.end();)
    {
        Item* pItem = GetItemByGuid(*itr);
        if (!pItem)
        {
            ERROR_LOG("Can't find item %s but is in refundable storage for player %s ! Removing.", itr->GetString().c_str(), GetGuidStr().c_str());
            m_refundableItems.erase(itr);
            itr = m_refundableItems.begin();
            continue;
        }

        if (pItem->UpdateRefundablePlayedTime())
        {
            m_refundableItems.erase(itr);
            itr = m_refundableItems.begin();
            continue;
...
void Player::AddRefundReference(ObjectGuid guid)
{
    m_refundableItems.insert(guid);
}

void Player::DeleteRefundReference(ObjectGuid guid)
{
    GuidSet::iterator itr = m_refundableItems.find(guid);
...
void Player::RefundItem(Item* item)
{
    ...
    for (uint8 i = 0; i < MAX_EXTENDED_COST_CURRENCIES; ++i)
    {
        ...
        ModifyCurrencyCount(curr, int32(iece->reqcurrcount[i]), false, false, true);
    }
...
{
    m_kickDelay = delay;
    if (GetSession() && CanSpeak())
        GetSession()->m_muteTime = time(NULL) + delay;
}
...
void Player::SetSpectator(bool on)
{
    if (on)
    {
        m_ExtraFlags |= PLAYER_EXTRA_SPECTATOR;
        CastSpell(this, 39870, true);
    }
    else
    {
        RemoveAurasDueToSpell(39870);
        m_ExtraFlags &= ~PLAYER_EXTRA_SPECTATOR;
    }
}

bool Player::Spectate(Player* target, bool sendErrors)
{
    if (IsSpectator())
    {
        if (sendErrors)
            ChatHandler(this).SendSysMessage(LANG_ALREADY_SPECTATING);
        return false;
    }

    if (!target || !target->IsInWorld() || target->GetObjectGuid() == GetObjectGuid())
    {
        if (sendErrors)
            ChatHandler(this).SendSysMessage(LANG_NO_CHAR_SELECTED);
        return false;
    }
    ...
    {
        if (sendErrors)
            ChatHandler(this).SendSysMessage(LANG_CANT_SPECTATE_SPECTATOR);
        return false;
    }*/

    // TODO:
    // if any player in bg forbids spectating - forbid for everyone?
    if (!target->AllowsSpectating())
    {
        if (sendErrors)
            ChatHandler(this).PSendSysMessage(LANG_SPECTATING_FORBIDDEN_PLAYER, target->GetName());
        return false;
    }
    ...
    {
        if (sendErrors)
            ChatHandler(this).SendSysMessage(LANG_CANT_SPECTATE_HERE);
        return false;
    }

    if (!isAlive() || hasUnitState(UNIT_STAT_DIED))
    {
        if (sendErrors)
            ChatHandler(this).SendSysMessage(LANG_CANT_SPECTATE_WHILE_DEAD);
        return false;
    }

    BattleGround* bg = target->GetBattleGround();
    if (!bg || !bg->isArena() || bg->GetStatus() == STATUS_WAIT_LEAVE || target->IsBeingTeleported())
    {
        if (sendErrors)
            ChatHandler(this).PSendSysMessage(LANG_TARGET_NOT_IN_ARENA, target->GetName());
        return false;
    }

    SetSpectator(true);
    SetBattleGroundId(target->GetBattleGroundId(), target->GetBattleGroundTypeId());
    SetBattleGroundEntryPoint();

    UnsummonPetTemporaryIfAny();

    switch(target->GetMapId())
    {
        case 559:       // Nagrand Arena
            TeleportTo(target->GetMapId(), 4056.02f, 2922.96f, 13.82f, GetAngle(target));
            break;
        case 562:       // Blade's Edge Arena
            TeleportTo(target->GetMapId(), 6202.45f, 252.57f, 3.56f, GetAngle(target));
            break;
        case 572:       // Ruins of Lordaeron
            TeleportTo(target->GetMapId(), 1286.02f, 1668.18f, 39.38f, GetAngle(target));
            break;
        case 617:       // Dalaran Sewers
            TeleportTo(target->GetMapId(), 1284.25f, 779.97f, 7.21f, GetAngle(target));
            break;
        case 618:       // Ring of Valor
            TeleportTo(target->GetMapId(), 761.87f, -284.19f, 28.30f, GetAngle(target));
            break;
    }
...
bool Player::UnSpectate(bool sendErrors)
{
    if (!IsSpectator())
    {
        if (sendErrors)
            ChatHandler(this).SendSysMessage(LANG_NOT_SPECTATING);
        return false;
    }

    ScheduleDelayedOperation(DELAYED_REMOVE_SPECTATOR);
    TeleportToBGEntryPoint();

    return true;
}

bool Player::ToggleAllowSpectate()
{
    if (m_ExtraFlags & PLAYER_EXTRA_FORBID_SPECTATE)
        m_ExtraFlags &= ~PLAYER_EXTRA_FORBID_SPECTATE;
    else
        m_ExtraFlags |= PLAYER_EXTRA_FORBID_SPECTATE;

    return (m_ExtraFlags & PLAYER_EXTRA_FORBID_SPECTATE) == 0;
}
