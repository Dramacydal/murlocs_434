...
bool Player::BuildEnumData( QueryResult * result, WorldPacket * p_data )
{
    ...
    if(!info)
    {
        ERROR_LOG("Player %u has incorrect race/class pair. Don't build enum.", guid);
        return false;
    }
    ...
    *p_data << uint32(fields[13].GetUInt32());              // guild id
    ...
    QueryResult *banresult = CharacterDatabase.PQuery("SELECT bandate,unbandate FROM character_banned WHERE "
        "guid = %u AND active = 1", guid);
    if (banresult)
    {
