...
uint32 Player::GetShieldBlockValue(bool capped) const
{
...
void Player::SetSkill(uint16 id, uint16 currVal, uint16 maxVal, uint16 step /*=0*/)
{
    ...
    uint32 oldSkillVal = GetSkillValue(id);
...
void Player::RewardReputation(Unit *pVictim, float rate)
{
    ...
    if (Rep->championingAura && HasAura(Rep->championingAura))
    {
...
void Player::SendQuestReward( Quest const *pQuest, uint32 XP, Object * questGiver )
{
    ...
    {
        data << uint32(0);
        data << uint32(pQuest->GetRewOrReqMoney() + int32(pQuest->GetRewMoneyMaxLevel() * sWorld.getConfig(CONFIG_FLOAT_RATE_DROP_MONEY) * moneyMod));
    }
...
bool Player::LoadFromDB(ObjectGuid guid, SqlQueryHolder *holder )
{
    ...
    //"health, power1, power2, power3, power4, power5, specCount, activeSpec, exploredZones, equipmentCache, knownTitles, actionBars, grantableLevels FROM characters WHERE guid = '%u'", GUID_LOPART(m_guid));
    ...
    m_GrantableLevelsCount = fields[62].GetUInt32();
...
void Player::SaveToDB()
{
    ...
    SqlStatement uberInsert = CharacterDatabase.CreateStatement(insChar, "INSERT INTO characters (guid,account,name,race,class,gender,level,xp,money,playerBytes,playerBytes2,playerFlags,"
        ...
        "power4, power5, power6, power7, power8, power9, power10, specCount, activeSpec, exploredZones, equipmentCache, knownTitles, actionBars, grantableLevels) "
        "VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, "
        "?, ?, ?, ?, ?, ?, "
        "?, ?, ?, "
        "?, ?, ?, ?, ?, ?, ?, "
...
void Player::BuildPlayerTalentsInfoData(WorldPacket *data)
{
    ...
    *data << uint8(m_activeSpec);                           // talent group index (0 or 1)
    ...
    if(m_specsCount)
    {
        ...
        //Ranger: Fix a crash in BuildPlayerTalentsInfoData
...
void Player::SetEquipmentSet(uint32 index, EquipmentSet eqset)
{
    if(eqset.Guid != 0)
    {
        ...
        {
            ERROR_LOG("Player %s tried to save equipment set "UI64FMTD" (index %u), but that equipment set not found!", GetName(), eqset.Guid, index);
            return;
        }
    }
