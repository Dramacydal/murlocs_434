...
void MotionMaster::MoveTargetedHome()
{
    ...
    else if (m_owner->GetTypeId() == TYPEID_UNIT && ((Creature*)m_owner)->GetCharmerOrOwnerGuid())
    {
        ...
        ERROR_LOG("%s attempt targeted home", m_owner->GetGuidStr().c_str());
}
...
void MotionMaster::MoveFollow(Unit* target, float dist, float angle)
{
    ...
    if(m_owner->GetFollowAngle())
      angle = m_owner->GetFollowAngle();
...
void MotionMaster::MoveSeekAssistance(float x, float y, float z)
{
    if (m_owner->GetTypeId() == TYPEID_PLAYER)
    {
        ERROR_LOG("%s attempt to seek assistance", m_owner->GetGuidStr().c_str());
    }
    else
    {
...
void MotionMaster::MoveSeekAssistanceDistract(uint32 time)
{
    if (m_owner->GetTypeId() == TYPEID_PLAYER)
    {
        ERROR_LOG("%s attempt to call distract after assistance", m_owner->GetGuidStr().c_str());
    }
    else
    {
...
void MotionMaster::MoveTaxiFlight(uint32 path, uint32 pathnode)
{
    if (m_owner->GetTypeId() == TYPEID_PLAYER)
    {
        ...
        {
            ERROR_LOG("%s attempt taxi to (nonexistent Path %u node %u)",
                m_owner->GetGuidStr().c_str(), path, pathnode);
        }
    }
    else
    {
        ERROR_LOG("%s attempt taxi to (Path %u node %u)",
            m_owner->GetGuidStr().c_str(), path, pathnode);
    }
}
