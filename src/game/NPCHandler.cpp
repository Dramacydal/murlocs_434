...
void WorldSession::HandleGossipSelectOptionOpcode( WorldPacket & recv_data )
{
    ...
    if (guid.IsAnyTypeCreature())
    {
        ...
        if (!sScriptMgr.OnGossipSelect(_player, pCreature, sender, action, code.empty() ? NULL : code.c_str()))
            _player->OnGossipSelect(pCreature, gossipListId, menuId, pCreature->GetEntry());
    }
    else if (guid.IsGameObject())
    {
        ...
        if (!sScriptMgr.OnGossipSelect(_player, pGo, sender, action, code.empty() ? NULL : code.c_str()))
            _player->OnGossipSelect(pGo, gossipListId, menuId, pGo->GetEntry());
    }
}
...
void WorldSession::SendStablePet( ObjectGuid guid )
{
    ...
    if(result)
    {
        do
        {
            ...
            data << uint8(fields[5].GetUInt32() < PET_SAVE_FIRST_STABLE_SLOT ? 1 : 2);  // 1 = current, 2/3 = in stable (any from 4,5,... create problems with proper show)
