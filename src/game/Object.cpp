...
Object::~Object( )
{
    if(IsInWorld())
    {
        ...
        ERROR_LOG("Object::~Object (GUID: %u TypeId: %u) deleted but still in world!!", GetGUIDLow(), GetTypeId());
        MANGOS_ASSERT(false);
    }

    if(m_objectUpdated)
    {
        ERROR_LOG("Object::~Object (GUID: %u TypeId: %u) deleted but still have updated status!!", GetGUIDLow(), GetTypeId());
        MANGOS_ASSERT(false);
    }
...
void Object::DestroyForPlayer( Player *target, bool anim ) const
{
    ...
    if (isType(TYPEMASK_UNIT))
    {
        ...
        if (unit->GetTypeId() == TYPEID_PLAYER)
        {
            ...
            if (player->IsInWorld() && player->GetSession() && !player->GetSession()->isLogingOut())
            {
                if (BattleGround* bg = player->GetBattleGround())
                {
                    if (bg->isArena() && bg->GetStatus() != STATUS_WAIT_LEAVE)
                        if (player->GetBGTeam() == target->GetBGTeam())
                            return;
                }
            }
        }
                ...
                else if (index == UNIT_DYNAMIC_FLAGS)
                {
                    ...
                    if (GetTypeId() == TYPEID_UNIT)
                    {
                        if (!target->isAllowedToLoot((Creature*)this))
                            val &= ~(UNIT_DYNFLAG_LOOTABLE | UNIT_DYNFLAG_TAPPED_BY_PLAYER);
                        // flag only for original loot recipent
                        else if (target->GetObjectGuid() != ((Creature*)this)->GetLootRecipientGuid())
                            val &= ~(UNIT_DYNFLAG_TAPPED | UNIT_DYNFLAG_TAPPED_BY_PLAYER);
                    }

                    // hide raf flag for non-linked players
                    if (GetTypeId() == TYPEID_PLAYER && !((Player*)this)->IsReferAFriendLinked(target))
                        val &= ~UNIT_DYNFLAG_REFER_A_FRIEND;
...
void Object::SetByteValue( uint16 index, uint8 offset, uint8 value )
{
    ...
    if(offset > 4)
    {
        ERROR_LOG("Object::SetByteValue: wrong offset %u", offset);
        return;
    }
...
void Object::SetUInt16Value( uint16 index, uint8 offset, uint16 value )
{
    ...
    if(offset > 2)
    {
        ERROR_LOG("Object::SetUInt16Value: wrong offset %u", offset);
        return;
    }
...
void Object::SetByteFlag( uint16 index, uint8 offset, uint8 newFlag )
{
    ...
    if(offset > 4)
    {
        ERROR_LOG("Object::SetByteFlag: wrong offset %u", offset);
        return;
    }
...
void Object::RemoveByteFlag( uint16 index, uint8 offset, uint8 oldFlag )
{
    ...
    if(offset > 4)
    {
        ERROR_LOG("Object::RemoveByteFlag: wrong offset %u", offset);
        return;
    }
...
{
    ERROR_LOG("Attempt %s nonexistent value field: %u (count: %u) for object typeid: %u type mask: %u",(set ? "set value to" : "get value from"),index,m_valuesCount,GetTypeId(),m_objectType);
...
{
    ERROR_LOG("Unexpected call of Object::AddToClientUpdateList for object (TypeId: %u Update fields: %u)",GetTypeId(), m_valuesCount);
    MANGOS_ASSERT(false);
}

void Object::RemoveFromClientUpdateList()
{
    ERROR_LOG("Unexpected call of Object::RemoveFromClientUpdateList for object (TypeId: %u Update fields: %u)",GetTypeId(), m_valuesCount);
    MANGOS_ASSERT(false);
}

void Object::BuildUpdateData( UpdateDataMapType& /*update_players */)
{
    ERROR_LOG("Unexpected call of Object::BuildUpdateData for object (TypeId: %u Update fields: %u)",GetTypeId(), m_valuesCount);
    MANGOS_ASSERT(false);
}
...
{
    //megai2: how we can be out of ourself in this world?
    if (obj == this)
        return true;
    ...
    if (isType(TYPEMASK_CREATURE_OR_PLAYER) && obj->isType(TYPEMASK_CREATURE_OR_PLAYER))
    {
        if (((Unit*)this)->GetTransport() && ((Unit*)obj)->GetTransport() && ((Unit*)this)->GetTransport()->GetObjectGuid() == ((Unit*)obj)->GetTransport()->GetObjectGuid())
        {
            float dtx = ((Unit*)this)->m_movementInfo.GetTransportPos()->x - ((Unit*)obj)->m_movementInfo.GetTransportPos()->x;
            float dty = ((Unit*)this)->m_movementInfo.GetTransportPos()->y - ((Unit*)obj)->m_movementInfo.GetTransportPos()->y;
            float disttsq = dtx * dtx + dty * dty;
            if (is3D)
            {
                float dtz = ((Unit*)this)->m_movementInfo.GetTransportPos()->z - ((Unit*)obj)->m_movementInfo.GetTransportPos()->z;
                disttsq += dtz * dtz;
            }
...
void WorldObject::BuildMonsterChat(WorldPacket *data, ObjectGuid senderGuid, uint8 msgtype, char const* text, uint32 language, char const* name, ObjectGuid targetGuid, char const* targetName)
{
    ...
    *data << text;
    *data << uint8(CHAT_TAG_NONE);                          // ChatTag
...
void WorldObject::SendGameObjectCustomAnim(ObjectGuid guid, uint32 animId)
{
...
Creature* WorldObject::GetClosestCreature(uint32 uiEntry, float fMaxSearchRange, bool isAlive /*= true*/)
{
    ...
    MaNGOS::NearestCreatureEntryInObjectRangeCheck creature_check(*this, uiEntry, fMaxSearchRange, isAlive);
    MaNGOS::CreatureLastSearcher<MaNGOS::NearestCreatureEntryInObjectRangeCheck> searcher(tempCreature, creature_check);

    Cell::VisitGridObjects(this, searcher, fMaxSearchRange);

    return tempCreature;
}

// return closest GO in grid, with range from pSource
GameObject* WorldObject::GetClosestGameObject(uint32 uiEntry, float fMaxSearchRange)
{
    GameObject* tempGo = NULL;

    MaNGOS::NearestGameObjectEntryInObjectRangeCheck go_check(*this, uiEntry, fMaxSearchRange);
    MaNGOS::GameObjectLastSearcher<MaNGOS::NearestGameObjectEntryInObjectRangeCheck> searcher(tempGo, go_check);
...
void WorldObject::GetGameObjectListWithEntryInGrid(std::list<GameObject*>& gameobjectList, uint32 uiEntry, float fMaxSearchRange) const
{
    CellPair pair(MaNGOS::ComputeCellPair(this->GetPositionX(), this->GetPositionY()));
    Cell cell(pair);
    cell.SetNoCreate();

    MaNGOS::AllGameObjectOfEntryInRangeCheck check(this, uiEntry, fMaxSearchRange);
    MaNGOS::GameObjectListSearcher<MaNGOS::AllGameObjectOfEntryInRangeCheck> searcher(gameobjectList, check);
    TypeContainerVisitor<MaNGOS::GameObjectListSearcher<MaNGOS::AllGameObjectOfEntryInRangeCheck>, GridTypeMapContainer> visitor(searcher);

    cell.Visit(pair, visitor, *(this->GetMap()), *this, fMaxSearchRange);
}

void WorldObject::GetCreatureListWithEntryInGrid(std::list<Creature*>& creatureList, uint32 uiEntry, float fMaxSearchRange) const
{
    CellPair pair(MaNGOS::ComputeCellPair(this->GetPositionX(), this->GetPositionY()));
    Cell cell(pair);
    cell.SetNoCreate();

    MaNGOS::AllCreaturesOfEntryInRangeCheck check(this, uiEntry, fMaxSearchRange);
    MaNGOS::CreatureListSearcher<MaNGOS::AllCreaturesOfEntryInRangeCheck> searcher(creatureList, check);
    TypeContainerVisitor<MaNGOS::CreatureListSearcher<MaNGOS::AllCreaturesOfEntryInRangeCheck>, GridTypeMapContainer> visitor(searcher);
