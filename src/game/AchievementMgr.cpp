    ...
    if (criteriaResult)
    {
        do
        {
            ...
            if (!criteria)
            {
                ...
                ERROR_LOG("Nonexistent achievement criteria %u data removed from table `character_achievement_progress`.",id);
...
void AchievementMgr<Guild>::LoadFromDB(QueryResult *achievementResult, QueryResult *criteriaResult)
{
    if (achievementResult)
    {
        do
        {
            ...
            Tokens tokens = StrSplit(fields[2].GetCppString(), " ");
            for (uint32 i = 0; i < tokens.size(); ++i)
                ca.guids.insert(ObjectGuid(HIGHGUID_PLAYER, 0, uint32(atol(tokens[i].c_str()))));
    ...
    if (criteriaResult)
    {
        ...
        do
        {
            ...
            if (!criteria)
            {
                ...
                ERROR_LOG("Non-existing achievement criteria %u data removed from table `guild_achievement_progress`.", id);
...
void AchievementMgr<T>::SendAchievementEarned(AchievementEntry const* achievement)
{
    ...
    GetOwner()->WriteWowArmoryDatabaseLog(1, achievement->ID);
    /** World of Warcraft Armory **/
}
...
void AchievementMgr<Guild>::CheckAllAchievementCriteria(Player* referencePlayer)
{
    // suppress sending packets
    for (uint32 i = 0; i < ACHIEVEMENT_CRITERIA_TYPE_TOTAL; ++i)
        UpdateAchievementCriteria(AchievementCriteriaTypes(i), 0, 0, NULL, 0);
}
...
void AchievementMgr<Player>::StartTimedAchievementCriteria(AchievementCriteriaTypes type, uint32 timedRequirementId, time_t startTime /*= 0*/)
{
    ...
    if (!sWorld.getConfig(CONFIG_BOOL_GM_ALLOW_ACHIEVEMENT_GAINS) && GetOwner()->GetSession()->GetSecurity() > SEC_CURATOR)
        return;
        ...
        switch (type)
        {
            ...
            case ACHIEVEMENT_CRITERIA_TYPE_KILL_CREATURE:
            {
                ...
                switch(achievementCriteria->referredAchievement)
                {
                    ...
                    {
                        VehicleKit* vehicleKit = referencePlayer->GetVehicle();
                        if(!vehicleKit)
                            continue;
            ...
            case ACHIEVEMENT_CRITERIA_TYPE_BE_SPELL_TARGET2:
            {
                ...
                {
                    case 7636:      // Defense of the Ancients
                    case 7740:
                    {
                        ...
                        if (bgsa->GetDefender() != plr->GetTeam())
                            continue;

                        bool ok = true;
                        for (int i = 1; i < BG_SA_GATE_COUNT; ++i)
                            if (((BattleGroundSA*)bg)->GetGateStatus(i) == BG_SA_GATE_STATUS_DESTROYED)
                            {
                                ok = false;
                                break;
                            }
            ...
            case ACHIEVEMENT_CRITERIA_TYPE_GET_KILLING_BLOWS:
            {
                ...
                switch(achievementCriteria->referredAchievement)
                {
                    ...
                    case 4256:                  // Demolition Derby (horde)
                    {
                        ...
                        if ((achievementCriteria->ID == 11497 || achievementCriteria->ID == 12178) && unit->GetEntry() == 34802 ||   // Glaive Thrower
                            (achievementCriteria->ID == 11498 || achievementCriteria->ID == 12179) && unit->GetEntry() == 34775 ||   // Demolisher
                            (achievementCriteria->ID == 11500 || achievementCriteria->ID == 12181) && unit->GetEntry() == 34793 ||   // Catapult
                            (achievementCriteria->ID == 11501 || achievementCriteria->ID == 12182) && unit->GetEntry() == 34776)     // Siege Engine
                            break;
                        continue;
                    }
                }
...
void AchievementMgr<Player>::CompletedAchievement(AchievementEntry const* achievement, Player* referencePlayer)
{
    ...
    if (achievement->flags & ACHIEVEMENT_FLAG_SHOW_IN_GUILD_NEWS)
        if (Guild* guild = sGuildMgr.GetGuildById(referencePlayer->GetGuildId()))
            guild->LogNewsEvent(GUILD_NEWS_PLAYER_ACHIEVEMENT, time(NULL), referencePlayer->GetGUID(), achievement->flags & ACHIEVEMENT_FLAG_SHOW_IN_GUILD_HEADER, achievement->ID);
...
void AchievementGlobalMgr::LoadAchievementCriteriaRequirements()
{
    ...
    for (uint32 entryId = 0; entryId < sAchievementCriteriaStore.GetNumRows(); ++entryId)
    {
        ...
        switch(criteria->requiredType)
        {
            ...
            {
                AchievementEntry const* achievement = sAchievementStore.LookupEntry(criteria->referredAchievement);
                // Checked in LoadAchievementCriteriaLis
...
void AchievementGlobalMgr::LoadCompletedAchievements()
{
    ...
    do
    {
        ...
        if (!sAchievementStore.LookupEntry(achievement_id))
        {
            ...
            ERROR_LOG("Nonexistent achievement %u data removed from table `character_achievement`.",achievement_id);
