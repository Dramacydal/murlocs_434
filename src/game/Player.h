...
enum PlayerFlags
{
    ...
    PLAYER_FLAGS_ALLOW_ONLY_ABILITY     = 0x00800000,       // EVENT_SPELL_UPDATE_USABLE and EVENT_UPDATE_SHAPESHIFT_USABLE, disabled all abilitys on tab except autoattack
...
enum PlayerExtraFlags
{
    ...
    PLAYER_EXTRA_SPECTATOR          = 0x1000,
    PLAYER_EXTRA_FORBID_SPECTATE    = 0x2000,
};
...
class TradeData
{
    ...
    public:                                                 // access functions
        ...
        TradeSlots const GetTradeSlotForItem(ObjectGuid itemGuid);
...
enum VisualsType
{
    VISUALS_TYPE_PREMIUM_ACC = 0,
    VISUALS_TYPE_MORPH       = 1,
};
struct Visuals
{
    Visuals() : m_visHead(1), m_visShoulders(1), m_visChest(1), m_visWaist(1), m_visLegs(1), m_visFeet(1),
         m_visWrists(1), m_visHands(1), m_visBack(1), m_visMainhand(1), m_visOffhand(1), m_visRanged(1), m_type(VISUALS_TYPE_PREMIUM_ACC) {}
...
class MANGOS_DLL_SPEC Player : public Unit
{
    ...
    public:
        ...
        void AddRefundReference(ObjectGuid guid);
        void DeleteRefundReference(ObjectGuid guid);
        ...
        void RemoveItem(uint8 bag, uint8 slot, bool update, bool swap = false);// see ApplyItemOnStoreSpell notes
        ...
        void UpdateRefundableItems();
        void AddSoulboundTradeableItem(ObjectGuid itemGuid);
        void RemoveSoulboundTradeableItem(ObjectGuid itemGuid);
        ...
        void UpdateItemEnchantAtSkill(Item* pItem, EnchantmentSlot slot, uint32 checkSkill, uint32 oldSkillValue, uint32 newSkillValue);        
        bool CheckEnchantmentActiveBySkills(Item* pItem, EnchantmentSlot slot);
        ...
        void OnGossipSelect(WorldObject *pSource, uint32 gossipListId, uint32 menuId, uint32 entry = 0);
        ...
        void RewardRage(uint32 damage, uint32 weaponSpeedHitFactor, bool attacker, Unit* pVictim = NULL);
        ...
        void SendActionButtons(uint8 state) const;
        ...
        void InitDisplayIds(bool switchFaction = false);
        ...
        uint32 GetShieldBlockDamageValue(bool capped = false) const override;        // overwrite Unit version (virtual)
        ...
        uint8 GetPreferedBG() { return m_preferedBG; }
        void SetPreferedBG(uint8 preferedBG) { m_preferedBG = preferedBG; }
        ...
        void SetRuneCooldown(uint8 index, uint16 cooldown, uint32 bySpell = 0)
        {
            ...
            if (bySpell != -1)
                m_runes->runes[index].ConsumedBySpell = bySpell;
            m_runes->SetRuneState(index, (cooldown == 0) ? true : false);
        }
        int32 RuneUsedBySpell(uint8 index)
        {
            return m_runes->runes[index].ConsumedBySpell;
        }
        ...
        bool HasTitle(CharTitlesEntry const* title) const { return HasTitle(title->bit_index); }        void SetTitle(CharTitlesEntry const* title, bool lost = false);

        bool canSeeSpellClickOn(Creature const* creature) const;

        ObjectGuid m_lastSpellTargetGuid; // ��� - �����.
        uint32 m_GMscore;

        // Ranger: DotA killing system ;)
        uint32 m_GMkillscore;
        uint32 m_GMtimekillscore;

        void SendReturnToAreaMessage(uint32 areaid, uint32 timer);
        bool DkQuestsCompleted();
        bool HasOrphan();
        ...
        // Bg Anti-AFK check
        time_t m_lastOkBgAfkTime;
        float m_bgAfkDistance;
        Position m_bgLastAfkPosition;

        // Premium Account Features
        bool IsPremium() { return GetSession() ? GetSession()->IsPremium() : false; }
        bool IsPremiumActive() { return GetSession() ? GetSession()->IsPremiumActive() : false; }
        AccountPremiumType GetPremiumType() { return GetSession() ? GetSession()->GetPremiumType() : PREMIUM_DEFAULT; }
        AccountPremiumRateType GetPremiumRateType() { return GetSession() ? GetSession()->GetPremiumRateType() : PREMIUM_RATE_XP; }

        float GetPremiumXPModifier();
        float GetPremiumHonorModifier();
        float GetPremiumMoneyModifier();

        void InstallPremiumFeatures(bool apply);

        void SetKickDelay(uint32 delay);

        void SetSpectator(bool on);
        bool IsSpectator() const { return m_ExtraFlags & PLAYER_EXTRA_SPECTATOR; }
        bool AllowsSpectating() const { return (m_ExtraFlags & PLAYER_EXTRA_FORBID_SPECTATE) == 0; }
        bool ToggleAllowSpectate();
...
////////////////////Rest System/////////////////////
    ...
    private:
        ...
        uint32 m_vehicleDespawnTimer;
        uint32 m_vehicleCheckTimer;
