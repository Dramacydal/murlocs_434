...
bool Guild::AddMember(ObjectGuid plGuid, uint32 plRank)
{
    ...
    else
    {
        ...
        {
            ERROR_LOG("%s has a broken data in field `characters` table, cannot add him to guild.", plGuid.GetString().c_str());
            return false;
        }
    }
...
bool Guild::LoadRanksFromDB(QueryResult *guildRanksResult)
{
    if (!guildRanksResult)
    {
        ERROR_LOG("Guild %u has broken `guild_rank` data, creating new...",m_Id);
    ...
    if (broken_ranks)
    {
        ERROR_LOG("Guild %u has broken `guild_rank` data, repairing...", m_Id);
        CharacterDatabase.BeginTransaction();
        CharacterDatabase.PExecute("DELETE FROM guild_rank WHERE guildid='%u'", m_Id);
...
bool Guild::LoadMembersFromDB(QueryResult *guildMembersResult)
{
    ...
    do
    {
        ...
        {
            ERROR_LOG("%s has a broken data in field `characters`.`data`, deleting him from guild!", newmember.guid.GetString().c_str());
            CharacterDatabase.PExecute("DELETE FROM guild_member WHERE guid = '%u'", lowguid);
            continue;
        }
        if (!newmember.ZoneId)
        {
            ERROR_LOG("%s has broken zone-data", newmember.guid.GetString().c_str());
        ...
        {
            ERROR_LOG("%s has a broken data in field `characters`.`class`, deleting him from guild!", newmember.guid.GetString().c_str());
            CharacterDatabase.PExecute("DELETE FROM guild_member WHERE guid = '%u'", lowguid);
            continue;
        }
...
void Guild::LoadGuildBankFromDB()
{
    ...
    do
    {
        ...
        if (tabId >= GetPurchasedTabs())
        {
            ERROR_LOG("Table `guild_bank_tab` have not purchased tab %u for guild %u, skipped", tabId, m_Id);
            continue;
        }
    ...
    //                                        0     1     2      3       4          5
    result = CharacterDatabase.PQuery("SELECT data, text, TabId, SlotId, item_guid, item_entry FROM guild_bank_item JOIN item_instance ii ON item_guid = ii.guid LEFT JOIN item_instance_text iit ON item_guid = iit.guid WHERE guildid='%u' ORDER BY TabId", m_Id);
    if (!result)
        return;
    ...
    do
    {
        ...
        if (TabId >= GetPurchasedTabs())
        {
            ERROR_LOG( "Guild::LoadGuildBankFromDB: Invalid tab for item (GUID: %u id: #%u) in guild bank, skipped.", ItemGuid,ItemEntry);
            continue;
        }

        if (SlotId >= GUILD_BANK_MAX_SLOTS)
        {
            ERROR_LOG( "Guild::LoadGuildBankFromDB: Invalid slot for item (GUID: %u id: #%u) in guild bank, skipped.", ItemGuid,ItemEntry);
            continue;
        }
        ...
        if (!proto)
        {
            ERROR_LOG( "Guild::LoadGuildBankFromDB: Unknown item (GUID: %u id: #%u) in guild bank, skipped.", ItemGuid,ItemEntry);
            continue;
        }
        ...
        if (!pItem->LoadFromDB(ItemGuid, fields))
        {
            ...
            ERROR_LOG("Item GUID %u not found in item_instance, deleting from Guild Bank!", ItemGuid);
            delete pItem;
            continue;
        }
...
void Guild::LoadGuildBankEventLogFromDB()
{
    ...
    for (uint32 tabId = 0; tabId < uint32(GetPurchasedTabs()); ++tabId)
    {
        ...
        do
        {
            ...
            if (NewEvent.isMoneyEvent())
            {
                ...
                ERROR_LOG("GuildBankEventLog ERROR: MoneyEvent LogGuid %u for Guild %u had incorrectly set its TabId to %u, correcting it to %u TabId", logGuid, m_Id, tabId, GUILD_BANK_MONEY_LOGS_TAB);
                continue;
            }
            else
                // add event to list
    ...
    do
    {
        ...
        if (!NewEvent.isMoneyEvent())
            ERROR_LOG("GuildBankEventLog ERROR: MoneyEvent LogGuid %u for Guild %u is not MoneyEvent - ignoring...", fields[0].GetUInt32(), m_Id);
        else
            // add event to list
            // events are ordered from oldest (in beginning) to latest (in the end)
    ...
    else                                                    // Bank -> Char swap with slot (move)
    {
        ...
        else                                                // Bank <-> Char swap items
        {
            ...
            if (pItemChar)
            {
                ...
                if (pl->GetSession()->GetSecurity() > SEC_CURATOR && sWorld.getConfig(CONFIG_BOOL_GM_LOG_TRADE))
                {
