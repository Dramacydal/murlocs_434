...
void WorldSession::HandleQuestgiverStatusQueryOpcode(WorldPacket & recv_data)
{
    ...
    switch(questgiver->GetTypeId())
    {
        ...
        default:
            ERROR_LOG("QuestGiver called for unexpected type %u", questgiver->GetTypeId());
            break;
    }
...
void WorldSession::HandleQuestgiverChooseRewardOpcode(WorldPacket & recv_data)
{
    ...
    {
        ERROR_LOG("Error in CMSG_QUESTGIVER_CHOOSE_REWARD - %s tried to get invalid reward (%u) (probably packet hacking)", _player->GetGuidStr().c_str(), _player->GetGUIDLow(), reward);
        return;
    }
...
void WorldSession::HandleQuestgiverCompleteQuest(WorldPacket& recv_data)
{
    ...
    // --- WPE Check
    QuestStatus status = _player->GetQuestStatus(quest);
    ...
    bool ok = true;
    if (pObject->HasInvolvedQuest(quest) || pObject->HasQuest(quest))
    {
        if (pQuest->IsAutoComplete())
        {
            if (!_player->CanTakeQuest(pQuest, false) && status == QUEST_STATUS_NONE)
                ok = false;
        }
        else if (status == QUEST_STATUS_NONE || status == QUEST_STATUS_UNAVAILABLE)
            ok = false;
    }
    else
        ok = false;
    ...
    if (!ok)
    {
        ...
        if (_player->GetSession()->GetSecurity() > SEC_CURATOR)
        {
            ss << "Exploit W " << quest << " TypeId: " << uint32(pObject->GetTypeId()) << " Entry: " << pObject->GetEntry()
                << " status: " << uint32(status) << " cantake: " << _player->CanTakeQuest(pQuest, false)
                << " IsAutoComplete: " << pQuest->IsAutoComplete();

            ChatHandler(_player).SendSysMessage(ss.str().c_str());
        }
        else
        {
            ss << "Exploit W " << quest << " " << uint32(pObject->GetTypeId()) << " " << pObject->GetEntry()
                << " " << uint32(status) << " " << (_player->CanTakeQuest(pQuest, false) ? 1 : 0)
                << " " << (pQuest->IsAutoComplete() ? 1 : 0);

            switch (quest)
            {
                case 24743:     // Shadow's Edge
                case 24549:     // Shadowmourne...
                    sWorld.BanAccount(BAN_CHARACTER_ACC, _player->GetName(), TimeStringToSecs("-1"), ss.str().c_str(), "Anticheat (WAF)");
                    break;
                default:
...
uint32 WorldSession::getDialogStatus(Player *pPlayer, Object* questgiver, uint32 defstatus)
{
    ...
    switch(questgiver->GetTypeId())
    {
        ...
        default:
            ...
            ERROR_LOG("Warning: GetDialogStatus called for unexpected type %u", questgiver->GetTypeId());
