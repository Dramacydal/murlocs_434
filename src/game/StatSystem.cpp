...
{
    if (apply)
        m_baseSpellPower += amount;
    else if (m_baseSpellPower < amount)
        m_baseSpellPower = 0;
    else
        m_baseSpellPower -= amount;
...
void Player::UpdateArmor()
{
    ...
    if (form == FORM_BEAR)
    {
        ...
        {
            if ((*itr)->GetSpellProto()->SpellIconID == 1558 && (*itr)->GetEffIndex() == EFFECT_INDEX_0 &&
        ...
        if (getClass() == CLASS_DRUID && GetShapeshiftForm())
        {
            if (SpellShapeshiftFormEntry const * entry = sSpellShapeshiftFormStore.LookupEntry(uint32(GetShapeshiftForm())))
                if (entry->flags1 & 0x20)
                    val2 += std::max(GetStat(STAT_AGILITY) - 10.0f, 0.0f) * chrEntry->apPerStr * 2;
        }
    }
...
void Player::CalculateMinMaxDamage(WeaponAttackType attType, bool normalized, float& min_damage, float& max_damage)
{
    ...
    else if (!CanUseEquippedWeapon(attType))                 // check if player not in form but still can't use (disarm case, broken, etc)
    {
...
{
    if (apply)
        m_baseManaRegen += amount;
    else if (m_baseManaRegen < amount)
        m_baseManaRegen = 0;
    else
        m_baseManaRegen -= amount;
    UpdateManaRegen();
