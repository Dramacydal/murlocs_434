...
void WorldSession::HandleUseItemOpcode(WorldPacket& recvPacket)
{
    ...
    if (proto->Area && proto->Area != pUser->GetAreaId() && proto->Area != pUser->GetZoneId() ||
    ...
    DEBUG_LOG("WORLD: CMSG_USE_ITEM packet, unitTargetGuid: %s, goTargetGuid: %s, corpseTargetGuid: %s, itemTargetGuid: %s, targetMask: %X",
        targets.getUnitTargetGuid().GetString().c_str(),
        targets.getGOTargetGuid().GetString().c_str(),
        targets.getCorpseTargetGuid().GetString().c_str(),
        targets.getItemTargetGuid().GetString().c_str(),
        targets.m_targetMask);
...
void WorldSession::HandleOpenItemOpcode(WorldPacket& recvPacket)
{
    ...
    if(lockId && !pItem->HasFlag(ITEM_FIELD_FLAGS, ITEM_DYNFLAG_UNLOCKED))
    {
        ...
        if (!lockInfo)
        {
            ...
            ERROR_LOG( "WORLD::OpenItem: item [guid = %u] has an unknown lockId: %u!", pItem->GetGUIDLow() , lockId);
            return;
        }
    ...
    if (pItem->HasFlag(ITEM_FIELD_FLAGS, ITEM_DYNFLAG_WRAPPED))// wrapped?
    {
        ...
        {
            ERROR_LOG("Wrapped item %u don't have record in character_gifts table and will deleted", pItem->GetGUIDLow());
...
void WorldSession::HandleCastSpellOpcode(WorldPacket& recvPacket)
{
    ...
    Unit* _mover = _player->GetMover();
    if (_mover != _player && _mover->GetTypeId()==TYPEID_PLAYER)
    {
    ...
    {
        _player->m_lastSpellTargetGuid = targets.getUnitTarget()->GetObjectGuid();
        ...
        _player->m_lastSpellTargetGuid.Clear();
...
void WorldSession::HandleCancelAuraOpcode( WorldPacket& recvPacket)
{
    ...
    if (!spellInfo)
    {
        DEBUG_LOG("WORLD: CMSG_CANCEL_AURA packet, UNKNOWN spellId: %u", spellId);
        return;
    }

    DEBUG_LOG("WORLD: CMSG_CANCEL_AURA packet, spellid: %u", spellId);

    bool gmCanCancel = _player->isGameMaster() && GetSecurity() >= SEC_ADMINISTRATOR;

    // megai2: maybe client cancel? like was on 309 in demonform
    // megai2: Spirit on redemption bugged cancel
    if (spellId == 27827 || spellId == 27795 || spellId == 27792)
        return;

    if (spellInfo->HasAttribute(SPELL_ATTR_CANT_CANCEL) && !gmCanCancel)
        return;

    if (IsPassiveSpell(spellInfo))
        return;

    if (!IsPositiveSpell(spellId) && !gmCanCancel)
    {
        // ignore for remote control state
        if (!_player->IsSelfMover())
        {
...
>>>>>>> 03a44c9... Mage 400 INTO master/434
    ...
    if (IsChanneledSpell(spellInfo))
    {
        ...
        if (Unit* charm = _player->GetCharm())
            if (charm->GetUInt32Value(UNIT_CREATED_BY_SPELL) == spellInfo->Id)
                _player->Uncharm();
        return;
    }
...
void WorldSession::HandlePetCancelAuraOpcode( WorldPacket& recvPacket)
{
    ...
    if(!spellInfo)
    {
        ERROR_LOG("WORLD: unknown PET spell id %u", spellId);
        return;
    }
    ...
    if (!pet)
    {
        ERROR_LOG("HandlePetCancelAuraOpcode - %s not exist.", guid.GetString().c_str());
        return;
    }

    if (guid != GetPlayer()->GetPetGuid() && guid != GetPlayer()->GetCharmGuid())
    {
        ERROR_LOG("HandlePetCancelAura. %s isn't pet of %s", guid.GetString().c_str(), GetPlayer()->GetGuidStr().c_str());
        return;
    }
...
void WorldSession::HandleSpellClick( WorldPacket & recv_data )
{
    ...
    DEBUG_LOG("WORLD: Recvd CMSG_SPELLCLICK Message guid: %s", guid.GetString().c_str());
    ...
    for (SpellClickInfoMap::const_iterator itr = clickPair.first; itr != clickPair.second; ++itr)
    {
        ...
        {
            Unit* caster = (itr->second.castFlags & SPELL_CLICK_FLAG_PLAYER_AS_CASTER) ? (Unit*)_player : (Unit*)unit;
            ObjectGuid casterGuid = caster->GetObjectGuid();
            if ((itr->second.castFlags & SPELL_CLICK_FLAG_OWNER_AS_AFFECTIVE_CASTER) && unit->GetOwner())
                casterGuid = unit->GetOwnerGuid();

            Unit* target = (itr->second.castFlags & SPELL_CLICK_FLAG_PLAYER_AS_TARGET) ? (Unit*)_player : (Unit*)unit;

            if (unit->GetTypeId() == TYPEID_UNIT && ((Creature*)unit)->AI())
            {
                if (!((Creature*)unit)->AI()->HandleSpellClick(caster, target, itr->second.spellId, NULL, NULL, NULL, casterGuid))
                    caster->CastCustomSpell(target, itr->second.spellId, NULL, NULL, NULL, itr->second.castFlags & SPELL_CLICK_FLAG_FORCE_NOT_TRIGGERED ? false : true, NULL, NULL, casterGuid);
            }
            else
                caster->CastCustomSpell(target, itr->second.spellId, NULL, NULL, NULL, itr->second.castFlags & SPELL_CLICK_FLAG_FORCE_NOT_TRIGGERED ? false : true, NULL, NULL, casterGuid);
...
void WorldSession::HandleGetMirrorimageData( WorldPacket & recv_data ) //Full copyright goes to TrinityCore2
{
    ...
    Creature* pCreature = _player->GetMap()->GetAnyTypeCreature(guid); //ObjectAccessor::GetUnitInWorld((WorldObject*)NULL, guid);
    ...
    data << guid;
    data << (uint32)pCaster->GetDisplayId();
