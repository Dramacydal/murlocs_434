...
void FleeingMovementGenerator<T>::_setTargetLocation(T &owner)
{
    ...
    if (owner.hasUnitState((UNIT_STAT_CAN_NOT_REACT | UNIT_STAT_ROOT) & ~UNIT_STAT_FLEEING))
        return;
...
bool FleeingMovementGenerator<T>::_getPoint(T &owner, float &x, float &y, float &z)
{
    ...
    // Dalaran Sewers: do not fall from ledge
    if (owner.GetMapId() == 617 && fabs(curr_z - z) > 3.6f)
        return false;
