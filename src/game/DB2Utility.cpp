...
{
    return sObjectMgr.GetItemPrototype(id) != NULL;
}
...
void DB2Utilities::WriteItemDbReply(DB2Storage<ItemEntry> const& /*store*/, uint32 /*locale*/, uint32 id, ByteBuffer& buffer)
{
...
void DB2Utilities::WriteItemSparseDbReply(DB2Storage<ItemSparseEntry> const& /*store*/, uint32 locale, uint32 id, ByteBuffer& buffer)
{
    ItemPrototype const* proto = sObjectMgr.GetItemPrototype(id);
    MANGOS_ASSERT(proto);
    ...
    buffer << float(proto->Unknown);
    buffer << float(proto->Unknown1);
    ...
    for (uint32 x = 0; x < MAX_ITEM_PROTO_STATS; ++x)
        buffer << int32(proto->ItemStat[x].ItemStatType2);

    for (uint32 x = 0; x < MAX_ITEM_PROTO_STATS; ++x)
        buffer << int32(proto->ItemStat[x].ItemStatValue2);
    ...
    sObjectMgr.GetItemLocaleStrings(id, locale, &name, &desc);
    ...
    buffer << uint32(proto->Unknown400_1);
    buffer << uint32(proto->Unknown400_2);
}
