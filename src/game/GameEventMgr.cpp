...
void GameEventMgr::UnApplyEvent(uint16 event_id)
{
    ...
    if (event_id == 102)
        sWorld.ApplyEvent102Morphs(false);
    else if (event_id == 103)
        sWorld.ApplyEvent103Morphs(false);
}
...
void GameEventMgr::ApplyNewEvent(uint16 event_id, bool resume)
{
    ...
    if (sWorld.getConfig(CONFIG_BOOL_EVENT_ANNOUNCE))
        sWorld.SendGMGlobalText(SEC_MODERATOR, LANG_EVENTMESSAGE, mGameEvent[event_id].description.c_str());
    ...
    if (event_id == 102)
        sWorld.ApplyEvent102Morphs(true);
    else if (event_id == 103)
        sWorld.ApplyEvent103Morphs(true);
}
...
void GameEventMgr::GameEventSpawn(int16 event_id)
{
    ...
    {
        ERROR_LOG("GameEventMgr::GameEventSpawn attempt access to out of range mGameEventCreatureGuids element %i (size: " SIZEFMTD ")",internal_event_id,mGameEventCreatureGuids.size());
        return;
    }
    ...
    {
        ERROR_LOG("GameEventMgr::GameEventSpawn attempt access to out of range mGameEventGameobjectGuids element %i (size: " SIZEFMTD ")",internal_event_id,mGameEventGameobjectGuids.size());
        return;
    }
    ...
    if (event_id > 0)
    {
        ...
        {
            ERROR_LOG("GameEventMgr::GameEventSpawn attempt access to out of range mGameEventSpawnPoolIds element %i (size: " SIZEFMTD ")", event_id, mGameEventSpawnPoolIds.size());
            return;
        }
...
void GameEventMgr::GameEventUnspawn(int16 event_id)
{
    ...
    if (internal_event_id < 0 || (size_t)internal_event_id >= mGameEventCreatureGuids.size())
    {
        ERROR_LOG("GameEventMgr::GameEventUnspawn attempt access to out of range mGameEventCreatureGuids element %i (size: " SIZEFMTD ")",internal_event_id,mGameEventCreatureGuids.size());
        return;
    }
    ...
    if (internal_event_id < 0 || (size_t)internal_event_id >= mGameEventGameobjectGuids.size())
    {
        ERROR_LOG("GameEventMgr::GameEventUnspawn attempt access to out of range mGameEventGameobjectGuids element %i (size: " SIZEFMTD ")",internal_event_id,mGameEventGameobjectGuids.size());
        return;
    }
    ...
    if (event_id > 0)
    {
        if ((size_t)event_id >= mGameEventSpawnPoolIds.size())
        {
            ERROR_LOG("GameEventMgr::GameEventUnspawn attempt access to out of range mGameEventSpawnPoolIds element %i (size: " SIZEFMTD ")", event_id, mGameEventSpawnPoolIds.size());
            return;
        }
