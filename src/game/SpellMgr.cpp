...
uint32 GetSpellCastTime(SpellEntry const* spellInfo, Spell const* spell)
{
    ...
    if (spell)
    {
        ...
        else if (spellInfo->SpellVisual[0] == 3881 && spell->GetCaster()->HasAura(67556)) // cooking with Chef Hat
            return 500;
    }
...
bool IsPositiveAura(uint32 spellId)
{
    ...
    return IsPositiveAura(spellproto);
}
bool IsPositiveAura(SpellEntry const * spellproto)
{
    for (uint32 i = 0; i < MAX_EFFECT_INDEX; ++i)
        if (IsSpellAppliesAura(spellproto, 1 << i) && !IsPositiveAura(spellproto, SpellEffectIndex(i)))
            return false;
        ...
        case SPELL_EFFECT_APPLY_AREA_AURA_FRIEND:
        {
            switch(spellEffect->EffectApplyAuraName)
            {
                ...
                case SPELL_AURA_PROC_TRIGGER_SPELL:
                    ...
                    // Carrying Seaforium charge & Carrying Huge Seaforium (IoC)
                    if (spellproto->Id == 52418 || spellproto->Id == 66271 || spellproto->Id == 68377)
                        return false;
                    break;
                case SPELL_AURA_MOD_STUN:                   //have positive and negative spells, we can't sort its correctly at this moment.
                    ...
                    // Foam Sword Attack
                    if (spellproto->Id == 62973)
                        return true;

                    // Dark Brewmaiden's Stun
                    if (spellproto->Id == 47340)
                        return false;

                    // End of Round
                    if (spellproto->Id == 52459)
                        return false;
                    break;
                ...
                case SPELL_AURA_MOD_DAMAGE_PERCENT_TAKEN:
                {
                    ...
                    if (spellproto->Id == 34709)    // Shadow Sight
                        return false;
                    break;
                }
...
bool IsPositiveEffect(SpellEntry const *spellproto, SpellEffectIndex effIndex)
{
    ...
    switch (spellproto->GetSpellEffectIdByIndex(effIndex))
    {
        ...
        case SPELL_EFFECT_APPLY_AURA:
        case SPELL_EFFECT_APPLY_AREA_AURA_FRIEND:
            return IsPositiveAura(spellproto, effIndex);
        default:
            break;
    }
...
SpellCastResult GetErrorAtShapeshiftedCast (SpellEntry const *spellInfo, uint32 form)
{
    ...
    if (form > 0)
    {
        ...
        if (!shapeInfo)
        {
            ERROR_LOG("GetErrorAtShapeshiftedCast: unknown shapeshift %u", form);
            return SPELL_CAST_OK;
        }
...
bool SpellMgr::IsSpellProcEventCanTriggeredBy(SpellProcEventEntry const * spellProcEvent, uint32 EventProcFlag, SpellEntry const * procSpell, uint32 procFlags, uint32 procExtra)
{
    ...
    if (EventProcFlag & (PROC_FLAG_KILLED | PROC_FLAG_KILL | PROC_FLAG_ON_TRAP_ACTIVATION | PROC_FLAG_ON_DEATH | PROC_FLAG_ON_JUMP))
        return true;
    ...
    if (procEvent_procEx == PROC_EX_NONE)
    {
        ...
        // No extra req, so can trigger for (damage/healing present)
...
bool SpellMgr::IsSameEffectSpell(uint32 spellId_1, uint32 spellId_2) const
{
    SpellEntry const *spellInfo_1 = sSpellStore.LookupEntry(spellId_1);
    SpellEntry const *spellInfo_2 = sSpellStore.LookupEntry(spellId_2);

    // initial checks
    if (spellInfo_1->GetSpellFamilyName() == SPELLFAMILY_POTION || spellInfo_2->GetSpellFamilyName() == SPELLFAMILY_POTION)
        return false;
    for (uint8 i = 0; i < MAX_EFFECT_INDEX; ++i)
    {
        ...
        {
            SpellEffectEntry const * effect2 = spellInfo_2->GetSpellEffect(SpellEffectIndex(k));
            if (!effect2)
                continue;

             if (effect1->Effect && effect2->Effect &&
                effect1->Effect == effect2->Effect &&
                effect1->EffectApplyAuraName == effect2->EffectApplyAuraName &&
                effect1->EffectMiscValue == effect2->EffectMiscValue &&
                effect1->EffectBasePoints == effect2->EffectBasePoints)
                 return true;
        }
    }
    ...
    for (uint8 i = 0; i < MAX_EFFECT_INDEX; ++i)
    {
        ...
        if(effect1->EffectApplyAuraName != effect2->EffectApplyAuraName)
            return false;

        if(effect1->EffectMiscValue != effect2->EffectMiscValue)
            return false;
    }
    ...
    switch(spellInfo_1->GetSpellFamilyName())
    {
        case SPELLFAMILY_GENERIC:
            switch(spellInfo_2->GetSpellFamilyName())
            {
                case SPELLFAMILY_GENERIC:                   // same family case
                {
                    ...
                    // Dark Essence & Light Essence (Trial of the Crusader)
                    ...
                    // Ranger: Energized (Item - Coliseum 25 Heroic Healer Trinket and Item - Coliseum 25 Normal Healer Trinket)
        ...
        case SPELLFAMILY_WARLOCK:
            if( classOptions2 && classOptions2->SpellFamilyName == SPELLFAMILY_WARLOCK )
            {
                ...
                // (Shadowflame or Glyph of Shadowflame) and (Corruption or Curse of Agony)
                if( spellInfo_1->SpellIconID == 3317 && (spellInfo_2->SpellIconID == 313  || spellInfo_2->SpellIconID == 544) ||
                    spellInfo_2->SpellIconID == 3317 && (spellInfo_1->SpellIconID == 313  || spellInfo_1->SpellIconID == 544) )
                    return false;

                // (Shadowflame or Glyph of Shadowflame) and Curse of Doom
        ...
        case SPELLFAMILY_WARRIOR:
            ...
            // Vigilance and Lightning Shield
            if (spellInfo_1->SpellIconID == 2834 && spellInfo_2->SpellIconID == 19)
                return false;
        ...
        case SPELLFAMILY_PRIEST:
            if (classOptions2 && classOptions2->SpellFamilyName == SPELLFAMILY_PRIEST)
            {
                ...
                if (classOptions2 && (spellInfo_1->Id == 47753 && classOptions2->SpellFamilyFlags & UI64LIT(0x0000000000000001)) ||
                    classOptions1 && (spellInfo_2->Id == 47753 && classOptions1->SpellFamilyFlags & UI64LIT(0x0000000000000001)))
                    return false;
        ...
        case SPELLFAMILY_ROGUE:
            if (classOptions2 && classOptions2->SpellFamilyName == SPELLFAMILY_ROGUE )
            {
                ...
                if (spellInfo_1->SpellIconID == 153 && spellInfo_2->SpellIconID == 500 ||
                    spellInfo_2->SpellIconID == 153 && spellInfo_1->SpellIconID == 500)
                    return false;
            }
        ...
        case SPELLFAMILY_PALADIN:
            ...
            if (spellInfo_1->SpellIconID == 502 && classOptions2 && classOptions2->SpellFamilyName == SPELLFAMILY_GENERIC && spellInfo_2->SpellIconID == 502 && spellInfo_2->SpellVisual[0] == 969)
                return false;
        ...
        case SPELLFAMILY_SHAMAN:
            ...
            // Lightning Shield and Vigilance
            if (spellInfo_1->SpellIconID == 19 && spellInfo_2->SpellIconID == 2834)
                return false;
            break;
        case SPELLFAMILY_DEATHKNIGHT:
...
bool SpellMgr::IsGroupBuff(SpellEntry const *spellInfo)
{
    ...
    for (int i = 0; i < MAX_EFFECT_INDEX; ++i)
    {
        ...
        switch (effect->EffectImplicitTargetA)
        {
            case TARGET_SINGLE_PARTY:
...
bool SpellMgr::IsStackableSpellAuraHolder(SpellEntry const* spellInfo, Unit const* unit)
{
    ...
    for (int i = 0; i < MAX_EFFECT_INDEX; ++i)
    {
        switch (spellInfo->GetEffectApplyAuraNameByIndex(SpellEffectIndex(i)))
        {
            // DoT/HoT etc and some more
            case SPELL_AURA_PERIODIC_DAMAGE:
            case SPELL_AURA_PERIODIC_DAMAGE_PERCENT:
            case SPELL_AURA_PERIODIC_LEECH:
            case SPELL_AURA_PERIODIC_HEAL:
            ...
            case SPELL_AURA_LINKED:
                if (unit->GetObjectGuid().IsVehicle())
                    return true;
            case SPELL_AURA_DUMMY:
                if (spellInfo->Id != 53601)      // Sacred Shield
                    return true;
...
void SpellMgr::LoadSpellPetAuras()
{
    ...
    do
    {
        ...
        else
        {
            ...
            {
                ERROR_LOG("Spell %u listed in `spell_pet_auras` does not have dummy aura or dummy effect", spell);
                continue;
            }
...
SpellCastResult SpellMgr::GetSpellAllowedInLocationError(SpellEntry const *spellInfo, uint32 map_id, uint32 zone_id, uint32 area_id, Player const* player)
{
    ...
    switch(spellInfo->Id)
    {
        ...
        {
            return player && player->IsSpectator() ? SPELL_CAST_OK : SPELL_FAILED_DONT_REPORT;
        }
        case 40851:
        {
            return zone_id == 3959 ? SPELL_CAST_OK : SPELL_FAILED_INCORRECT_AREA;
        }
        ...
        case 57055:                                         // Mini
        {
            return map_id == 619 ? SPELL_CAST_OK : SPELL_FAILED_DONT_REPORT;
        }
        case 69065:                                         // Impaled
        ...
        {
            if (player && !player->HasAura(43880) && !player->HasAura(43883))
...
void SpellMgr::CheckUsedSpells(char const* table)
{
    ...
    do
    {
        ...
        {
            ERROR_LOG("Table '%s' for spell %u have wrong SpellFamily value(%u), skipped.",table,spell,family);
            continue;
        }
        ...
        if (spellIcon < -1)
        {
            ERROR_LOG("Table '%s' for spell %u have wrong SpellIcon value(%u), skipped.",table,spell,spellIcon);
            continue;
        }
        ...
        if (spellVisual < -1)
        {
            ERROR_LOG("Table '%s' for spell %u have wrong SpellVisual value(%u), skipped.",table,spell,spellVisual);
            continue;
        }
        ...
        {
            ERROR_LOG("Table '%s' for spell %u have wrong SpellEffect type value(%u), skipped.",table,spell,effectType);
            continue;
        }

        if (auraType < -1 || auraType >= TOTAL_AURAS)
        {
            ERROR_LOG("Table '%s' for spell %u have wrong SpellAura type value(%u), skipped.",table,spell,auraType);
            continue;
        }

        if (effectIdx < -1 || effectIdx >= 3)
        {
            ERROR_LOG("Table '%s' for spell %u have wrong EffectIdx value(%u), skipped.",table,spell,effectIdx);
            continue;
        }
        ...
        if(spell)
        {
            ...
            if(!spellEntry)
            {
                ERROR_LOG("Spell %u '%s' not exist but used in %s.",spell,name.c_str(),code.c_str());
                continue;
            }
            ...
            if(familyMaskA != UI64LIT(0xFFFFFFFFFFFFFFFF) || familyMaskB != 0xFFFFFFFF)
            {
                if(familyMaskA == UI64LIT(0x0000000000000000) && familyMaskB == 0x00000000)
                {
                    ...
                    {
                        ERROR_LOG("Spell %u '%s' not fit to (" I64FMT "," I32FMT ") but used in %s.",
                ...
                else
                {
                    ...
                    {
                        ERROR_LOG("Spell %u '%s' not fit to (" I64FMT "," I32FMT ") but used in %s.",spell,name.c_str(),familyMaskA,familyMaskB,code.c_str());
                        continue;
                    }
                }
            }
            ...
            {
                ERROR_LOG("Spell %u '%s' icon(%u) <> %u but used in %s.",spell,name.c_str(),spellEntry->SpellIconID,spellIcon,code.c_str());
                continue;
            }

            if(spellVisual >= 0 && spellEntry->SpellVisual[0] != spellVisual)
            {
                ERROR_LOG("Spell %u '%s' visual(%u) <> %u but used in %s.",spell,name.c_str(),spellEntry->SpellVisual[0],spellVisual,code.c_str());
                continue;
            }
            ...
            if (effectIdx >= EFFECT_INDEX_0)
            {
                ...
                {
                    ERROR_LOG("Spell %u '%s' effect%d <> %u but used in %s.",spell,name.c_str(),effectIdx+1,effectType,code.c_str());
                    continue;
                }

                if(auraType >= 0 && spellEffect && spellEffect->EffectApplyAuraName != uint32(auraType))
                {
                    ERROR_LOG("Spell %u '%s' aura%d <> %u but used in %s.",spell,name.c_str(),effectIdx+1,auraType,code.c_str());
                    continue;
                }
            }
            else
            {
                ...
                {
                    ERROR_LOG("Spell %u '%s' not have effect %u but used in %s.",spell,name.c_str(),effectType,code.c_str());
                    continue;
                }

                if(auraType >= 0 && !IsSpellHaveAura(spellEntry,AuraType(auraType)))
                {
                    ERROR_LOG("Spell %u '%s' not have aura %u but used in %s.",spell,name.c_str(),auraType,code.c_str());
                    continue;
                }
            }
        }
        else
        {
            ...
            {
                if(effectIdx >= 0)
                    ERROR_LOG("Spells '%s' not found for family %i (" I64FMT "," I32FMT ") icon(%i) visual(%i) category(%i) effect%d(%i) aura%d(%i) but used in %s",
                    ...
                    ERROR_LOG("Spells '%s' not found for family %i (" I64FMT "," I32FMT ") icon(%i) visual(%i) category(%i) effect(%i) aura(%i) but used in %s",
...
DiminishingGroup GetDiminishingReturnsGroupForSpell(SpellEntry const* spellproto, bool triggered)
{
    ...
    switch(spellproto->GetSpellFamilyName())
    {
        case SPELLFAMILY_GENERIC:
            ...
            else if (spellproto->SpellVisual[0] == 2816 && spellproto->SpellIconID == 15)
        ...
        {
            // Gouge
            if (spellproto->IsFitToFamilyMask(UI64LIT(0x8)))
                return DIMINISHING_DISORIENT;
            // Blind
        ...
        case SPELLFAMILY_HUNTER:
        {
            ...
            else if (spellproto->IsFitToFamilyMask(UI64LIT(0x40000)) && spellproto->SpellIconID == 132)
        ...
        case SPELLFAMILY_WARLOCK:
        {
            ...
            if (spellproto->IsFitToFamilyMask(UI64LIT(0x00080000000)))
                return DIMINISHING_LIMITONLY;
            // Howl of Terror
            else if (classOptions && classOptions->SpellFamilyFlags & UI64LIT(0x800000000))
                return DIMINISHING_FEAR_BLIND;
            // Seduction
            else if (classOptions && classOptions->SpellFamilyFlags & UI64LIT(0x00040000000))
                return DIMINISHING_FEAR_BLIND;
            // Curse of Elements
            else if (classOptions && classOptions->SpellFamilyFlags & UI64LIT(0x20000000000))
                return DIMINISHING_LIMITONLY;
            break;
        }
        case SPELLFAMILY_PALADIN:
        ...
        case SPELLFAMILY_DRUID:
        {
            ...
            else if (classOptions && classOptions->SpellFamilyFlags & UI64LIT(0x00000200))
                return DIMINISHING_CONTROL_ROOT;
            // Faerie Fire
        ...
        case SPELLFAMILY_WARRIOR:
        {
            ...
            if (spellproto->IsFitToFamilyMask(UI64LIT(0x00000000002)))
                return DIMINISHING_LIMITONLY;
            // Intimidating Shout
            else if (classOptions && classOptions->SpellFamilyFlags & UI64LIT(0x40000))
                return DIMINISHING_FEAR_BLIND;
            // Charge - since 3.1.0
        ...
        case SPELLFAMILY_DEATHKNIGHT:
        {
            ...
            if (spellproto->SpellIconID == 2797)
                return DIMINISHING_DISORIENT;
            // Mark of Blood
            else if ((classOptions && classOptions->SpellFamilyFlags & UI64LIT(0x10000000))
    ...
    if ((mechanic & ((1<<(MECHANIC_KNOCKOUT-1))|(1<<(MECHANIC_POLYMORPH-1))|(1<<(MECHANIC_SAPPED-1)))) &&
         spellproto->SpellIconID != 292) // except Blast Wave
        return DIMINISHING_DISORIENT;
...
int32 GetDiminishingReturnsLimitDuration(DiminishingGroup group, SpellEntry const* spellproto)
{
    ...
    switch (spellproto->GetSpellFamilyName())
    {
        ...
        {
            // Curse of Elements
            if (classOptions && classOptions->SpellFamilyFlags & UI64LIT(0x20000000000))
                return 120000;
            // Banish
...
bool IsSpellIgnoresHealReductionEffects(SpellEntry const * spellInfo)
{
    if (!spellInfo)
        return false;
...
bool IsBinarySpell(SpellEntry const * spellProto)
{
    for (uint8 i = 0; i < MAX_EFFECT_INDEX; ++i)
    {
        SpellEffectEntry const * effect = spellProto->GetSpellEffect(SpellEffectIndex(i));
        if (!effect)
            continue;

        if ((effect->Effect == SPELL_EFFECT_SCHOOL_DAMAGE || effect->Effect == SPELL_EFFECT_POWER_DRAIN ||
            effect->Effect == SPELL_EFFECT_HEALTH_LEECH || effect->Effect == SPELL_EFFECT_ENVIRONMENTAL_DAMAGE ||
            effect->Effect == SPELL_EFFECT_POWER_BURN || effect->Effect == SPELL_EFFECT_HEAL) ||
            effect->EffectApplyAuraName &&
            (effect->EffectApplyAuraName == SPELL_AURA_PERIODIC_DAMAGE || effect->EffectApplyAuraName == SPELL_AURA_PERIODIC_HEAL ||
...
uint32 _GetFirstSpellInChain(uint32 spell_id)
{
    return sSpellMgr.GetFirstSpellInChain(spell_id);
}
