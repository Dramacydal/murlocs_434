...
void WorldSession::HandleBattlemasterJoinOpcode( WorldPacket & recv_data )
{
    ...
    {
        ERROR_LOG("Battleground: invalid bgtype (%u) received. possible cheater? player guid %u",bgTypeId_,_player->GetGUIDLow());
        return;
    }
    ...
    {
        ERROR_LOG("Battleground: no available bg / template found");
        return;
    }
    ...
    if (!joinAsGroup)
    {
        ...
        // dk in starting zones can't join bg
        if (!_player->DkQuestsCompleted())
            return;
        ...
        {
            SendNotification(LANG_YOU_IN_FLIGHT);
            return;
        }
    }
    else
...
void WorldSession::HandleBattlefieldListOpcode( WorldPacket &recv_data )
{
    ...
    if (!bl)
    {
        ERROR_LOG("Battleground: invalid bgtype received.");
        return;
    }
...
void WorldSession::HandleBattleFieldPortOpcode( WorldPacket &recv_data )
{
    ...
    if (!sBattlemasterListStore.LookupEntry(bgTypeId_))
    {
        ERROR_LOG("BattlegroundHandler: invalid bgtype (%u) received.", bgTypeId_);
        return;
    }
    ...
    {
        ERROR_LOG("BattlegroundHandler: Invalid CMSG_BATTLEFIELD_PORT received from player (%u), he is not in bg_queue.", _player->GetGUIDLow());
        return;
    }
    ...
    // Ranger: не портуем на бг если мы летим на такси - что-то вся система с выходом с бг ооочень криво работает
    if (_player->IsTaxiFlying())
    {
        SendNotification(LANG_YOU_IN_FLIGHT);
        return;
    ...
    {
        ERROR_LOG("BattlegroundHandler: itrplayerstatus not found.");
        return;
    }
    // if action == 1, then instanceId is required
    if (!ginfo.IsInvitedToBGInstanceGUID && action == 1)
    {
        ERROR_LOG("BattlegroundHandler: instance not found.");
        return;
    }
    ...
    if (action == 1 && ginfo.arenaType == ARENA_TYPE_NONE)
    {
        ...
        {
            ERROR_LOG("Battleground: Player %s (%u) has level (%u) higher than maxlevel (%u) of battleground (%u)! Do not port him to battleground!",
    ...
    switch( action )
    {
        ...
        case 0:                                         // leave queue
            ...
            break;
        default:
            ERROR_LOG("Battleground port: unknown action %u", action);
            break;
    }
}
...
void WorldSession::HandleBattlemasterJoinArena( WorldPacket & recv_data )
{
    ...
    if (!bg)
    {
        ERROR_LOG("Battleground: template bg (all arenas) not found");
        return;
    }
