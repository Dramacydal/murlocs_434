...
void BattleGroundIC::Reset()
{
    ...
    m_ActiveEvents[IC_EVENT_ADD_VEH] = BG_EVENT_NONE;
    m_ActiveEvents[IC_EVENT_BOSS_H] = BG_EVENT_NONE;
    m_ActiveEvents[IC_EVENT_BOSS_A] = BG_EVENT_NONE;
    m_ActiveEvents[IC_EVENT_ADD_TELEPORT] = BG_EVENT_NONE;

    // spawn bosses and their guards just when one of the gates is destroyed, to prevent some abusing
    m_ActiveEvents[IC_EVENT_ADD_A_BOSS] = BG_EVENT_NONE;
    m_ActiveEvents[IC_EVENT_ADD_A_GUARDS] = BG_EVENT_NONE;
    m_ActiveEvents[IC_EVENT_ADD_H_BOSS] = BG_EVENT_NONE;
    m_ActiveEvents[IC_EVENT_ADD_H_GUARDS] = BG_EVENT_NONE;

    gunshipHorde = NULL;
    gunshipAlliance = NULL;
}

void BattleGroundIC::SendTransportsInit(Player* player)
{
...
void BattleGroundIC::UpdateScore(TeamIndex teamIdx, int32 points )
{
    ...
    UpdateWorldState(teamIdx == TEAM_INDEX_HORDE ? IC_WS_HORDE_REINFORC : IC_WS_ALLIANCE_REINFORC, m_TeamScores[teamIdx]);
}
        ...
        if (m_NodeTimers[node])
        {
            ...
            else
            {
                ...
                if (node == BG_IC_NODE_HANGAR)
                    if (teamIndex == TEAM_INDEX_ALLIANCE)
                        gunshipAlliance->BuildStartMovePacket(GetBgMap());
                    else
                        gunshipHorde->BuildStartMovePacket(GetBgMap());
            }
        }
    }
...
{
    FillInitialWorldState(data, count, IC_WS_ALLIANCE_REINFORC_ON, 1);
    FillInitialWorldState(data, count, IC_WS_HORDE_RENFORC_ON,     1);
    FillInitialWorldState(data, count, IC_WS_ALLIANCE_REINFORC,     m_TeamScores[TEAM_INDEX_ALLIANCE]);
    FillInitialWorldState(data, count, IC_WS_HORDE_REINFORC,        m_TeamScores[TEAM_INDEX_HORDE]);

    // Gate icons
    for (uint8 z = 0; z < BG_IC_GATE_MAX; ++z)
        FillInitialWorldState(data, count, BG_IC_Gate_WS[z][GateStatus[z] == BG_IC_GO_GATES_DAMAGED ? 1 : 0], 1);
...
{
    gunshipHorde = CreateTransport(BG_IC_GO_HORDE_GUNSHIP, TRANSPORT_PERIOD_TIME);
    gunshipAlliance = CreateTransport(BG_IC_GO_ALLIANCE_GUNSHIP, TRANSPORT_PERIOD_TIME);
    ...
    for (uint8 i = 0; i < BG_IC_NODE_A_KEEP; ++i)
        SpawnEvent(i, BG_IC_NODE_TYPE_NEUTRAL, true);
...
void BattleGroundIC::EventPlayerDamageGO(Player *player, GameObject* target_obj, uint32 eventId, uint32 spellId)
{
    ...
    switch (target_obj->GetEntry())
    {
        case BG_IC_GO_ALLIANCE_GATE_1:
        {
            ...
            if (eventId == 22082)
            {
                SendMessage2ToAll(LANG_BG_IC_A_WEST_GATE_DESTROYED, CHAT_MSG_BG_SYSTEM_ALLIANCE, player, spellId);
                UpdateWorldState(BG_IC_Gate_WS[type][0], 0);    // needed for removing the old icon
                UpdateWorldState(BG_IC_Gate_WS[type][1], 1);
                GateStatus[type] = BG_IC_GO_GATES_DAMAGED;      // correct icon is at "damage" state (not flag), not "destroy"
                RewardHonorToTeam(85, teamIndex == TEAM_INDEX_ALLIANCE ? ALLIANCE : HORDE);
        ...
        case BG_IC_GO_ALLIANCE_GATE_2:
        {
            ...
            if (eventId == 22078)
            {
                SendMessage2ToAll(LANG_BG_IC_A_EAST_GATE_DESTROYED, CHAT_MSG_BG_SYSTEM_ALLIANCE, player, spellId);
                UpdateWorldState(BG_IC_Gate_WS[type][0], 0);
                UpdateWorldState(BG_IC_Gate_WS[type][1], 1);
                GateStatus[type] = BG_IC_GO_GATES_DAMAGED;
                RewardHonorToTeam(85, teamIndex == TEAM_INDEX_ALLIANCE ? ALLIANCE : HORDE);
                SpawnEvent(IC_EVENT_ADD_A_BOSS, 0, true);
                SpawnEvent(IC_EVENT_ADD_A_GUARDS, 0, true);
        ...
        case BG_IC_GO_ALLIANCE_GATE_3:
        {
            ...
            if (eventId == 22080)
            {
                SendMessage2ToAll(LANG_BG_IC_A_FRONT_GATE_DESTROYED, CHAT_MSG_BG_SYSTEM_ALLIANCE, player, spellId);
                UpdateWorldState(BG_IC_Gate_WS[type][0], 0);
                UpdateWorldState(BG_IC_Gate_WS[type][1], 1);
                GateStatus[type] = BG_IC_GO_GATES_DAMAGED;
                RewardHonorToTeam(85, teamIndex == TEAM_INDEX_ALLIANCE ? ALLIANCE : HORDE);
        ...
        case BG_IC_GO_HORDE_GATE_1:
        {
            ...
            if (eventId == 22081)
            {
                SendMessage2ToAll(LANG_BG_IC_H_WEST_GATE_DESTROYED, CHAT_MSG_BG_SYSTEM_HORDE, player, spellId);
                UpdateWorldState(BG_IC_Gate_WS[type][0], 0);
                UpdateWorldState(BG_IC_Gate_WS[type][1], 1);
                GateStatus[type] = BG_IC_GO_GATES_DAMAGED;
                RewardHonorToTeam(85, teamIndex == TEAM_INDEX_ALLIANCE ? ALLIANCE : HORDE);
        ...
        case BG_IC_GO_HORDE_GATE_2:
        {
            ...
            if (eventId == 22083)
            {
                SendMessage2ToAll(LANG_BG_IC_H_EAST_GATE_DESTROYED, CHAT_MSG_BG_SYSTEM_HORDE, player, spellId);
                UpdateWorldState(BG_IC_Gate_WS[type][0], 0);
                UpdateWorldState(BG_IC_Gate_WS[type][1], 1);
                GateStatus[type] = BG_IC_GO_GATES_DAMAGED;
                RewardHonorToTeam(85, teamIndex == TEAM_INDEX_ALLIANCE ? ALLIANCE : HORDE);
        ...
        case BG_IC_GO_HORDE_GATE_3:
        {
            ...
            if (eventId == 22079)
            {
                SendMessage2ToAll(LANG_BG_IC_H_FRONT_GATE_DESTROYED, CHAT_MSG_BG_SYSTEM_HORDE, player, spellId);
                UpdateWorldState(BG_IC_Gate_WS[type][0], 0);
                UpdateWorldState(BG_IC_Gate_WS[type][1], 1);
                GateStatus[type] = BG_IC_GO_GATES_DAMAGED;
                RewardHonorToTeam(85, teamIndex == TEAM_INDEX_ALLIANCE ? ALLIANCE : HORDE);
...
bool BattleGroundIC::hasAllNodes(TeamIndex team)
{
    for (int i = BG_IC_NODE_DOCKS; i <= BG_IC_NODE_REFINERY; ++i)
        if (m_Nodes[i] != BG_IC_NODE_TYPE_OCCUPIED + uint8(team))
            return false;
...
uint32 BattleGroundIC::GetPlayerScore(Player *Source, uint32 type)
{
    ...
    {
        case SCORE_BASES_ASSAULTED:
            return ((BattleGroundICScore*)itr->second)->BasesAssaulted;
        case SCORE_BASES_DEFENDED:
            return ((BattleGroundICScore*)itr->second)->BasesDefended;
    }

    return BattleGround::GetPlayerScore(Source, type);
}

void BattleGroundIC::HandlePlayerLogin(Player* player)
{
    SendTransportsInit(player);
}
