...
void BattleGroundAV::HandleQuestComplete(uint32 questid, Player *player)
{
    ...
    ERROR_LOG("BattleGroundAV: Quest %i completed", questid);
    switch(questid)
    {
...
void BattleGroundAV::EventPlayerDefendsPoint(Player* player, BG_AV_Nodes node)
{
    ...
    {
        ERROR_LOG("BattleGroundAV: player defends point which doesn't belong to his team %i", node);
        return;
    }
...
uint32 BattleGroundAV::GetPlayerScore(Player *Source, uint32 type)
{
    ...
    {
        case SCORE_GRAVEYARDS_ASSAULTED:
            return ((BattleGroundAVScore*)itr->second)->GraveyardsAssaulted;
        case SCORE_GRAVEYARDS_DEFENDED:
            return ((BattleGroundAVScore*)itr->second)->GraveyardsDefended;
        case SCORE_TOWERS_ASSAULTED:
            return ((BattleGroundAVScore*)itr->second)->TowersAssaulted;
        case SCORE_TOWERS_DEFENDED:
            return ((BattleGroundAVScore*)itr->second)->GraveyardsDefended;
        case SCORE_SECONDARY_OBJECTIVES:
            return ((BattleGroundAVScore*)itr->second)->SecondaryObjectives;
    }

    return BattleGround::GetPlayerScore(Source, type);
}

void BattleGroundAV::CheckBuggers()
{
    if (GetStatus() == STATUS_WAIT_JOIN)
    {
        for (BattleGroundPlayerMap::const_iterator itr = GetPlayers().begin(); itr != GetPlayers().end(); ++itr)
        {
            if (Player* plr = sObjectMgr.GetPlayer(itr->first))
            {
                ...
                // horde buggers
                if (plr->GetPositionX() < -267.86f)
                {
                    if (plr->GetPositionX() > -1367.63f)
                        plr->TeleportTo(GetMapId(), -1437.54f, -609.76f, 52.16f, plr->GetOrientation());
                }
                // alliance buggers
                else
                {
                    if (plr->GetPositionX() < 777.61f)
                        plr->TeleportTo(GetMapId(), 873.0f, -491.28f, 97.54f, plr->GetOrientation());
                }
            }
        }
    }
