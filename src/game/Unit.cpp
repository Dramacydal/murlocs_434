    ...
    switch(spellProto->GetSpellFamilyName())
    {
        ...
        case SPELLFAMILY_DEATHKNIGHT:
        {
            ...
            if (classOptions && classOptions->SpellFamilyFlags & UI64LIT(0x0000000200000002))
            {
                ...
                {
                    if ((found && (*i)->GetSpellProto()->GetEffectMiscValue((*i)->GetEffIndex()) == 7244) || //Glacier Rot
                        ((classOptions && classOptions->SpellFamilyFlags & UI64LIT(0x2)) && (*i)->GetSpellProto()->SpellIconID == 2721))                       //Improved Icy Touch
                        DoneTotalMod *= ((*i)->GetModifier()->m_amount+100.0f) / 100.0f;
                }
            }
...
int32 Unit::SpellBaseDamageBonusDone(SpellSchoolMask schoolMask)
{
    ...
    if (GetTypeId() == TYPEID_PLAYER)
    {
        ...
        int32 auraMod = GetTotalAuraModifier(SPELL_AURA_MOD_INCREASE_SPELL_POWER_PCT);
        if (auraMod < -100)
            auraMod = -100;

        DoneAdvertisedBenefit = int32(DoneAdvertisedBenefit * (100.0f + auraMod) / 100.0f);
