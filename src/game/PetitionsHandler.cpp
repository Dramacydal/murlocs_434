...
void WorldSession::HandlePetitionShowSignOpcode(WorldPacket & recv_data)
{
    ...
    if(!result)
    {
        ERROR_LOG("Can't find petition with guid = %u.", petitionguid_low);
        return;
    }
    Field *fields = result->Fetch();
...
void WorldSession::HandlePetitionSignOpcode(WorldPacket & recv_data)
{
    ...
    if(!result)
    {
        ERROR_LOG("any petition on server...");
        return;
    }
...
void WorldSession::HandleTurnInPetitionOpcode(WorldPacket & recv_data)
{
    ...
    {
        ERROR_LOG("CMSG_TURN_IN_PETITION: petition table not have data for guid %u!", petitionGuid.GetCounter());
        return;
    }
    ...
    else                                                    // or arena team
    {
        ...
        {
            ERROR_LOG("PetitionsHandler: arena team create failed.");
            delete at;
            delete result;
            return;
        }
