...
int WorldSocket::SendPacket (const WorldPacket& pct)
{
    ...
    else
    {
        ...
        {
            ERROR_LOG("WorldSocket::SendPacket enqueue_tail");
            //mb->release();
            return -1;
        }
    }
...
int WorldSocket::handle_output_queue (GuardType& g)
{
    ...
    {
        ERROR_LOG("WorldSocket::handle_output_queue dequeue_head");
        return -1;
    }
    ...
    else if (n < (ssize_t)send_len) //now n > 0
    {
        ...
        {
            ERROR_LOG("WorldSocket::handle_output_queue enqueue_head");
            mblk->release();
            return -1;
        }
...
int WorldSocket::ProcessIncoming (WorldPacket* new_pct)
{
    ...
    {
        ERROR_LOG( "SESSION: received nonexistent opcode 0x%.4X", opcode);
        return -1;
    }
    ...
    DUMP_WORLD_PACK(uint32(get_handle()), new_pct->GetOpcode(), LookupOpcodeName(new_pct->GetOpcode()), new_pct, true);
    ...
    {
        ERROR_LOG("WorldSocket::ProcessIncoming ByteBufferException occured while parsing an instant handled packet (opcode: %u) from client %s, accountid=%i.",
