    ...
    loot(this)
{
    ...
    m_valuesCount_335 = CORPSE_END_335;
...
bool Corpse::Create( uint32 guidlow, Player *owner)
{
    ...
    if(!IsPositionValid())
    {
        ERROR_LOG("Corpse (guidlow %d, owner %s) not created. Suggested coordinates isn't valid (X: %f Y: %f)",
...
void Corpse::DeleteBonesFromWorld()
{
    ...
    if (!corpse)
    {
        ERROR_LOG("Bones %u not found in world.", GetGUIDLow());
        return;
    }
...
bool Corpse::LoadFromDB(uint32 lowguid, Field *fields)
{
    ...
    {
        ERROR_LOG("%s Owner %s have wrong corpse type (%i), not load.", GetGuidStr().c_str(), GetOwnerGuid().GetString().c_str(), m_type);
        return false;
    }
    ...
    if(!info)
    {
        ERROR_LOG("Player %u has incorrect race/class pair.", GetGUIDLow());
        return false;
    }
    ...
    if(!IsPositionValid())
    {
        ERROR_LOG("%s Owner %s not created. Suggested coordinates isn't valid (X: %f Y: %f)",
