...
void AuctionHouseMgr::SendAuctionWonMail(AuctionEntry *auction)
{
    ...
    if (sWorld.getConfig(CONFIG_BOOL_GM_LOG_TRADE))
    {
        ...
        else
        {
            ...
            if (bidder_security > SEC_CURATOR)               // not do redundant DB requests
            {
        ...
        if (bidder_security > SEC_CURATOR)
        {
    ...
    if (bidder || bidder_accId)
    {
        ...
        if (sWorld.getConfig(CONFIG_BOOL_LOG_AUCTION))
        {
            CharacterDatabase.PExecute("INSERT INTO `auction_log` (`guid`, `seller`, `item`, `itemGuid`, `money`, `type`) VALUES "
                "('%u', '%u', '%u', '" UI64FMTD "', '%u', '%u')",
                auction->bidder, auction->owner, pItem->GetEntry(), pItem->GetObjectGuid().GetCounter(), auction->bid, auction->buyout ? 0 : 1);
        }
    ...
    else
    {
        ...
        CharacterDatabase.PExecute("DELETE FROM item_instance_text WHERE guid='%u'", pItem->GetGUIDLow());
        RemoveAItem(auction->itemGuidLow);                  // we have to remove the item, before we delete it !!
...
{                                                           // return an item in auction to its owner by mail
    ...
    if (!pItem)
    {
        ERROR_LOG("Auction item (GUID: %u) not found, and lost.", auction->itemGuidLow);
        return;
    }
...
void AuctionHouseMgr::LoadAuctionItems()
{
    ...
    QueryResult *result = CharacterDatabase.Query("SELECT data,text,itemguid,item_template FROM auction JOIN item_instance ii ON itemguid = ii.guid LEFT JOIN item_instance_text iit ON itemguid = iit.guid");
    ...
    do
    {
        ...
        if (!proto)
        {
            ERROR_LOG("AuctionHouseMgr::LoadAuctionItems: Unknown item (GUID: %u id: #%u) in auction, skipped.", item_guid,item_template);
            continue;
        }
    ...
    do
    {
        ...
        else
        {
            ...
            if (!pItem)
            {
                ...
                ERROR_LOG("Auction %u has not a existing item : %u, deleted", auction->Id, auction->itemGuidLow);
                delete auction;
                continue;
            }
...
int AuctionEntry::CompareAuctionEntry(uint32 column, const AuctionEntry *auc, Player* viewPlayer) const
{
    if (!auc)
    {
        ERROR_LOG("CompareAuctionEntry: AuctionEntry is NULL! This should not happen! WTF?");
        return 0;
    }
...
bool AuctionEntry::BuildAuctionInfo(WorldPacket & data) const
{
    ...
    if (!pItem)
    {
        ERROR_LOG("auction to item, that doesn't exist !!!!");
        return false;
    }
    data << uint32(Id);
