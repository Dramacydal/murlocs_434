...
class MANGOS_DLL_SPEC Map : public GridRefManager<NGridType>
{
    ...
    public:
        ...
        void MonsterYellToMap(ObjectGuid guid, int32 textId, uint32 language, Unit* target, uint32 zoneId = 0);
        void MonsterYellToMap(CreatureInfo const* cinfo, int32 textId, uint32 language, Unit* target, uint32 senderLowGuid = 0, uint32 zoneId = 0);
...
#ifdef DUMP_MAP_STAT
        uint32 processionsCount;
        uint32 lagProcCount;
        uint32 summProcessTime;
        uint32 higestProcessTime;
        uint32 normalProcessTime;
        uint32 lastInfoDump;
#endif
