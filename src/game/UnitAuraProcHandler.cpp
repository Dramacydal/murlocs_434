...
pAuraProcHandler AuraProcHandler[TOTAL_AURAS]=
{
    ...
    &Unit::HandleNULLProc,                                  //279 SPELL_AURA_INITIALIZE_IMAGES
    ...
    &Unit::HandleNULLProc,                                  //284 SPELL_AURA_LINKED 51 spells
    ...
    &Unit::HandleNULLProc,                                  //288 SPELL_AURA_MOD_PARRY_FROM_BEHIND_PERCENT
    ...
    &Unit::HandleNULLProc,                                  //293 SPELL_AURA_ADD_MECHANIC_ABILITIES
    &Unit::HandleNULLProc,                                  //294 SPELL_AURA_STOP_NATURAL_MANA_REGEN
    &Unit::HandleNULLProc,                                  //295 unused (3.2.2a)
    &Unit::HandleNULLProc,                                  //296 SPELL_AURA_SET_VEHICLE_ID
    ...
    &Unit::HandleNULLProc,                                  //301 SPELL_AURA_HEAL_ABSORB
    &Unit::HandleNULLProc,                                  //302 unused (3.2.2a)
    &Unit::HandleNULLProc,                                  //303 SPELL_AURA_MOD_DAMAGE_DONE_VERSUS_AURASTATE
    &Unit::HandleNULLProc,                                  //304 SPELL_AURA_MOD_FAKE_INEBRIATE
...
bool Unit::IsTriggeredAtSpellProcEvent(Unit *pVictim, SpellAuraHolder* holder, SpellEntry const* procSpell, uint32 procFlag, uint32 procExtra, WeaponAttackType attType, bool isVictim, SpellProcEventEntry const*& spellProcEvent )
{
    ...
    if(!isVictim && GetTypeId() == TYPEID_PLAYER)
    {
        if(spellProto->EquippedItemClass == ITEM_CLASS_WEAPON)
        {
            ...
            if(!item || item->IsBroken() || !CanUseEquippedWeapon(attType) || item->GetProto()->Class != ITEM_CLASS_WEAPON || !((1<<item->GetProto()->SubClass) & spellProto->EquippedItemSubClassMask))
                return false;
        }
    ...
    if (spellProto->Id == 44404 || spellProto->Id == 54486 || spellProto->Id == 54488 || spellProto->Id == 54489 || spellProto->Id == 54490)
        ...
        if (procSpell && procSpell->SpellFamilyFlags & UI64LIT(0x367FE00000021))
            chance /= 2;
    ...
    switch(dummySpell->SpellFamilyName)
    {
        ...
        case SPELLFAMILY_WARRIOR:
        {
            ...
            else if (dummySpell->SpellIconID == 3214)
            {
                ...
                basepoints[0] = GetShieldBlockValue(true) * triggerAmount / 100;
                break;
            }
            // Sweeping Strikes
        ...
        case SPELLFAMILY_ROGUE:
        {
            switch(dummySpell->Id)
            {
                ...
                case 56800:
                {
                    if (!procSpell || procSpell->SpellFamilyName != SPELLFAMILY_ROGUE || procSpell->SpellIconID != 243) // Amaru: e?eaua oeaae o aaenoaaa!
                        return SPELL_AURA_PROC_FAILED;
        ...
        case SPELLFAMILY_SHAMAN:
        {
            ...
            if (dummySpell->SpellFamilyFlags & UI64LIT(0x0000040000000000))
            {
                ...
                if (caster)
                {
                    ...
                    {
                        if ((*i)->GetSpellProto()->SpellFamilyName == SPELLFAMILY_SHAMAN && ((*i)->GetSpellProto()->SpellIconID == 19 || (*i)->GetSpellProto()->SpellIconID == 2015) && (*i)->GetEffIndex() == EFFECT_INDEX_1)
                        {
        ...
        case SPELLFAMILY_DEATHKNIGHT:
        {
            ...
            if (dummySpell->SpellIconID == 1614)
            {
                ...
                if (procSpell->Dispel != DISPEL_DISEASE)
                    return SPELL_AURA_PROC_FAILED; 
...
SpellAuraProcResult Unit::HandleProcTriggerSpellAuraProc(Unit *pVictim, uint32 damage, uint32 absorb, Aura* triggeredByAura, SpellEntry const *procSpell, uint32 procFlags, uint32 procEx, uint32 cooldown)
{
    ...
    switch (auraSpellInfo->SpellFamilyName)
    {
        ...
        case SPELLFAMILY_HUNTER:
        {
            ...
            {
                if (!procSpell || !(procSpell->SpellFamilyFlags & UI64LIT(0x0000000100020000))    //Aimed Shot & Steady Shot
    ...
    switch(trigger_spell_id)
    {
        ...
        case 53817:
        {
            ...
            if (Aura* mw = GetAura(53817, EFFECT_INDEX_0))
                if (Aura* dummy = GetDummyAura(70832))
                    if (mw->GetStackAmount() == mw->GetSpellProto()->StackAmount && roll_chance_i(dummy->GetBasePoints()))
        ...
        case 56453:
        {
            ...
            else if (procSpell->SpellFamilyFlags & UI64LIT(0x0000000000000004))
                return SPELL_AURA_PROC_FAILED;
...
SpellAuraProcResult Unit::HandleAddPctModifierAuraProc(Unit* /*pVictim*/, uint32 /*damage*/, uint32 /*absorb*/, Aura* triggeredByAura, SpellEntry const *procSpell, uint32 /*procFlag*/, uint32 procEx, uint32 /*cooldown*/)
{
    ...
    switch(spellInfo->SpellFamilyName)
    {
        ...
        case SPELLFAMILY_SHAMAN:
        {
            ...
            {
                if (!procSpell || (procSpell->SpellFamilyFlags & UI64LIT(0x8000000001C3)) == 0)
                    return SPELL_AURA_PROC_FAILED;
            }
            break;
        }
...
SpellAuraProcResult Unit::HandleRemoveByDamageProc(Unit* pVictim, uint32 damage, uint32 absorb, Aura* triggeredByAura, SpellEntry const *procSpell, uint32 procFlag, uint32 procEx, uint32 cooldown)
{
    ...
    if (!triggeredByAura->GetSpellProto()->procFlags)
        return SPELL_AURA_PROC_FAILED;
...
{
    switch (triggeredByAura->GetSpellProto()->SpellFamilyName)
    {
        case SPELLFAMILY_GENERIC:
        {
