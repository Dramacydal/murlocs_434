...
void Creature::Update(uint32 update_diff, uint32 diff)
{
    switch( m_deathState )
    {
        case JUST_ALIVED:
            ...
            ERROR_LOG("Creature (GUIDLow: %u Entry: %u ) in wrong state: JUST_ALIVED (4)",GetGUIDLow(),GetEntry());
            break;
        case JUST_DIED:
            // Don't must be called, see Creature::SetDeathState JUST_DIED -> CORPSE promoting.
            ERROR_LOG("Creature (GUIDLow: %u Entry: %u ) in wrong state: JUST_DEAD (1)",GetGUIDLow(),GetEntry());
            break;
        case DEAD:
        {
...
void Creature::RegenerateHealth()
{
    ...
    // Valithria should not regenerate
    switch(GetEntry())
    {
        case 36789:
        case 37950:
...
void Creature::SaveToDB()
{
    ...
    if(!data)
    {
        ERROR_LOG("Creature::SaveToDB failed, cannot get creature data!");
        return;
    }
...
void Creature::SaveToDB(uint32 mapid, uint8 spawnMask, uint32 phaseMask)
{
    ...
    {
        uint32 mapId = trans->GetGOInfo()->moTransport.mapID;
        if (!mapId)
            return;

        Transport::Passenger* pass = trans->FindPassenger(GetObjectGuid());
        if (!pass)
            return;

        TransportCreatureInfo tinfo;
        tinfo.guid          = 0;
        tinfo.entry         = cinfo->Entry;
        tinfo.map           = mapId;
        tinfo.position_x    = GetTransOffsetX();
        tinfo.position_y    = GetTransOffsetY();
        tinfo.position_z    = GetTransOffsetZ();
        tinfo.orientation   = GetTransOffsetO();
        tinfo.emote         = 0;

        TransportCreatureList& crlist = sMapMgr.m_TransportCreatures[mapId];
        for (TransportCreatureList::iterator itr = crlist.begin(); itr != crlist.end(); ++itr)
        {
            if (itr->guid == pass->localGuid)
            {
                tinfo.emote = itr->emote;
                tinfo.guid = itr->guid;
                crlist.erase(itr);
                break;
            }
            else if (itr->guid >= tinfo.guid)
                tinfo.guid = itr->guid + 1;
        }
        crlist.push_back(tinfo);

        WorldDatabase.PExecute("DELETE FROM `creature_transport` WHERE `guid` = %u AND `map` = %u", tinfo.guid, tinfo.map);
        WorldDatabase.PExecute("INSERT INTO `creature_transport` (`guid`, `entry`, `map`, `position_x`, `position_y`, `position_z`, `orientation`, `emote`) VALUE "
            "(%u, %u, %u, %f, %f, %f, %f, %u)", tinfo.guid, tinfo.entry, tinfo.map, tinfo.position_x, tinfo.position_y, tinfo.position_z, tinfo.orientation, tinfo.emote);

        pass->localGuid = tinfo.guid;
        return;
    }
...
void Creature::DeleteFromDB()
{
    if (Transport* trans = GetTransport())
    {
        if (uint32 transMap = GetTransport()->GetGOInfo()->moTransport.mapID)
        {
            if (Transport::Passenger* pass = trans->FindPassenger(GetObjectGuid()))
            {
                WorldDatabase.PExecute("DELETE FROM `creature_transport` WHERE `guid` = %u AND `map` = %u",
                    pass->localGuid, transMap);
                TransportCreatureMap::const_iterator itr = sMapMgr.m_TransportCreatures.find(transMap);
                if (itr != sMapMgr.m_TransportCreatures.end())
                {
                    TransportCreatureList crlist = itr->second;
                    for (TransportCreatureList::iterator itr2 = crlist.begin(); itr2 != crlist.end(); ++itr2)
                    {
                        if (itr2->guid == pass->localGuid)
                        {
                            crlist.erase(itr2);
                            break;
                        }
...
SpellEntry const *Creature::ReachWithSpellAttack(Unit *pVictim)
{
    ...
    for(uint32 i = 0; i < CREATURE_MAX_SPELLS; ++i)
    {
        ...
        if(!spellInfo)
        {
            ERROR_LOG("WORLD: unknown spell id %i", m_spells[i]);
            continue;
        }
