...
void RealmList::UpdateRealms(bool init)
{
    ...
    LoginDatabase.Execute("UPDATE account_banned SET active = 0 WHERE unbandate<=UNIX_TIMESTAMP(now()) AND unbandate<>bandate AND active = 1");
    LoginDatabase.Execute("DELETE FROM ip_banned WHERE unbandate<=UNIX_TIMESTAMP(now()) AND unbandate<>bandate");
    ...
    if(result)
    {
        do
        {
            ...
            {
                ERROR_LOG("Realm (id %u, name '%s') can only be flagged as OFFLINE (mask 0x02), NEWPLAYERS (mask 0x20), RECOMMENDED (mask 0x40), or SPECIFICBUILD (mask 0x04) in DB", Id, name.c_str());
