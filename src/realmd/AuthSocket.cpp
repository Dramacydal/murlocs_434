            ...
            if(QueryResult *loginfail = LoginDatabase.PQuery("SELECT id, failed_logins FROM account WHERE username = '%s'", _safelogin.c_str()))
            {
                ...
                if( failed_logins >= MaxWrongPassCount )
                {
                    ...
                    if(WrongPassBanType)
                    {
                        ...
                        LoginDatabase.PExecute("INSERT INTO account_banned VALUES ('%u',UNIX_TIMESTAMP(),UNIX_TIMESTAMP()+'%u','PassGuard','Failed login autoban',1)",
                    ...
                    else
                    {
                        ...
                        LoginDatabase.PExecute("INSERT INTO ip_banned VALUES ('%s',UNIX_TIMESTAMP(),UNIX_TIMESTAMP()+'%u','PassGuard','Failed login autoban')",
...
bool AuthSocket::_HandleReconnectChallenge()
{
    ...
    if (!result)
    {
        ERROR_LOG("[ERROR] user %s tried to login and we cannot find his session key in the database.", _login.c_str());
        close_connection();
        return false;
    }
...
bool AuthSocket::_HandleReconnectProof()
{
    ...
    {
        ERROR_LOG("[ERROR] user %s tried to login, but session invalid.", _login.c_str());
        close_connection();
        return false;
    }
}
...
bool AuthSocket::_HandleRealmList()
{
    ...
    if(!result)
    {
        ERROR_LOG("[ERROR] user %s tried to login and we cannot find him in the database.",_login.c_str());
        close_connection();
        return false;
    }
