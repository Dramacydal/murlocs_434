...
class MANGOS_DLL_SPEC QueryResult
{
    public:
#ifdef SQL_STAT_COLLECT
        QueryResult(uint64 rowCount, uint32 fieldCount, void* caller)
            : mFieldCount(fieldCount), mRowCount(rowCount) { fetchTimer = 0; fetchCount = 0; lifetime = getMSTime(); srcDb = caller; }
#else
