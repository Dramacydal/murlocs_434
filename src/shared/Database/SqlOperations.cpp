...
bool SqlQueryHolder::SetQuery(size_t index, const char *sql)
{
    ...
    {
        ERROR_LOG("Query index (" SIZEFMTD ") out of range (size: " SIZEFMTD ") for query: %s", index, m_queries.size(), sql);
        return false;
    }

    if(m_queries[index].first != NULL)
    {
        ERROR_LOG("Attempt assign query to holder index (" SIZEFMTD ") where other query stored (Old: [%s] New: [%s])",
...
bool SqlQueryHolder::SetPQuery(size_t index, const char *format, ...)
{
    if(!format)
    {
        ERROR_LOG("Query (index: " SIZEFMTD ") is empty.",index);
        return false;
    }
