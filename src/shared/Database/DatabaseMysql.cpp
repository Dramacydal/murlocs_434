...
DatabaseMysql::DatabaseMysql()
{
    ...
    if( db_count++ == 0 )
    {
        ...
        {
            ERROR_LOG("FATAL ERROR: Used MySQL library isn't thread-safe.");
...
bool MySQLConnection::Initialize(const char *infoString)
{
    ...
    if (!mysqlInit)
    {
        ERROR_LOG( "Could not initialize Mysql connection" );
        return false;
    }
    ...
    //  #if MYSQL_VERSIOB_ID >= 50013
            ...
            {
                DETAIL_LOG("Failed to turn on MYSQL_OPT_RECONNECT.");
            }
            else
            {
                DETAIL_LOG("Successfully turned on MYSQL_OPT_RECONNECT.");
            }
    #else
            DETAIL_LOG("Your mySQL client lib version does not support reconnecting after a timeout.");
            DETAIL_LOG("If this causes you any trouble we advice you to upgrade");
            DETAIL_LOG("your mySQL client libs to at least mySQL 5.0.13 to resolve this problem.");
    #endif
...
bool MySQLConnection::_Query(const char *sql, MYSQL_RES **pResult, MYSQL_FIELD **pFields, uint64* pRowCount, uint32* pFieldCount)
{
    ...
    uint32 tmpPerfTime = WorldTimer::getMSTimeDiff(_s, getMSTime());
    realQueryPerformTime += tmpPerfTime;
    if (tmpPerfTime > slowestQueryTime)
    {
        sprintf(slowestQueryData, "%s", sql);
        slowestQueryTime = tmpPerfTime;
    }
    ++realQueryCnt;
#endif
...
#ifdef SQL_STAT_COLLECT
    QueryResultMysql *queryResult = new QueryResultMysql(result, fields, rowCount, fieldCount, this);
#else
...
#ifdef SQL_STAT_COLLECT 
        uint32 tmpPerfTime = getMSTimeDiff(_s, getMSTime());
        realQueryPerformTime += tmpPerfTime;
        if (tmpPerfTime > slowestQueryTime)
        {
            sprintf(slowestQueryData, "%s", sql);
...
bool MySQLConnection::_TransactionCmd(const char *sql)
{
    ...
    {
        ERROR_LOG("SQL: %s", sql);
        ERROR_LOG("SQL ERROR: %s", mysql_error(mMysql));
        return false;
    }
    else
    {
...
#ifdef SQL_STAT_COLLECT 
	realQueryPerformTime += getMSTimeDiff(_s, getMSTime());
#endif
    return true;
}
