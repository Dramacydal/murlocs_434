...
uint32 SQLStorage::GetTotalSize() const
{
    ...
    for (uint32 x = 0; x < GetSrcFieldCount(); ++x)
    {
        if (GetDstFormat(x) == FT_STRING)
        {
            ...
            // FIXME
            //for (uint32 y = 0; y < GetMaxEntry(); ++y)
            //    if (const char * const * str = LookupEntry<const char*>(y))
            ...
            //        line_size += strlen(*str) * sizeof(char);
        ...
        else if (GetDstFormat(x) == FT_LOGIC)
            offset += sizeof(bool);
        else if (GetDstFormat(x) == FT_BYTE)
            offset += sizeof(char);
        else
            offset += 4;
    }
    ...
    return (offset * GetRecordCount() + line_size);
}
