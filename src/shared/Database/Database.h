...
class MANGOS_DLL_SPEC Database
{
        ...
        void DumpQueryEnd(uint32 lifetime, uint32 fetch, uint32 fetchCnt)
        {
            ++queryFinishedCount;
            queryLifetime += lifetime;
            fetchTime += fetch;
            fetchesCount += fetchCnt;
        }
        void DumpInfo(uint32 online, uint32 realmId, uint32 dbName, Database* dumpTo)
        {
            //megai2: recalc every 1m
            if (getMSTimeDiff(recalcTimer, getMSTime()) >= 60000)
            {
                float scale = getMSTimeDiff(recalcTimer, getMSTime()) / 60000.0f;
                longQueryes = queryCount - queryFinishedCount;
                queryAvgLifetime = queryLifetime / (queryFinishedCount + 1);
                queryPerSecond = queryCount / int(60 * scale);
                fetchAvgTime = fetchTime / (fetchesCount+1);
                fetchPerSecond = fetchesCount / int(60 * scale);
                recalcTimer = getMSTime();
                realQueryAvgPerformTime = realQueryPerformTime / (realQueryCnt+1);
                queryCount = 0;
                queryFinishedCount = 0;
                queryLifetime = 0;
                fetchTime = 0;
                fetchesCount = 0;
                realQueryPerformTime = 0;
                realQueryCnt = 0;
                char escapedDt[MAX_QUERY_LEN];
                escape_string(escapedDt, slowestQueryData, strlen(slowestQueryData));
                dumpTo->PExecute("INSERT INTO db_stat (longQueryName, longQueryTime, queryAvgLifetime, longQueryes, queryPerSecond, fetchAvgTime, fetchPerSecond, realQueryAvgPerformTime, online, realmId, dbName) VALUES (\"%s\", %u, %u, %i, %u, %u, %u, %u, %u, %u, %u)",
                escapedDt, slowestQueryTime, queryAvgLifetime, longQueryes, queryPerSecond, fetchAvgTime, fetchPerSecond, realQueryAvgPerformTime, online, realmId, dbName);
                slowestQueryTime = 0;
            }
        }
        uint32 getQueryPerSecond()
        {
            return queryPerSecond;
        }

        uint32 getQueryAvgLifetime()
        {
            return queryAvgLifetime;
        }

        int32 getLongQueryes()
        {
            return longQueryes;
        }

        uint32 getFetchAvgTime()
        {
            return fetchAvgTime;
        }

        uint32 getFetchPerSecond()
        {
        return fetchPerSecond;
        }

        uint32 getRealQueryAvgPerformTime()
        {
            return realQueryAvgPerformTime;
        }
#endif
    ...
    protected:
        ...
        //megai2: collect part
        uint32 realQueryCnt;
        uint32 realQueryPerformTime;
        uint32 queryCount;
        uint32 queryFinishedCount;
        uint32 queryLifetime;
        uint32 fetchTime;
        uint32 fetchesCount;
        uint32 recalcTimer;
        //megai2: static part
        uint32 queryPerSecond;
        uint32 queryAvgLifetime;
        int32 longQueryes;
        uint32 fetchAvgTime;
        uint32 fetchPerSecond;
        uint32 realQueryAvgPerformTime;
        uint32 slowestQueryTime;
        char slowestQueryData[MAX_QUERY_LEN];
#endif
