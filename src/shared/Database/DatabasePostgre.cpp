...
DatabasePostgre::DatabasePostgre()
{
    ...
    if( db_count++ == 0 )
    {
        if (!PQisthreadsafe())
        {
            ERROR_LOG("FATAL ERROR: PostgreSQL libpq isn't thread-safe.");
            exit(1);
        }
    }
}
...
bool PostgreSQLConnection::Initialize(const char *infoString)
{
    ...
    {
        ERROR_LOG( "Could not connect to Postgre database at %s: %s",
...
bool PostgreSQLConnection::_TransactionCmd(const char *sql)
{
    ...
    {
        ERROR_LOG("SQL: %s", sql);
        ERROR_LOG("SQL ERROR: %s", PQerrorMessage(mPGconn));
        return false;
    }
    else
    {
