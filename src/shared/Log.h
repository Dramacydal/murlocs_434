...
#define DEBUG_LOG_IN_LINE(...)                          \
    do {                                                \
        if (sLog.HasLogLevelOrHigher(LOG_LVL_DEBUG))    \
            sLog.outDebugInLine(__VA_ARGS__);           \
    } while(0)          
...
#define DUMP_WORLD_PACK(socket, opcode, opcodeName, packet, incoming) sLog.outWorldPacketDump(socket, opcode, opcodeName, packet, incoming)
