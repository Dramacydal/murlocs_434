...
struct MANGOS_DLL_DECL boss_aranAI : public ScriptedAI
{
    ...
    void SpellHit(Unit* pAttacker, const SpellEntry* Spell)
    {
        ...
        if ((Spell->GetSpellEffectIdByIndex(EFFECT_INDEX_0) != SPELL_EFFECT_INTERRUPT_CAST &&
            Spell->GetSpellEffectIdByIndex(EFFECT_INDEX_1) != SPELL_EFFECT_INTERRUPT_CAST &&
            Spell->GetSpellEffectIdByIndex(EFFECT_INDEX_2)) || !m_creature->IsNonMeleeSpellCasted(false))
            return;
