...
struct MANGOS_DLL_DECL boss_halazziAI : public ScriptedAI
{
    ...
    {
        if (pSpell->GetEffectApplyAuraNameByIndex(EFFECT_INDEX_0) != SPELL_AURA_TRANSFORM)
            return;

        // possibly hack and health should be set by Aura::HandleAuraTransform()
        if (const CreatureInfo* pInfo = GetCreatureTemplateStore(pSpell->GetEffectMiscValue(EFFECT_INDEX_0)))
            DoUpdateStats(pInfo);
