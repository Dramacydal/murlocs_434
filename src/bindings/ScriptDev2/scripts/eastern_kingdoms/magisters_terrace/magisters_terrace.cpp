...
struct MANGOS_DLL_DECL npc_kalecgosAI : public ScriptedAI
{
    ...
    void DoWorkaroundForQuestCredit()
    {
        ...
        if (SpellEntry const* pSpell = GetSpellStore()->LookupEntry(SPELL_ORB_KILL_CREDIT))
        {
            for(Map::PlayerList::const_iterator i = lList.begin(); i != lList.end(); ++i)
            {
                if (Player* pPlayer = i->getSource())
                {
                    if (uint32 val = pSpell->GetEffectMiscValue(EFFECT_INDEX_0))
                        pPlayer->KilledMonsterCredit(val);
                }
            }
        }
    }
