...
struct MANGOS_DLL_DECL boss_jeklikAI : public ScriptedAI
{
    ...
    void UpdateAI(const uint32 uiDiff)
    {
        ...
        if (m_bIsPhaseOne)
        {
            ...
            if (m_uiSpawnBatsTimer < uiDiff)
            {
                ...
                for (uint8 i = 0; i < 6; ++i)
                {
                    ...
                    m_creature->SummonCreature(NPC_BLOODSEEKER_BAT, fNewX, fNewY, fNewZ, fO, TEMPSUMMON_TIMED_OOC_DESPAWN, 15000);
                }
        ...
        else                                                // Phase Two
        {
            ...
            {
                if (Unit* pTarget = m_creature->SelectAttackingTarget(ATTACKING_TARGET_RANDOM, 0))
                    m_creature->SummonCreature(NPC_FRENZIED_BAT, pTarget->GetPositionX(), pTarget->GetPositionY(), pTarget->GetPositionZ() + 15.0f, 0, TEMPSUMMON_TIMED_OOC_DESPAWN, 15000);
