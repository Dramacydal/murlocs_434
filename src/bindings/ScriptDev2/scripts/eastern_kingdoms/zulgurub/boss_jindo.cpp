    ...
    void UpdateAI(const uint32 uiDiff)
    {
        ...
        if (m_uiDelusionsTimer < uiDiff)
        {
            ...
            if (DoCastSpellIfCan(pTarget, SPELL_DELUSIONS_OF_JINDO) == CAST_OK)
            {
                ...
                if (Creature* pSummoned = m_creature->SummonCreature(NPC_SHADE_OF_JINDO, fX, fY, fZ, 0, EMPSUMMON_TIMED_OOC_DESPAWN, 15000))
        ...
        if (m_uiTeleportTimer < uiDiff)
        {
            if (Unit* pTarget = m_creature->SelectAttackingTarget(ATTACKING_TARGET_RANDOM, 0))
            {
                ...
                for (uint8 i = 0; i < MAX_SKELETONS; ++i)
                {
                    ...
                    if (Creature* pSummoned = m_creature->SummonCreature(NPC_SACRIFICED_TROLL, fX, fY, fZ, 0.0f, EMPSUMMON_TIMED_OOC_DESPAWN, 15000))
                        pSummoned->AI()->AttackStart(pTarget);
                }
