...
struct MANGOS_DLL_DECL npc_gnome_citizen_motivatedAI : public FollowerAI
{
    npc_gnome_citizen_motivatedAI(Creature* pCreature) : FollowerAI(pCreature) { Reset(); }
    ...
    void MoveInLineOfSight(Unit *pWho)
    {
        ...
        if (!m_creature->getVictim() && !HasFollowState(STATE_FOLLOW_COMPLETE) && pWho->GetEntry() == NPC_CAPTAIN_TREAD_SPARKNOZZLE)
        {
            if (m_creature->IsWithinDistInMap(pWho, INTERACTION_DISTANCE))
            {
                if (Player* pPlayer = GetLeaderForFollower())
                {
                    if (pPlayer->GetQuestStatus(QUEST_A_FEW_GOOD_GNOMES) == QUEST_STATUS_INCOMPLETE)
                        pPlayer->KilledMonsterCredit(QUEST_CREDIT_MOTIVATED_CITIZEN);
                }
                SetFollowComplete();
            }
        }
    ...
    void UpdateFollowerAI(const uint32 uiDiff)
    {
        if (Player* pPlayer = GetLeaderForFollower())
        {
            if (!m_creature->IsWithinDistInMap(pPlayer, 70.0f))
                SetFollowComplete();
        }
        else
            SetFollowComplete();
...
CreatureAI* GetAI_npc_gnome_citizen_motivated(Creature* pCreature)
{
    return new npc_gnome_citizen_motivatedAI(pCreature);
}

struct MANGOS_DLL_DECL npc_gnome_citizenAI : public ScriptedAI
{
    npc_gnome_citizenAI(Creature* pCreature) : ScriptedAI(pCreature) {Reset();}
    ...
    void SpellHit(Unit* pWho, const SpellEntry* pSpell)
    {
        ...
        if (pSpell->Id == SPELL_MOTIVATE && pPlayer->GetQuestStatus(QUEST_A_FEW_GOOD_GNOMES) == QUEST_STATUS_INCOMPLETE)
        {
            if (Creature* pCreature = m_creature->SummonCreature(NPC_GNOME_CITIZEN_MOTIVATED, m_creature->GetPositionX(), m_creature->GetPositionY(), m_creature->GetPositionZ(), m_creature->GetOrientation(), TEMPSUMMON_TIMED_DESPAWN, 40*MINUTE*IN_MILLISECONDS))
            {
               if (npc_gnome_citizen_motivatedAI* pGnome_citizen_motivatedAI = dynamic_cast<npc_gnome_citizen_motivatedAI*>(pCreature->AI()))
               {
                   pPlayer->KilledMonsterCredit(QUEST_CREDIT_CITIZEN);
                   pGnome_citizen_motivatedAI->StartFollow(pPlayer);
               }
            }
...
CreatureAI* GetAI_npc_gnome_citizen(Creature* pCreature)
{
    return new npc_gnome_citizenAI(pCreature);
}
...
void AddSC_ironforge()
{
    ...
    pNewScript = new Script;
    pNewScript->Name = "npc_gnome_citizen";
    pNewScript->GetAI = &GetAI_npc_gnome_citizen;
    pNewScript->RegisterSelf();

    pNewScript = new Script;
    pNewScript->Name = "npc_gnome_citizen_motivated";
    pNewScript->GetAI = &GetAI_npc_gnome_citizen_motivated;
    pNewScript->RegisterSelf();
}
