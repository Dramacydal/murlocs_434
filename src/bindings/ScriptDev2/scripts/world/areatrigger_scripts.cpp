...
bool AreaTrigger_at_childrens_week_spot(Player* pPlayer, AreaTriggerEntry const* pAt)
{
    ...
    // Dark Portal
    if (pAt->id == 4356 && pPlayer->GetMiniPet() &&
        (pPlayer->GetMiniPet()->GetEntry() == 22817 || pPlayer->GetMiniPet()->GetEntry() == 22818))
        pPlayer->AreaExploredOrEventHappens(pPlayer->GetTeam() == HORDE ? 10951 : 10952);
