    ...
    switch(uiSpellId)
    {
        case SPELL_ANUNIAQS_NET:
        {
            if (uiEffIndex == EFFECT_INDEX_0)
            {
                ...
                {
                    if (Creature* pShark = pCaster->SummonCreature(NPC_REEF_SHARK, pGOTarget->GetPositionX(), pGOTarget->GetPositionY(), pGOTarget->GetPositionZ(), 0.0f, EMPSUMMON_TIMED_OOC_DESPAWN, 30000))
        ...
        case SPELL_CAST_FISHING_NET:
        {
            if (uiEffIndex == EFFECT_INDEX_0)
            {
                ...
                if (urand(0, 2))
                {
                    if (Creature* pMurloc = pCaster->SummonCreature(NPC_ANGRY_MURLOC, pCaster->GetPositionX(), pCaster->GetPositionY()+20.0f, pCaster->GetPositionZ(), 0.0f, EMPSUMMON_TIMED_OOC_DESPAWN, 10000))
        ...
        case SPELL_ULTRASONIC_SCREWDRIVER:
        {
            if (uiEffIndex == EFFECT_INDEX_0)
            {
                if (pCreatureTarget->IsCorpse())
                {
                    ...
                    if (SpellEntry const * pSpell = GetSpellStore()->LookupEntry(newSpellId))
                    {
                        pCaster->CastSpell(pCreatureTarget, pSpell->Id, true);

                        if (SpellEffectEntry const * effect = pSpell->GetSpellEffect(uiEffIndex))
                            if (Pet* pPet = pCaster->FindGuardianWithEntry(effect->EffectMiscValue))
            ...
            if (uiEffIndex == EFFECT_INDEX_2)
            {
                switch(urand(0,2))
                {
                    ...
                    case 1:
                    {
                        for (int i = 0; i<2; ++i)
                        {
                            if (Creature* pSandGnome = pCaster->SummonCreature(NPC_SAND_GNOME, pCreatureTarget->GetPositionX(), pCreatureTarget->GetPositionY(), pCreatureTarget->GetPositionZ(), 0.0f, EMPSUMMON_TIMED_OOC_DESPAWN, 30000))
                    ...
                    case 2:
                    {
                        for (int i = 0; i<2; ++i)
                        {
                            if (Creature* pMatureBoneSifter = pCaster->SummonCreature(NPC_MATURE_BONE_SIFTER, pCreatureTarget->GetPositionX(), pCreatureTarget->GetPositionY(), pCreatureTarget->GetPositionZ(), 0.0f, EMPSUMMON_TIMED_OOC_DESPAWN, 30000))
