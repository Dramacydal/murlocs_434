...
SD%Complete: 90
SDComment: Quest support: 10503, 10504, 10556, 10609, 10859, 10674. (npc_daranelle needs bit more work before consider complete)
...
struct MANGOS_DLL_DECL npc_bloodmaul_brutebaneAI : public ScriptedAI
{
    npc_bloodmaul_brutebaneAI(Creature* pCreature) : ScriptedAI(pCreature) { Reset(); }
    ...
    void UpdateAI(const uint32 diff)
    {
       ...
       if(Creature* Ogre = GetClosestCreatureWithEntry(m_creature, NPC_OGRE_BRUTE, 50.0f))
       {
...
struct MANGOS_DLL_DECL npc_ogre_bruteAI : public ScriptedAI
{
    npc_ogre_bruteAI(Creature* pCreature) : ScriptedAI(pCreature) { Reset(); }
    ...
    void MoveInLineOfSight(Unit *who)
    {
        ...
        if (m_creature->IsWithinDistInMap(who, 50.0f))
        {
            ...
            {
                if (((Player*)who)->GetQuestStatus(QUEST_GETTING_THE_BLADESPIRE_TANKED) == QUEST_STATUS_INCOMPLETE || ((Player*)who)->GetQuestStatus(QUEST_BLADESPIRE_KEGGER) == QUEST_STATUS_INCOMPLETE)
    ...
    void MovementInform(uint32 type, uint32 id)
    {
        ...
        if (pPlayer && id == 1)
        {
            GameObject* Keg = GetClosestGameObjectWithEntry(m_creature, GO_KEG, 20.0f);
            if (Keg)
                Keg->Delete();
            m_creature->HandleEmoteCommand(EMOTE_ONESHOT_EAT);
            m_creature->GetMotionMaster()->MoveTargetedHome();
            Creature* Credit = GetClosestCreatureWithEntry(m_creature, NPC_QUEST_CREDIT, 50.0f);
            if (pPlayer && Credit)
...
enum
{
    NPC_GATHERER_HORDE         = 22333,
    NPC_GATHERER_ALLIANCE      = 21926,
    RAZAANI_LIGHT_ORB_CREDIT   = 21929
};

struct MANGOS_DLL_DECL razaani_light_orbAI : public ScriptedAI
{
    razaani_light_orbAI(Creature* pCreature) : ScriptedAI(pCreature) {Reset(); GoToHome();}
    ...
    void UpdateAI(const uint32 diff)
    {
        ...
        {
            pGathererH = GetClosestCreatureWithEntry(m_creature, NPC_GATHERER_HORDE, 20.0f);
            pGathererA = GetClosestCreatureWithEntry(m_creature, NPC_GATHERER_ALLIANCE, 20.0f);

            if (!pGathererH && !pGathererA)
                return;
   
            if (pGathererH && pGathererA)
                pGatherer = urand(0,1) ? pGathererH : pGathererA;
            else
                pGatherer = pGathererH ? pGathererH : pGathererA;
    
            // this should not happen!
            if (!pGatherer || !pGatherer->IsInWorld())
                return;
    
            m_creature->SetSpeedRate(MOVE_FLIGHT, 0.4f);
            m_creature->SetSpeedRate(MOVE_RUN, 0.4f);

            m_creature->GetMotionMaster()->MovePoint(0, pGatherer->GetPositionX(), pGatherer->GetPositionY(), pGatherer->GetPositionZ(), true);
    ...
    void MovementInform(uint32 type, uint32 uiPointId)
    {
        ...
        if (uiPointId == 0)
        {
            ...
            Unit* pCreatorGatherer = pGatherer->GetCreator();

            if (!pCreatorGatherer || !pCreatorGatherer->IsInWorld() || !pCreatorGatherer->isAlive() || pCreatorGatherer->GetTypeId() != TYPEID_PLAYER)
            {
                m_creature->ForcedDespawn();
                return;
            }

            ((Player*)pCreatorGatherer)->KilledMonsterCredit(RAZAANI_LIGHT_ORB_CREDIT);
            m_creature->ForcedDespawn();
        }
    }
};

CreatureAI* GetAI_razaani_light_orb(Creature* pCreature)
{
    return new razaani_light_orbAI(pCreature);
}
...
void AddSC_blades_edge_mountains()
{
    ...
    pNewScript = new Script;
    pNewScript->Name = "npc_bloodmaul_brutebane";
    pNewScript->GetAI = &GetAI_npc_bloodmaul_brutebane;
    pNewScript->RegisterSelf();

    pNewScript = new Script;
    pNewScript->Name = "npc_ogre_brute";
    pNewScript->GetAI = &GetAI_npc_ogre_brute;
    pNewScript->RegisterSelf();

    pNewScript = new Script;
    pNewScript->Name = "razaani_light_orb";
    pNewScript->GetAI = &GetAI_razaani_light_orb;
    pNewScript->RegisterSelf();
}
