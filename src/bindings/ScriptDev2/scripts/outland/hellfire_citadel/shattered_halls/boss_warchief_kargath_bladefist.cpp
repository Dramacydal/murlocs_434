...
struct MANGOS_DLL_DECL boss_warchief_kargath_bladefistAI : public ScriptedAI
{
    ...
    void SpawnAssassin()
    {
        m_creature->SummonCreature(NPC_SHATTERED_ASSASSIN, AssassEntrance[0], AssassEntrance[1]+8, AssassEntrance[2], 0, EMPSUMMON_TIMED_OOC_DESPAWN, 24000);
        m_creature->SummonCreature(NPC_SHATTERED_ASSASSIN, AssassEntrance[0], AssassEntrance[1]-8, AssassEntrance[2], 0, EMPSUMMON_TIMED_OOC_DESPAWN, 24000);
        m_creature->SummonCreature(NPC_SHATTERED_ASSASSIN, AssassExit[0], AssassExit[1]+8, AssassExit[2], 0, EMPSUMMON_TIMED_OOC_DESPAWN, 24000);
        m_creature->SummonCreature(NPC_SHATTERED_ASSASSIN, AssassExit[0], AssassExit[1]-8, AssassExit[2], 0, EMPSUMMON_TIMED_OOC_DESPAWN, 24000);
    }
        ...
        else                                                // !m_bInBlade
        {
            ...
            if (m_uiSummonAssistantTimer < uiDiff)
            {
                for (uint32 i = 0; i < m_uiSummoned; ++i)
                {
                    switch (urand(0, 2))
                    {
                        case 0: m_creature->SummonCreature(NPC_HEARTHEN_GUARD, AddsEntrance[0], AddsEntrance[1], AddsEntrance[2], 0.0f, EMPSUMMON_TIMED_OOC_DESPAWN, 20000); break;
                        case 1: m_creature->SummonCreature(NPC_SHARPSHOOTER_GUARD, AddsEntrance[0], AddsEntrance[1], AddsEntrance[2], 0.0f, EMPSUMMON_TIMED_OOC_DESPAWN, 20000); break;
                        case 2: m_creature->SummonCreature(NPC_REAVER_GUARD, AddsEntrance[0], AddsEntrance[1], AddsEntrance[2], 0.0f, EMPSUMMON_TIMED_OOC_DESPAWN, 20000); break;
                    }
                }
