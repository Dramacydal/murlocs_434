...
struct MANGOS_DLL_DECL boss_mekgineer_steamriggerAI : public ScriptedAI
{
    ...
    void SummonMechanichs()
    {
        ...
        DoSpawnCreature(ENTRY_STREAMRIGGER_MECHANIC,5,5,0,0, TEMPSUMMON_TIMED_OOC_OR_CORPSE_DESPAWN, 240000);
        DoSpawnCreature(ENTRY_STREAMRIGGER_MECHANIC,-5,5,0,0, TEMPSUMMON_TIMED_OOC_OR_CORPSE_DESPAWN, 240000);
        DoSpawnCreature(ENTRY_STREAMRIGGER_MECHANIC,-5,-5,0,0, TEMPSUMMON_TIMED_OOC_OR_CORPSE_DESPAWN, 240000);

        if (urand(0, 1))
            DoSpawnCreature(ENTRY_STREAMRIGGER_MECHANIC,5,-7,0,0, TEMPSUMMON_TIMED_OOC_OR_CORPSE_DESPAWN, 240000);

        if (urand(0, 1))
            DoSpawnCreature(ENTRY_STREAMRIGGER_MECHANIC,7,-5,0,0, TEMPSUMMON_TIMED_OOC_OR_CORPSE_DESPAWN, 240000);
    }
