        ...
        switch (m_Phase)
        {
            ...
            case PHASE_SUMMON_AGENTS:
                ...
                if (m_uiPhaseTwoTimer < uiDiff)
                {
                    ...
                    {
                        for (int j = 1; j <= 4; ++j)
                            m_creature->SummonCreature(NPC_SOLARIUM_AGENT, m_aPortals[i][0], m_aPortals[i][1], m_aPortals[i][2], 0, TEMPSUMMON_TIMED_OOC_DESPAWN, 5000);
                    }
            ...
            case PHASE_SUMMON_PRIESTS:
                ...
                if (m_uiPhaseThreeTimer < uiDiff)
                {
                    ...
                    for (int j = 0; j <= 2; ++j)
                        if (j != i)
                            m_creature->SummonCreature(NPC_SOLARIUM_PRIEST, m_aPortals[j][0], m_aPortals[j][1], m_aPortals[j][2], 0, TEMPSUMMON_TIMED_OOC_DESPAWN, 5000);
