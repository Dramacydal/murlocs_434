...
void instance_draktharon_keep::OnCreatureCreate(Creature* pCreature)
{
    switch (pCreature->GetEntry())
    {
        ...
        case NPC_TROLLGORE:
            m_mNpcEntryGuidStore[NPC_TROLLGORE] = pCreature->GetObjectGuid();
            break;
        case NPC_CRYSTAL_CHANNEL_TARGET:
            m_lNovosDummyGuids.push_back(pCreature->GetObjectGuid());
            break;
        case NPC_DRAKKARI_INVADER_1:
        case NPC_DRAKKARI_INVADER_2:
            if (pCreature->GetPositionZ() > 30.0f)
                pCreature->Mount(DRAKKARI_MOUNT);
            else
                pCreature->SetLevitate(false);

            pCreature->SetWalk(false);

            pCreature->GetMotionMaster()->MovePoint(1, -255.0f, -661.0f, 26.5f, false);
            break;
