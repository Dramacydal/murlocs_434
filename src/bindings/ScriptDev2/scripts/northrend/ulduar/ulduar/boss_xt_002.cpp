...
struct MANGOS_DLL_DECL boss_xt_002AI : public ScriptedAI
{
    ...
    void SummonAdds()
    {
        ...
        for (int j = 0; j < (m_bIsRegularMode ? 3 : 5); ++j)
        {
            ...
            uint8 i = urand(0, 3);
            if (Creature* pTemp = m_creature->SummonCreature(addentry, SummonLoc[i].x + urand(0, 10), SummonLoc[i].y + urand(0, 10), SummonLoc[i].z, 0, TEMPSUMMON_TIMED_OOC_DESPAWN, 180000))
            {
