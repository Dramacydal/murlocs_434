...
Player* ScriptedInstance::GetGroupLeader()
{
    Map::PlayerList const& lPlayers = instance->GetPlayers();
    ...
    {
        Player* pPlayer = itr->getSource();
        if (pPlayer && !pPlayer->isGameMaster() && pPlayer->GetGroup() && pPlayer->GetGroup()->GetLeaderGuid() == pPlayer->GetObjectGuid())
            return pPlayer;
    }
...
void ScriptedInstance::SendGMEncounterNotification(Unit* who, uint32 encounter)
{
    if (lastGMEncounterAnnounce + 5 > time(NULL))
        return;

    lastGMEncounterAnnounce = time(NULL);

    bool dead = false;
    bool originalPlayer = true;
    Player* pl = who ? who->GetCharmerOrOwnerPlayerOrPlayerItself() : NULL;
    if (!pl)
    {
        originalPlayer = false;
        pl = GetPlayerInMap(true, false);
        if (!pl)
        {
            pl = GetPlayerInMap(false, false);
            if (!pl)
                return;
    ...
    // Ranger: remove this after proper fix
    #ifndef _WIN32
    if (originalPlayer)
        sWorld.SendGMGlobalText(SEC_MODERATOR, LANG_RAID_BOSS_CHEAT_1, pl->GetName(), pl->GetName(), GetEncounterName(encounter).c_str());
    else
        sWorld.SendGMGlobalText(SEC_MODERATOR, LANG_RAID_BOSS_CHEAT_2, GetEncounterName(encounter).c_str(), pl->GetName(), pl->GetName(), dead ? "(лепрб)" : "");
    #endif

}

void ScriptedInstance::UpdateAchievementCriteria(uint32 type, uint32 miscvalue1, uint32 miscvalue2, Unit *unit, uint32 time)
{
    Map::PlayerList const& lPlayers = instance->GetPlayers();
    ...
    if (!lPlayers.isEmpty())
    {
        ...
        {
            if (Player* pPlayer = itr->getSource())
                pPlayer->UpdateAchievementCriteria(AchievementCriteriaTypes(type), miscvalue1, miscvalue2, unit, time);
        }
    }
    else
