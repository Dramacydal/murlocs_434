...
void FillSpellSummary()
{
    ...
    for (uint32 i = 0; i < GetSpellStore()->GetNumRows(); ++i)
    {
        ...
        for (uint8 j = 0; j < 3; ++j)
        {
            SpellEffectEntry const * effect = pTempSpell->GetSpellEffect(SpellEffectIndex(j));
            if (!effect)
                continue;
            ...
            if (effect->EffectImplicitTargetA == TARGET_CHAIN_DAMAGE ||
                effect->EffectImplicitTargetA == TARGET_CURRENT_ENEMY_COORDINATES)
                SpellSummary[i].Targets |= 1 << (SELECT_TARGET_SINGLE_ENEMY-1);

            //Spell targets AoE at enemy
            if (effect->EffectImplicitTargetA == TARGET_ALL_ENEMY_IN_AREA ||
                effect->EffectImplicitTargetA == TARGET_ALL_ENEMY_IN_AREA_INSTANT ||
                effect->EffectImplicitTargetA == TARGET_CASTER_COORDINATES ||
            ...
            if (effect->EffectImplicitTargetA == TARGET_CHAIN_DAMAGE ||
                effect->EffectImplicitTargetA == TARGET_CURRENT_ENEMY_COORDINATES ||
                effect->EffectImplicitTargetA == TARGET_ALL_ENEMY_IN_AREA ||
                effect->EffectImplicitTargetA == TARGET_ALL_ENEMY_IN_AREA_INSTANT ||
                effect->EffectImplicitTargetA == TARGET_CASTER_COORDINATES ||
                effect->EffectImplicitTargetA == TARGET_ALL_ENEMY_IN_AREA_CHANNELED)
            ...
            if (effect->EffectImplicitTargetA == TARGET_SELF ||
                effect->EffectImplicitTargetA == TARGET_SINGLE_FRIEND ||
                effect->EffectImplicitTargetA == TARGET_SINGLE_PARTY)
                SpellSummary[i].Targets |= 1 << (SELECT_TARGET_SINGLE_FRIEND-1);

            //Spell targets aoe friends
            if (effect->EffectImplicitTargetA == TARGET_ALL_PARTY_AROUND_CASTER ||
                effect->EffectImplicitTargetA == TARGET_AREAEFFECT_PARTY ||
                effect->EffectImplicitTargetA == TARGET_CASTER_COORDINATES)
            ...
            if (effect->EffectImplicitTargetA == TARGET_SELF ||
                effect->EffectImplicitTargetA == TARGET_SINGLE_FRIEND ||
                effect->EffectImplicitTargetA == TARGET_SINGLE_PARTY ||
                effect->EffectImplicitTargetA == TARGET_ALL_PARTY_AROUND_CASTER ||
                effect->EffectImplicitTargetA == TARGET_AREAEFFECT_PARTY ||
                effect->EffectImplicitTargetA == TARGET_CASTER_COORDINATES)
            ...
            if (effect->Effect == SPELL_EFFECT_HEAL ||
                ...
                (effect->Effect == SPELL_EFFECT_APPLY_AURA  && effect->EffectApplyAuraName == 8))
