...
extern int main(int argc, char **argv)
{
    ...
    while ((option = cmd_opts()) != EOF)
    {
        switch (option)
        {
            ...
            case 's':
            {
                ...
                {
                    ERROR_LOG("Runtime-Error: -%c unsupported argument %s", cmd_opts.opt_opt(), mode);
                    usage(argv[0]);
...
#endif
            case ':':
                ERROR_LOG("Runtime-Error: -%c option requires an input argument", cmd_opts.opt_opt());
                usage(argv[0]);
                Log::WaitBeforeContinueIfNeed();
                return 1;
            default:
                ERROR_LOG("Runtime-Error: bad format of commandline arguments");
                usage(argv[0]);
                Log::WaitBeforeContinueIfNeed();
                return 1;
        }
    ...
    {
        ERROR_LOG("Could not find configuration file %s.", cfg_file);
        Log::WaitBeforeContinueIfNeed();
        return 1;
    }
