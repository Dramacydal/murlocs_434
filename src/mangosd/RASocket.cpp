...
int RASocket::open(void* )
{
	...
	sendf("\r\n");
	CliCommandHolder* cmd = new CliCommandHolder(0, SEC_CONSOLE, this, ".server info", &RASocket::zprint, &RASocket::commandFinished);
...
int RASocket::handle_input(ACE_HANDLE)
{
    ...
    if(closing_)
    {
        ERROR_LOG("Called RASocket::handle_input with closing_ = true");
        return -1;
    }
